...
- budget/HR/org-management simulator or messenger-first surfaces
...
`DEC-042` records the accepted post-v1 pivot direction, `DEC-044` records the earlier safe display-layer baseline, and `DEC-045` widens the shell into a fuller company/ERP command-center posture. Any implementation that changes the default user-facing product entry toward `Mission / Council / Execution / Deliverables` must still preserve the frozen v1 engine contracts unless a later explicit decision changes them.
...
- current `main` keeps that priority deferred rather than active: no second provider adapter is needed for current completion, and provider expansion should be reconsidered only if optional real-live housekeeping or a concrete operator gap demonstrates that the current `openai-responses` boundary is insufficient.
...
- the first repo-content redaction implementation is locked to `preview/export only`: it must not create redacted derivative copies and must not replace the current raw stored artifact content plus metadata as the source of truth.
...
- current `main` now applies the first preview-only repo-content redaction to `Artifacts -> Artifact Detail -> change-summary` structured preview for `File Updates`; file paths remain visible in the structured preview while stored repo-content payloads stay redacted there and remain available only in the unchanged raw-content block below.
...
- the operator-facing commit-side label is fixed to `Resume Approved Local Commit`, and the executing CTA belongs only in the existing `Task Detail` commit guard area. `Artifacts` and `Decision Inbox` may at most surface provenance/readiness context or a navigation hint back to that panel.
- current `main` implements that bounded form directly in `Task Detail`: the local commit CTA uses `Resume Approved Local Commit`, while `Artifacts` and `Decision Inbox` now stay navigation-only for commit continuation and route back to the same guard area instead of exposing a second executing CTA.
...
- the operator-facing release-side label is fixed to `Resume Approved Close Out`, and the executing CTA belongs only in the existing `Task Detail` release/close-out guard area. `Artifacts` and `Decision Inbox` may only show navigation hints when a current approved release bundle exists; stale or blocked bundles continue to show guard reasons without a resume hint.
- current `main` implements that bounded release-side form directly in `Task Detail`: the close-out CTA uses `Resume Approved Close Out`, while `Artifacts` and `Decision Inbox` stay navigation-only for close-out follow-up and route back to the same guard area instead of exposing a second executing CTA.
