...
- `provider-slice-02` implements the first concrete live provider as `OpenAI Responses API` for `planner`, `provider-slice-03` extends that same adapter boundary to `architect`, `provider-slice-04` extends it to `task-breaker`, and `provider-slice-05` extends it to `builder-preflight` without widening `builder-live-mutation`, `reviewer`, `release-package`, or `close-out` execution.
