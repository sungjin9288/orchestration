...
- `provider-slice-02` implements the first concrete live provider as `OpenAI Responses API` for `planner`, `provider-slice-03` extends that same adapter boundary to `architect`, `provider-slice-04` extends it to `task-breaker`, and `provider-slice-05` extends it to `builder-preflight` without widening `builder-live-mutation`, `reviewer`, `release-package`, or `close-out` execution. `strategy-slice-06` and `strategy-slice-07` define the next builder-live-mutation and reviewer boundaries only; neither widens commit-package, local commit, release-package, or close-out execution.
...
- Verification direction stays local-first: keep the current `local-stub` regression gate unchanged, keep planner plus architect plus task-breaker plus builder-preflight live regression coverage unchanged, add builder-live-mutation-specific synthetic opt-in smoke coverage before any live mutation ships, and limit optional real live smoke to explicit opt-in planner plus architect plus task-breaker plus builder-preflight coverage only until `strategy-slice-06` and `strategy-slice-07` are implemented.
...
## Defined Builder Live-Mutation Boundary
- `strategy-slice-06` defines the next live-role boundary only. The implemented live runtime remains planner plus architect plus task-breaker plus builder-preflight until a later `provider-slice-06`; `reviewer` continues to stay blocked and degraded in live mode, and no release-package or close-out semantics widen.
...
- Config and readiness should stay narrow: the current project provider config shape remains `mode`, canonical adapter id, operator-pinned `model`, and env-var name only; project-level provider summary stays coarse readiness; role-specific readiness may open at most `planner + architect + task-breaker + builder-preflight + builder-live-mutation`, while `reviewer` remains degraded and blocked.
...
- Reviewer live remains outside this slice. In a live-mode project, the post-mutation reviewer step stays blocked and degraded as an explicit operator step rather than a silent fallback; the operator may change provider mode deliberately later, but this boundary does not expand reviewer live execution.

## Defined Reviewer Live Boundary
- `strategy-slice-07` defines the next downstream live-role boundary only. The implemented live runtime remains planner plus architect plus task-breaker plus builder-preflight until later `provider-slice-06`; reviewer remains blocked and degraded until later `provider-slice-07`, and commit-package, local commit, release-package, and close-out semantics do not widen.
