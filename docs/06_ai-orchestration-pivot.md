...
## Shell Posture
새 기본 shell은 marketing-style website나 ERP dashboard가 아니라 LLM-native orchestration
workspace처럼 읽혀야 한다.

- 첫 viewport의 중심은 회사 디렉터리나 상태판이 아니라 operator의 goal composer다.
- 역할은 큰 roster보다 `operator goal -> role position -> synthesis -> bounded execution -> review -> delivery` workstream 안에서 실제 발언과 책임으로 나타난다.
- workstream은 generic messenger bubble이 아니라 역할, 상태, 근거, 다음 gate를 가진 구조화된 turn을 사용한다.
- `Mission / Council / Execution / Deliverables`는 각각 목표, 역할 정렬, 실행, 결과라는 기본 동선으로 유지한다.
- underlying authority는 여전히 local-first execution engine과 `advanced ops mode`에 남고, prompt submit 자체는 provider, source mutation, commit, push, release 권한을 넓히지 않는다.
- company/ERP 표현은 optional context metaphor로만 남기며 primary shell의 정보 구조를 지배하지 않는다.
