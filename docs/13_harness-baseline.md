...
- `ui/app.js` consumes only that snapshot-derived brief in the `Ops overview` signal strip plus read-only `하네스 실행 안내` registers inside the `Ops overview` org panel, `Taskboard`, `Logs`, `Artifacts`, and `Decision Inbox`, and does not reopen `doctor` or recompute host posture from raw harness arrays
- keeps the layering explicit: `doctor.summary -> consumer status -> consumer brief -> snapshot derived -> ops overview signal + register -> taskboard register -> logs register -> artifacts register -> decision-inbox register`
