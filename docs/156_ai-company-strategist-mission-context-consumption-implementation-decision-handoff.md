...
- Existing v29 records remain legacy evidence; only the first valid context-bound write may migrate
  the state to v30.
- Exact replay resolves before mutable source validation and performs no save or adapter call.
- The attachment, MemoryItem, MemoryRecall, Mission, StaffingPlan, source, provider, prompt, policy,
  ExecutionPlan, WorkOrder, Git, and release bytes remain outside the new mutation boundary.
