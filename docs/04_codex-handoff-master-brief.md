...
## Immediate Priority
The current source `main` baseline is `7bfa36d2a4e38a96cde2954c9b2ad1a41a153ff5` after Dogfood Run 076 cleanup-completed evidence was committed and pushed. Dogfood Run 077 has executed after explicit approval; linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-074` and branch `worktree/v1-dogfood-runner-074` are retained dirty by design, while runtime evidence remains under `var/runtime-v1-dogfood-runner-v1-dogfood-runner-074`.
...
- current `main` publish state is reported by `git status --short --branch`; Dogfood Run 076 cleanup-completed evidence is published, and Dogfood Run 077 retained-evidence is local until commit/push/cleanup approval
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 077 evidence is recorded
- Current published-head V1 kickoff proof passed at head `7bfa36d2a4e38a96cde2954c9b2ad1a41a153ff5` before Dogfood Run 077 execute
...
- Dogfood Run 077 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-074` and branch `worktree/v1-dogfood-runner-074` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
...
- approve the local retained-evidence commit for Dogfood Run 077 after verification
- approve publishing the Run 077 retained-evidence commit to `origin/main` after it is committed locally
- approve destructive cleanup for the Dogfood Run 077 retained linked worktree and branch only after retained-evidence is preserved
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 077 retained-evidence is committed/published and the cleanup decision is settled
