...
## Immediate Priority
The current source `main` published evidence baseline is `79164ba` after Dogfood Run 083 cleanup-completed evidence was committed and pushed. Dogfood Run 084 execute completed after explicit `run-another-dogfood-execute` approval; linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-081` and branch `worktree/v1-dogfood-runner-081` are retained dirty by design while retained-evidence is being recorded locally before commit/push/cleanup.
...
- current `main` publish state is reported by `git status --short --branch`; Dogfood Run 083 cleanup-completed evidence is published as `79164ba`, while Dogfood Run 084 retained-evidence is being recorded locally before commit/push/cleanup
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 084 evidence is recorded
- Dogfood Run 084 executed from published head `79164ba685cbd51e88da24fc225436e687a08870`; retained-evidence is being recorded locally before commit/push/cleanup; latest V1 kickoff runtime/browser proof remains recorded at head `aae311fa16dafdc8ca1bc3054148eb0df26b4523`
...
- Dogfood Run 084 retained-evidence is being recorded locally before commit/push/cleanup
- Dogfood Run 084 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-081` and branch `worktree/v1-dogfood-runner-081` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
...
- approve the local retained-evidence commit for Dogfood Run 084 after verification
- approve destructive cleanup for the Dogfood Run 084 retained linked worktree and branch only after retained-evidence is preserved
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 084 retained-evidence is committed and the cleanup decision is settled
