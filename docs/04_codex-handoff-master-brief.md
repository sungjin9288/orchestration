...
## Immediate Priority
The current source `main` published evidence baseline is `a3cfc4d` after the workflow handoff clarity and review-passed routing fixes were committed and pushed. Dogfood Run 087 executed from that clean/published head with slug `v1-dogfood-runner-084`; linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-084` and branch `worktree/v1-dogfood-runner-084` remain intentionally dirty by design while retained-evidence is being recorded locally before commit/push/cleanup.
...
- current `main` publish state is reported by `git status --short --branch`; Dogfood Run 086 cleanup-completed evidence was published as `efbd1c1`, the latest published head is `a3cfc4d`, and Dogfood Run 087 retained-evidence is being recorded locally before commit/push/cleanup
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 087 evidence is recorded
- Dogfood Run 087 executed from published head `a3cfc4d66cc871ce03c7a60ea65e85c0a83f6ad0`; retained-evidence is being recorded locally before commit/push/cleanup; latest V1 kickoff runtime/browser proof remains recorded at head `aae311fa16dafdc8ca1bc3054148eb0df26b4523`
...
- Dogfood Run 087 retained-evidence is being recorded locally before commit/push/cleanup
- Dogfood Run 087 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-084` and branch `worktree/v1-dogfood-runner-084` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
...
- approve the local retained-evidence commit for Dogfood Run 087 after verification
- approve destructive cleanup for the Dogfood Run 087 retained linked worktree and branch only after retained-evidence is preserved
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 087 retained-evidence is committed/published and the cleanup decision is settled
