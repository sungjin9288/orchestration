...
## Immediate Priority
The current source `main` published baseline is `5ed1c3f` after Dogfood Run 087 cleanup-completed evidence and the operator home runway slice were committed and pushed. Dogfood Run 088 executed after explicit `run-another-dogfood-execute` approval; linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-085` and branch `worktree/v1-dogfood-runner-085` are retained dirty by design while retained-evidence is recorded locally before commit/push/cleanup.
...
- current `main` publish state is reported by `git status --short --branch`; current published head is `5ed1c3f`, and Dogfood Run 088 retained-evidence is being recorded locally before commit/push/cleanup
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 088 evidence is recorded
- Dogfood Run 088 executed from published head `5ed1c3f812202a53c90982d87a040dd6f8d75785`; retained-evidence is being recorded locally before commit/push/cleanup; latest V1 kickoff runtime/browser proof remains recorded at head `aae311fa16dafdc8ca1bc3054148eb0df26b4523`
...
- Dogfood Run 088 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-085` and branch `worktree/v1-dogfood-runner-085` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
- Dogfood Run 088 runtime evidence remains available under `var/runtime-v1-dogfood-runner-v1-dogfood-runner-085`
- Dogfood Run 088 retained-evidence is being recorded locally before commit/push/cleanup
...
- approve the local retained-evidence commit for Dogfood Run 088 after verification
- approve destructive cleanup for the Dogfood Run 088 retained linked worktree and branch only after retained-evidence is preserved
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 088 retained-evidence is committed/published and the cleanup decision is settled
