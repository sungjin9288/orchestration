...
## Immediate Priority
The last clean/published v1 development baseline is `main` at `dae8891d66301dc80aad1ea5a2f06a772a9794db` after Dogfood Run 050 retained and cleanup evidence was published. Dogfood Run 051 executed from that clean/published baseline with slug `v1-dogfood-runner-048`; its retained linked worktree is preserved dirty by design until retained-evidence is committed and destructive cleanup is explicitly approved.
...
- current `main` publish state is reported by `git status --short --branch`; after Run 051 retained-evidence docs are recorded, `main` is expected to be dirty until the retained-evidence commit is created, then cleanup and publish remain blocked until explicitly approved
- `node scripts/verification_status.mjs` must remain green after the Run 051 retained-evidence update
- Dogfood Run 001 through Dogfood Run 051 evidence is recorded
...
- Dogfood Run 051 retained-evidence is being recorded locally before commit, cleanup, or push
...
- approve the local retained-evidence commit for Dogfood Run 051 after verification
- approve destructive cleanup for the Dogfood Run 051 retained linked worktree and branch after retained-evidence is preserved
- publish only after retained-evidence is committed and explicit push approval is given
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 051 retained-evidence is committed and cleanup lifecycle is settled
