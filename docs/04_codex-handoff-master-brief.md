...
## Immediate Priority
The last clean/published v1 development baseline is `main` at `48bb3d95327f523514c670ef55c60a53b7c24264` after Dogfood Run 036 cleanup-completed evidence was published. Dogfood Run 037 retained-evidence commit `98a8122` was created locally before cleanup, the retained linked worktree cleanup has completed, and this cleanup-completed evidence update restores the no-retained-worktree baseline once committed and pushed.
...
- current `main` publish state is reported by `git status --short --branch`; during the Run 037 cleanup-completed evidence update, `main` is expected to be dirty until the cleanup-completed commit is created, then ahead of `origin/main` until the approved push publishes it
- `node scripts/verification_status.mjs` must remain green after the Run 037 cleanup-completed evidence update
- Dogfood Run 001 through Dogfood Run 037 evidence is recorded
- Dogfood Run 037 retained-evidence is committed locally as `98a8122`
