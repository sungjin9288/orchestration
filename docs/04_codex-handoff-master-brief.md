...
## Immediate Priority
The current source `main` baseline is `13d600b0cc593f0e05549fbed38e87d84aefdd94` after Dogfood Run 073 cleanup-completed evidence was committed and pushed. Dogfood Run 074 has now executed from clean/published `main`; linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-071` and branch `worktree/v1-dogfood-runner-071` are intentionally retained dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval, while runtime evidence remains under `var/runtime-v1-dogfood-runner-v1-dogfood-runner-071`.
...
- current `main` publish state is reported by `git status --short --branch`; Dogfood Run 073 cleanup-completed evidence is published, and Dogfood Run 074 retained-evidence is local until commit/push approval
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 074 evidence is recorded
- Current published-head V1 kickoff proof passed at head `13d600b0cc593f0e05549fbed38e87d84aefdd94` before Dogfood Run 074 execute
...
- Dogfood Run 074 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-071` and branch `worktree/v1-dogfood-runner-071` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
- Dogfood Run 074 runtime evidence remains available under `var/runtime-v1-dogfood-runner-v1-dogfood-runner-071`
- Dogfood Run 074 retained-evidence is being recorded locally before commit/push/cleanup
...
- approve the local retained-evidence commit for Dogfood Run 074 after verification
- approve publishing the Run 074 retained-evidence commit to `origin/main` after it is committed locally
- approve destructive cleanup for the Dogfood Run 074 retained linked worktree and branch only after retained evidence is preserved
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 074 retained-evidence is committed and the cleanup decision is settled
