...
## Immediate Priority
The last clean/published v1 development baseline is `main` at `bcf040258a02c6d12244cf3ed75dde1fe302d897` after Dogfood Run 027 retained evidence was published. Dogfood Run 027 retained linked worktree cleanup has now completed after explicit destructive cleanup approval; cleanup-completed evidence remains local until a separate commit and push approval closes it.
...
- current `main` publish state is reported by `git status --short --branch`; after the Run 027 cleanup-completed evidence commit, `main` is expected to be ahead of `origin/main` until explicit push approval is given
- `node scripts/verification_status.mjs` must remain green after the Run 027 cleanup-completed evidence update
- Dogfood Run 001 through Dogfood Run 027 evidence is recorded
- push completed before Dogfood Run 027 cleanup; Dogfood Run 027 retained-evidence docs are already published on current `origin/main`
...
- approve a local cleanup-completed evidence commit for Dogfood Run 027
