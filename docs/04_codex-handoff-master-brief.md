...
## Immediate Priority
The current source `main` published evidence baseline is `3ce74da` after Dogfood Run 093 cleanup-completed evidence was committed and pushed. Dogfood Run 094 execute has now completed after explicit operator approval; retained-evidence is being recorded locally before retained-evidence commit/push.
...
- current `main` publish state is reported by `git status --short --branch`; Dogfood Run 094 retained-evidence is being recorded locally before commit/push
- `node scripts/verification_status.mjs` must remain green after any future evidence update
- Dogfood Run 001 through Dogfood Run 094 evidence is recorded
...
- Dogfood Run 094 retained linked worktree `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-091` and branch `worktree/v1-dogfood-runner-091` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
...
- Dogfood Run 094 retained linked worktree path `/Users/sungjin/dev/personal/orchestration--v1-dogfood-runner-091` and branch `worktree/v1-dogfood-runner-091` remain intentionally dirty by design until retained-evidence commit/publish and explicit destructive cleanup approval
- Dogfood Run 094 runtime evidence remains available under `var/runtime-v1-dogfood-runner-v1-dogfood-runner-091`
- Dogfood Run 092 cleanup-completed evidence was committed and published as `44550a6` before Dogfood Run 093 execute
...
- commit Dogfood Run 094 retained-evidence docs locally only after verification and explicit commit approval
- publish the Dogfood Run 094 retained-evidence commit only after explicit `git push origin main` approval
- clean up the Dogfood Run 094 retained linked worktree and branch only after retained-evidence is preserved and explicit destructive cleanup approval is given
...
- do not run another intentional `--execute --slug <slug>` dogfood pass until Run 094 retained-evidence is committed/published and the cleanup decision is settled
