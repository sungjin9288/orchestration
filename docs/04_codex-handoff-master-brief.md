...
## Immediate Priority
The last clean/published v1 development baseline is `main` at `be50c1d2d772a88f0a18a19140d3e511a87ebc01` after Dogfood Run 049 retained and cleanup evidence was published. Dogfood Run 050 executed from that clean/published baseline with slug `v1-dogfood-runner-047`; its retained linked worktree is preserved dirty by design until retained-evidence is committed and explicit destructive cleanup is approved.
...
- current `main` publish state is reported by `git status --short --branch`; after Run 050 retained-evidence docs are recorded, `main` is expected to be dirty until the retained-evidence commit is created, then cleanup and publish remain blocked until explicitly approved
- `node scripts/verification_status.mjs` must remain green after the Run 050 retained-evidence update
- Dogfood Run 001 through Dogfood Run 050 evidence is recorded
...
- approve the local retained-evidence commit for Dogfood Run 050 after verification
- approve destructive cleanup for the Dogfood Run 050 retained linked worktree and branch only after retained-evidence is preserved
- publish only after retained-evidence is committed and explicit push approval is given
- do not run another intentional `--execute --slug <slug>` dogfood run until Run 050 retained-evidence is committed and cleanup lifecycle is settled
