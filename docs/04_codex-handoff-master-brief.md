...
## Immediate Priority
The last clean/published v1 development baseline is `main` at `db9ce6bb8ed261dda08baa82c5cedcf44fee1b4c` after Dogfood Run 031 cleanup-completed evidence was published. Dogfood Run 032 retained-evidence commit `a0b3677` was created locally before cleanup, the retained linked worktree cleanup has completed, and this cleanup-completed evidence update restores the no-retained-worktree baseline once committed and pushed.
...
- current `main` publish state is reported by `git status --short --branch`; during the Run 032 cleanup-completed evidence update, `main` is expected to be dirty until the cleanup-completed commit is created, then ahead of `origin/main` until the approved push publishes it
- `node scripts/verification_status.mjs` must remain green after the Run 032 cleanup-completed evidence update
- Dogfood Run 001 through Dogfood Run 032 evidence is recorded
- Dogfood Run 032 retained-evidence is committed locally as `a0b3677`
