...
- current `main` publish state is reported by `git status --short --branch`; during the Run 044 cleanup-completed evidence update, `main` is expected to be dirty until the cleanup-completed evidence commit is created, then ahead of `origin/main` until the approved push publishes it
- `node scripts/verification_status.mjs` must remain green after the Run 044 cleanup-completed evidence update
...
- approve the cleanup-completed evidence commit for Dogfood Run 044 after verification
