...
decisions가 runtime을 schema v17까지 확장했고 one exact immutable durable StaffingPlan
preview/accept/persist/inspect path를 추가했다. Accepted-plan Council/solo binding, general scheduler,
parallel specialists, automatic rework, and Mission memory application은 아직 없다.
...
schema-v6 foundation을 만들었고, current persisted runtime은 schema v17이며 company policy는 여전히 `state.json`에 저장되지 않는다.
