...
decisions가 runtime을 schema v18까지 확장했고 one exact immutable durable StaffingPlan
...
schema-v6 foundation을 만들었고, current persisted runtime은 schema v18이며 company policy는 여전히 `state.json`에 저장되지 않는다.
