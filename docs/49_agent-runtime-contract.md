...
decisions가 runtime을 schema v16까지 확장했지만 standalone durable StaffingPlan, general
scheduler, parallel specialists, automatic rework, and Mission memory application은 아직 없다.
...
schema-v6 foundation을 만들었고, current persisted runtime은 schema v16이며 company policy는 여전히 `state.json`에 저장되지 않는다.
...
Standalone StaffingPlan, general scheduling, parallel specialists, and profile mutation은 별도
fielded decision 전까지 구현하지 않는다.
...
preview만 반환한다. `loadStateReadonly()` path는 current schema v16을 읽고 save/record mutation을 만들지 않는다. Automatic search/ranking/
...
current schema v16에서도 one exact recorded recall,
