...
decision handoff는 `DEC-120`, exact implementation은 `DEC-121`로 기록됐다. The implemented path recomputes DEC-118 from one exact
...
`MemoryItem(status=stored)`. Recommendation retrieval/application, import/export/delete/refresh,
expiry mutation, cross-workspace use, skill,
...
handoff는 `DEC-123`, exact response-only implementation은 `DEC-124`로 기록됐다. The implemented
schema-preserving slice lets one exact operator-selected unexpired stored item plus bounded project-local
recallSpec produce a reviewable response/browser-memory preview, but cannot enumerate, search, rank,
recommend, inject Mission or WorkOrder context, apply memory, create a durable recall record, call
...
handoff는 `DEC-129`, exact implementation은 `DEC-130`으로 기록됐다. `docs/86_ai-company-mission-memory-context-preview-plan.md`는 one
...
and connectors를 열지 않는다. Runtime/API/UI는 exact-id form, canonical Mission digest, 64 KiB
...
Stage 7B planning is recorded as `DEC-228`, the complete schema-v30 implementation handoff as
`DEC-229`, and the exact implementation as `DEC-230`. The path starts only from an accepted StaffingPlan through StaffingEntry into a
...
connector use remain blocked. The runtime/API/UI implementation stops at human alignment and exact
receipt inspection.
...
sourceProgressDigest만 다루며 `allowedActions=[]`로 멈춘다. `DEC-188`은 exact runtime/API/UI
...
ReworkPlan은 immutable `review-required`로 남고, `DEC-194`는 exact accepted evidence append와
...
`DEC-196`은 complete fielded implementation handoff를 기록하고, `DEC-197`은 exact
schema-v24 sidecar, active-before-worker attempt, local-stub no-write preflight, exact inspection을
구현한다. 이 경계는 mutation Approval 생성 전에서 멈춘다.
...
boundary. `DEC-197` implements only that exact sidecar path. Mutation Approval, source mutation,
Reviewer/QA execution, retry, recovery, scheduling, and provider-backed authority remain closed.
...
Stage 5H planning-only `DEC-207`, handoff-only `DEC-208`, and implementation `DEC-209` define one dedicated QA path
...
Stage 5I planning-only `DEC-210`, handoff-only `DEC-211`, and implementation `DEC-212` define one
schema-v24-preserving response-only `ReworkDeliveryPackagePreview` from exact DEC-203 mutation, DEC-206 Reviewer,
...
Implemented durable MemoryItem target:
...
The `DEC-121` implementation separates readiness from storage approval, requires exact DEC-118
recomputation, and adds only one sequence/map plus one immutable project-scoped stored record.
Retrieval, recommendation, application, export, deletion, refresh, cross-workspace use, skill,
provider, source/Git/release, scheduling, policy, bypass, and connector authority remains blocked.
