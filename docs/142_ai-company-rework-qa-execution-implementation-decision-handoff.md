...
targetAuthority=one explicit local-stub shell-free QA execution from one exact passed schema-v24 DEC-206 Reviewer re-execution using the existing QA WorkOrder and WorkOrderAttempt #1 and stopping before DeliveryPackage Mission close-out Git or release
