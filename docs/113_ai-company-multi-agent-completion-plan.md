...
Stage 5G planning-only `DEC-204`, handoff-only `DEC-205`, and implementation `DEC-206` define the
one allowed return to this
...
Stage 5I planning-only `DEC-210`, handoff-only `DEC-211`, and implementation `DEC-212` define the
no-write boundary: one
...
replay, and rollback. `DEC-206` implements only that exact path, recomputes mutation evidence from
raw Artifact bytes during settlement and reload, and suppresses generic QA step and recovery at
the Stage 5G checkpoint. QA execution, another attempt or rework, retry, recovery, provider
...
rollback, and focused verification. Implementation `DEC-212` consumes only that exact boundary, and
...
Planning-only `DEC-213` fixes Stage 5J as one future schema-v25 immutable
...
source-worker-stop confirmation, one immutable schema-v28 `OpsAttemptResume`, and one
replacement QA WorkOrderAttempt attempt #2. `DEC-223` records the complete fielded implementation
handoff. `DEC-224` implements the bounded schema/runtime/API/UI path: the worker remains shell-free,
settles only its exact replacement attempt id, preserves the source attempt and quarantine bytes,
and stops at delivery-ready or terminal QA failure. Builder, Reviewer,
...
Stage 7A planning-only `DEC-225` separates durable evidence from runtime consumption. One exact
