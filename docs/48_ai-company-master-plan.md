...
- `company/blueprint.json`과 `company/roles/*.md`는 strict validation을 통과한 source-backed
  ...
  envelope로 노출된다.
- Persisted runtime은 schema v30이다. Durable ExecutionPlan, WorkOrder, HandoffPacket,
  ...
  OpsAttemptDisposition quarantine evidence, exact QA-only OpsAttemptResume evidence, and immutable
  reviewed MissionContextAttachment evidence를 보존한다.
...
- Durable StaffingPlan은 one active draft Mission에서 response-only preview, separate exact
  ...
  `DEC-222` plans the first Stage 6C recovery boundary as one schema-v28 QA-only safe-checkpoint
  ...
  `DEC-229` records the complete schema-v30 implementation handoff, and `DEC-230` implements the
  exact context-bound route, Strategist-only first-attempt consumption, exact receipt inspection,
  generic snapshot redaction, and downstream scheduler block.
...
- Recorded decisions: `DEC-163` through `DEC-230`
...
- Stage 7A planning: `DEC-225` fixes one schema-v29 immutable MissionContextAttachment from
...
- Stage 7B planning: `DEC-228` fixes one exact schema-v30 first-attempt local-stub Council entry
  ...
  scheduler, source, Git, release, and connector authority remain blocked. Exact bounded
  implementation is accepted as `DEC-230`.
...
- Stage 5F planning: `DEC-201` fixes one dedicated local-stub source mutation that reuses
  WorkOrderAttempt #3, keeps DEC-200 Approval immutable, validates source-derived targets and
  baseline digests, owns rollback and interruption evidence through existing Run/Artifact
...
- Stage 5H planning: `DEC-207` fixes one existing QA WorkOrder plus one new WorkOrderAttempt #1 over exact DEC-206
  ...
  fielded implementation handoff
- Stage 5H implementation: `DEC-209` atomically persists the QA attempt and running Run, binds
...
- Stage 5I implementation: `DEC-210` fixes one dedicated schema-v24 response-only
...
- Stage 5J implementation: `DEC-213` fixes one schema-v25 sequence/map-only immutable
  ...
  and `DEC-215` implements only record creation, replay, and exact inspection
- Stage 5K planning: `DEC-216` fixes one future schema-v26 append-only
  `ReworkDeliveryPackageAcceptance` from an exact source-current DEC-215 package; `DEC-217` records
  the complete fielded implementation handoff and reserves implementation for exact `DEC-218`
...
Planning-only `DEC-213`, handoff-only `DEC-214`, and implementation `DEC-215` define the
record-and-inspect-only Stage 5J boundary. The dedicated record is recomputed from DEC-212 and does
...
settlement. Exact `DEC-221` implements only that boundary. Stage 6C planning-only `DEC-222`,
...
schema-v29 immutable MissionContextAttachment from an exact current DEC-130 preview and separate
operator review. Exact attachment persistence and Mission-bound inspection exist, while Strategist/planner consumption, Mission or
...
blocked from every downstream scheduler or WorkOrder path. Wider context consumption requires a later decision.
