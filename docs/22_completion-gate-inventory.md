...
| --- | --- | --- | --- | --- |
| Required aggregate synthetic gate | pass | `node scripts/verification_status.mjs` | `ok=true`; required `1/1`; informational `97/97` | Keep as the default required docs/runtime aggregate gate. |
...
| Growth engine post-completion routing | pass | `node scripts/growth-engine-status.mjs`, `node scripts/growth-reflection-evaluator.mjs`, `node scripts/smoke-growth-engine-status.mjs`, `node scripts/smoke-growth-reflection-evaluator.mjs` | Growth status and reflection now route the zero-open baseline through read-only `growth-evidence-ledger-proposal-queue-handoff` after Growth Evidence Ledger proposal readiness is implemented, and keep the source-mutation lifecycle chain as supporting evidence only | Start proposal queue handoff as status/doc-smoke evidence only before any runtime, UI, memory, provider, proposal-generation, proposal-application, queue mutation, or source-mutation expansion. |
...
- growth engine routing now keeps the next default vNext workstream on
  `growth-evidence-ledger-proposal-queue-handoff` after Growth Evidence Ledger proposal readiness is
