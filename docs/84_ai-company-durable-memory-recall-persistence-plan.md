...
Planning authority는 schema-v15 후보와 exact persistence contract까지만 열었다. Runtime, API, UI,
schema migration, record creation은 이후 complete fielded `DEC-127`에서 별도로 승인됐다.
...
- At planning time, runtime state was schema v14 with immutable `MemoryItem(status=stored)` records.
...
- Schema/runtime/API/UI implementation: accepted as `DEC-127` and implemented as the exact bounded slice.
- Current runtime is schema v15 with additive `memoryRecall` sequence/map, DEC-124 response/browser-
  memory preview, one exact durable persistence command, and exact source-bound inspection.
- List/history/index, automatic retrieval/search/ranking/recommendation, Mission or WorkOrder injection,
  memory application, provider/source/Git/release/scheduling/policy/bypass/connectors remain blocked.
