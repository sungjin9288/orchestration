...
- inspect the Dogfood Run 085 cleanup-completed evidence update, evidence inventory, and current published-head kickoff evidence triage; only open a new implementation slice for a concrete regression or usability issue
...
- commit Dogfood Run 085 cleanup-completed evidence docs locally only after verification and explicit commit approval
- publish the Dogfood Run 085 cleanup-completed evidence commit only after explicit `git push origin main` approval
- do not run another intentional `--execute --slug <slug>` dogfood pass until Run 085 cleanup-completed evidence is committed/published and the push decision is settled
