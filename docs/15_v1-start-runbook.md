...
- inspect the Dogfood Run 052 cleanup-completed evidence; only open a new implementation slice for a concrete regression or usability issue
...
- commit Dogfood Run 052 cleanup-completed evidence locally after verification
- publish Dogfood Run 052 cleanup-completed evidence only after the cleanup-completed commit is created locally and explicit push approval is given
- do not run another intentional `--execute --slug <slug>` dogfood pass until Dogfood Run 052 cleanup-completed evidence is committed and the push decision is settled
