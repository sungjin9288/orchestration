...
Phase 7 recovery planning은 `DEC-095`, implementation handoff는 `DEC-096`으로 기록됐다. Council
approval이나 prior checkpoint만으로 restart resume이 열리지 않는다. Future resume은 current digest와
...
고정한다. Phase 7 planning smoke는 runtime을 바꾸지 않고 safe resume/quarantine fielded gate를 고정한다.
