...
`DEC-129`, exact implementation은 `DEC-130`으로 기록됐다. Council은 recorded recall이나 target Mission을 자동 선택하거나 relevance를
score/rank/recommend하지 않는다. Response-only preview는 operator-selected exact recall과 exact
...
Planning-only `DEC-177` and handoff-only `DEC-178` define a fixed Researcher/QA evidence batch
...
OpsSupervisionPreview over an operator-selected active WorkOrder or specialist attempt. `DEC-185`
implements only that response-only path, keeps `allowedActions=[]`, and cannot alter Council
...
Planning-only `DEC-186` and handoff-only `DEC-187` define one response-only Reviewer rework preview
over a source-current StaffingEntry-bound Council delivery path. `DEC-188` implements only that
exact inspection path: it normalizes `changes-requested` findings and inherited execution scope but
...
Planning-only `DEC-198` and handoff-only `DEC-199` define Stage 5E as an exact Builder-rework
...
QA re-execution. Planning-only `DEC-204`, handoff-only `DEC-205`, and implementation `DEC-206`
authorize one exact local-stub Reviewer attempt #2 with a pass-side stop at `QA_READY`. This does
not reopen Council revision and does not authorize QA execution or any broader graph transition;
...
Planning-only `DEC-213` and handoff-only `DEC-214` also remain outside Council authority. The
future Stage 5J candidate may retain one exact DEC-212 projection as an immutable
