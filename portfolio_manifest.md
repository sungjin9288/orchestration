...
- external share status: `scripts/portfolio-share-status.mjs`는 package readiness, verified-link state, optional local static page readiness, generated site bundle readiness, optional live-provider env visibility, remaining blockers를 read-only JSON으로 요약한다. `PORTFOLIO_LOCAL_SHARE_PAGE_DIR`가 설정된 경우에만 local share page files, package checksum, claim-safe text, generated dist manifest/zip checksum, local git state를 읽는다.
