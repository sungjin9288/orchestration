...
- `docs/129_ai-company-durable-reviewer-rework-plan.md`
...
- `docs/147_ai-company-rework-delivery-package-acceptance-plan.md`
- `docs/148_ai-company-rework-delivery-package-acceptance-implementation-decision-handoff.md`
- `docs/149_ai-company-ops-attempt-quarantine-plan.md`
...
The implementation now strictly loads one repo-backed blueprint and nine role contracts. Persisted
  execution state is schema v30 after the bounded Strategist context-consumption migration, while
...
`DEC-166` consumes that handoff and implements one source-current response-only preview, separate
exact acceptance, atomic schema-v16-to-v17 migration plus immutable StaffingPlan append, exact-id
...
bound revision/resume/auto-chain, bounded read-only parallel specialists, Reviewer rework, Ops
recovery, reviewed Mission context attachment, provider expansion, and dogfood remain separate
verified stages. `DEC-170` records Stage 3 operator-stepped WorkOrder scheduler planning in
`docs/117_ai-company-operator-stepped-workorder-scheduler-plan.md`; `DEC-171` records the complete
fielded schema-v19 implementation handoff in
`docs/118_ai-company-operator-stepped-workorder-scheduler-implementation-decision-handoff.md`.
`DEC-172` implements the bounded local path: one exact source-current StaffingEntry-bound
Builder/Reviewer/QA plan, one active-before-coordinator WorkOrderAttempt per explicit start or step,
...
StaffingPlan policy remains disabled. Planning-only `DEC-180` defines the schema-v21 failed-cell
retry boundary that keeps the source batch and both first attempts immutable, and `DEC-181` records
its complete fielded implementation handoff in
`docs/123_ai-company-specialist-cell-retry-plan.md` and
`docs/124_ai-company-specialist-cell-retry-implementation-decision-handoff.md`. `DEC-182`
implements only one separately approved same-role attempt #2 for one exact failed first attempt:
active retry evidence is saved before one local worker invocation, settlement uses one fresh-state
CAS write, exact replay never reruns the worker, and original batch/attempt records remain
...
Planning-only `DEC-186` defines one schema-v21-preserving response-only
`ReviewerReworkPlanPreview` in `docs/127_ai-company-reviewer-rework-preview-plan.md`, and `DEC-187`
records its complete fielded implementation handoff in
`docs/128_ai-company-reviewer-rework-preview-implementation-decision-handoff.md`. `DEC-188`
implements the exact seven-key runtime/API path and browser-memory inspection action. The
projection accepts only one exact source-current StaffingEntry-bound ExecutionPlan stopped at
Reviewer `changes-requested`, caps the review Artifact before read, preserves source-ordered
duplicate findings, inherits the original target allowlist and verification commands without
widening, computes one canonical source progress digest, caps a later Builder attempt at one, and
...
implementation handoff in
`docs/130_ai-company-durable-reviewer-rework-plan-implementation-decision-handoff.md`. `DEC-191`
implements one schema-v22 immutable `ReworkPlan(status=review-required)` after recomputing the exact
DEC-188 preview and validating a separate record approval. Exact replay is no-write, exact-id and
current-chain GETs expose only the retained evidence, and the generic snapshot excludes the record
map. Rework decisions, Builder append, retry, preflight, approval, mutation, Reviewer/QA execution,
scheduling, and provider authority remain blocked.

Planning-only `DEC-192` defines the Stage 5C append-only ReworkPlanAcceptance boundary in
`docs/131_ai-company-rework-plan-acceptance-plan.md`, and `DEC-193` records its complete fielded
implementation handoff in
`docs/132_ai-company-rework-plan-acceptance-implementation-decision-handoff.md`. `DEC-194`
implements one schema-v23 immutable accepted fact bound to the exact DEC-191 record and recomputed
DEC-188 source evidence. Exact replay is no-write even after source drift, exact GET inspection is
ReworkPlan-bound, the generic snapshot excludes the acceptance map, and the UI exposes no Builder
...
implementation handoff in
`docs/134_ai-company-builder-rework-preflight-implementation-decision-handoff.md`. `DEC-197`
implements the schema-v24 immutable dispatch, existing-Builder durable attempt #3, active-before-
worker persistence, one local-stub no-write preflight, and exact inspection while keeping the fixed
...
implementation handoff in
`docs/140_ai-company-reviewer-reexecution-implementation-decision-handoff.md`. `DEC-206`
implements that exact local-stub path while keeping schema v24 and the fixed three WorkOrders. It
binds the current DEC-203 mutation bundle to one existing-Reviewer WorkOrderAttempt #2, separates
historical mutation evidence from the later current graph, saves the active attempt and Reviewer Run
before worker invocation, and runs Reviewer only. Raw Artifact bytes are rebound on reload and
settlement, while an exact terminal replay remains inspectable after unrelated later source drift.
A pass stops at `QA_READY` with an empty durable action set, and both the generic operator step and
recovery projection keep QA disabled until a separate decision; changes-requested or failure
remains blocked at Reviewer. QA
...
implementation handoff in
`docs/142_ai-company-rework-qa-execution-implementation-decision-handoff.md`. Exact `DEC-209`
implements that bounded path while keeping schema v24 and the fixed graph. It reuses the existing QA
WorkOrder and appends exactly one new WorkOrderAttempt #1,
...
`DELIVERY_READY` without composing a DeliveryPackage. Exact replay and reload validate the
request/input digests, attempt/Run/Artifact refs, and checkpoint lineage. QA attempt #2, retry,
recovery, DeliveryPackage, Mission/task close-out, provider/source mutation, Git/release, memory,
scheduling, policy bypass, and connectors remain blocked.

Planning-only `DEC-210` defines the Stage 5I response-only rework DeliveryPackage boundary in
`docs/143_ai-company-rework-delivery-package-preview-plan.md`, and `DEC-211` records its complete
fielded implementation handoff in
...
Planning-only `DEC-216` defines the Stage 5K rework DeliveryPackage acceptance boundary in
...
Planning-only `DEC-225` defines Stage 7A as one schema-v29 immutable
...
Mission, and exposes exact Mission-bound inspection only. `DEC-227` implements that bounded
...
an allowlisted position projection without context receipts. The explicit runtime/API/UI path stops
at human alignment, hydrates retained receipts only through exact StaffingEntry inspection, and
keeps generic snapshots redacted; scheduler, WorkOrder, provider, prompt, policy, source, Git, release, retry,
...
LLM-native visual-system convergence is accepted by `DEC-159` and implemented from
`docs/110_llm-native-visual-system-convergence-plan.md`. The current shell now uses one semantic
workbench token set, a high-contrast new-Mission command, indicator/label/count navigation,
connected source-backed turns, one bounded desktop context inspector, and a contained Advanced Ops
layout. At 390px the existing project/provider/gate/refresh metadata occupies one actual 44px row.
Every existing route, action, count, evidence disclosure, runtime record, review gate, approval gate,
provider contract, and state boundary remains unchanged.

The first-viewport corrective redesign is accepted by `DEC-160` and implemented from
`docs/111_llm-native-first-viewport-corrective-redesign-plan.md`. Operator review found the prior
token and spacing convergence visually insufficient, so the shell now uses a charcoal command rail
and the first-run Mission view uses a context-plus-command workspace. `project_path` is the dominant
...
LLM-native mobile Mission title readability is accepted by `DEC-152` and implemented from
`docs/103_llm-native-mobile-mission-title-readability-plan.md`. The collapsed three-row mobile rail
...
LLM-native Mission next-gate navigation is accepted by `DEC-156` and implemented from
`docs/107_llm-native-mission-next-gate-navigation-plan.md`. When an active Mission Thread has a
source-backed non-Mission next action, its lead shows the current gate and one native link to the
existing lower gate after the recorded evidence. Within this new lead-to-gate navigation pair, only
the lower gate keeps an `open-surface-for-mission` action; Graph, the composer, recorded turns, routes, runtime/API/schema/
dependencies, persistence, provider behavior, and every authority boundary remain unchanged.

LLM-native primary workstream language is accepted by `DEC-157` and implemented from
`docs/108_llm-native-primary-workstream-language-plan.md`. Mission, Council, Execution, and
Deliverables now use natural Korean operator language for system-authored primary status and section
labels while retaining `Mission`, `Council`, `Execution`, `Deliverables`, `Thread`, and `Graph` as
product object names. Exact identifiers and internal state remain in the existing collapsed evidence
details; source/provider/user-authored content, action contracts, readiness, render order, routes,
runtime/API/schema/dependencies, persistence, provider behavior, and every authority boundary remain
unchanged.
...
historical unbound sessions remain compatible. DEC-170 and DEC-171 define one explicit local
dependency-ready role step with durable attempt evidence, and DEC-172 implements that bounded
schema-v19 path. Provider-backed WorkOrders, retry/rework/recovery, parallel or autonomous
scheduling, memory application, source/profile mutation, approval bypass, and runtime-agent
commit/push/release remain blocked.
...
- Completion gate inventory: `docs/22_completion-gate-inventory.md` and
  ...
  registered aggregate `265` checks, UI QA `69` checks, zero-open backlog, post-completion router,
  README smoke count,
...
- Reviewed Mission context attachment implementation: `DEC-225`, `DEC-226`, `DEC-227`,
  `docs/153_ai-company-reviewed-mission-context-attachment-plan.md`,
  `docs/154_ai-company-reviewed-mission-context-attachment-implementation-decision-handoff.md`, and
  `scripts/smoke-ai-company-reviewed-mission-context-attachment-planning.mjs`,
  `scripts/smoke-ai-company-reviewed-mission-context-attachment.mjs`, and
  `scripts/smoke-ui-slice-716.mjs` prove one
...
- Strategist Mission context consumption: `DEC-228`, `DEC-229`, `DEC-230`,
  `docs/155_ai-company-strategist-mission-context-consumption-plan.md`,
  `docs/156_ai-company-strategist-mission-context-consumption-implementation-decision-handoff.md`,
  `scripts/smoke-ai-company-strategist-mission-context-consumption-planning.mjs`,
  `scripts/smoke-ai-company-strategist-mission-context-consumption.mjs`, and
  `scripts/smoke-ui-slice-717.mjs` prove the schema-v30 first-attempt accepted-StaffingPlan Council
  path, Strategist-only frozen context, allowlisted Conductor projection, exact no-write replay,
  exact-inspection hydration, generic snapshot redaction, and downstream scheduler blocking.
...
```bash
find scripts -maxdepth 1 -type f -name 'smoke-*.mjs' | wc -l      # 1014 smoke files
find scripts -maxdepth 1 -type f -name '*qa-slice*.mjs' | wc -l   # 10 QA slice files
find scripts -maxdepth 1 -type f -name 'smoke-ui-slice-*.mjs' | wc -l # 717 UI smoke files
```
...
- `node scripts/smoke-completion-gate-inventory-current-evidence.mjs`: completion inventory counts,
  aggregate registration `265`, UI QA registration `69`, zero-open backlog, post-completion router,
...
- `node scripts/ui_qa_status.mjs`: required `95/95` pass. Snapshot reachability remains informational
  and was skipped because the optional port-4315 UI server was not running.
- `node scripts/verification_status.mjs`: required `1/1` and informational `315/315` pass; total
  `316/316` pass. The aggregate includes the
  ...
  contract, the source-density-derived sparse Mission Graph presentation contract, and the complete
  naturally wrapped mobile current-Mission title contract.
...
- The current implemented browser/runtime completion gate is evidence-closed through DEC-161. The
  multi-agent completion implementation slices are current through DEC-230, including Stage 5K
  ...
  define the schema-v30 Strategist-only consumption plan and complete handoff; DEC-230 implements
  only the explicit Strategist first-attempt path while wider role consumption remains blocked. DEC-173
...
- `DEC-186` and `DEC-187` are consumed planning and handoff evidence for the exact response-only
  ReviewerReworkPlanPreview implemented by `DEC-188`. The runtime module, exact seven-key GET, and
  browser-memory inspection action remain no-write; that preview path does not itself migrate state,
  append Builder work, retry, preflight, approve, mutate source, execute Reviewer/QA, schedule,
  invoke a provider, or select work automatically.
- `DEC-189` and `DEC-190` are consumed planning and fielded handoff evidence for the schema-v22
  immutable `ReworkPlan(status=review-required)` implemented by `DEC-191`. One exact record, record
  route, current-chain hydration, and read-only UI evidence exist; ReworkPlan decisions, Builder
  append, retry, preflight, approval, source mutation, role execution, scheduling, provider, memory,
  Git/release, policy, collection, bypass, and connector authority remain unimplemented.
- `DEC-192` and `DEC-193` are consumed planning and fielded handoff evidence for the schema-v23
  append-only ReworkPlanAcceptance implemented by `DEC-194`. One exact accepted record, no-write
  replay, ReworkPlan-bound GET, snapshot exclusion, and rationale-gated UI action exist; ReworkPlan
...
- `DEC-204` through `DEC-206` are consumed planning, handoff, and implementation evidence for one
  existing-Reviewer WorkOrderAttempt #2 over exact DEC-203 mutation evidence. A pass stops at the
  actionless `QA_READY` checkpoint; generic QA and recovery remain suppressed there.
- `DEC-207` through `DEC-209` are consumed planning, handoff, and implementation evidence for one
...
- `DEC-159` changes browser presentation only. It adds no route, action, state write, runtime/API/
  schema/dependency behavior, provider call, automatic navigation, source mutation, scheduling,
...
- `DEC-151` permits only source-density-derived Mission Graph presentation. It retains all six
  ...
  mutation, Git/release authority, scheduling, policy bypass, and connectors remain outside the
  implemented scope.
- `DEC-152` permits only responsive current-Mission title presentation. It keeps the existing
  three-row mobile rail, full-width Mission history and Advanced Ops disclosures, exact source list,
  selection route, pending-gate projection, and desktop navigation. Rename, aliases, search,
...
- `DEC-136` adds measurement-only context telemetry. It reports no raw values and performs no payload
  ...
  or token/cost claim.
- `DEC-162` reconciles the pre-implementation schema-v16 AI Company evidence, `DEC-163` approves completion planning,
  ...
  records its complete fielded implementation handoff; `DEC-197` implements only that bounded
  local-stub sidecar path. Dynamic or additional
  cells, bound
