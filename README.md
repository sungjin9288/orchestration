...
and browser memory. Schema v14, durable records, exact inspection, and snapshots remain unchanged.
...
- `DEC-122` permits planning for one exact-id operator-selected response-only MemoryRecall preview,
  ...
  Current schema v14, stored MemoryItems, exact GET, and durable snapshots remain unchanged; only one
