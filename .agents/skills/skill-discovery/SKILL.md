...
- Skill discovery can be healthy even when OpenSpace execution is blocked by host LLM credentials.
- If delegation through `execute_task` returns `blocked_missing_host_llm_credentials`, do not treat it as a missing skill or repo wiring failure.
- In that case, use any discovered local skill instructions yourself when they are applicable, or continue with repo-native tools.
- Report the credential boundary explicitly instead of implying that OpenSpace delegation completed.
