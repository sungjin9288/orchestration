...
- optional browser smoke가 runtime mutation과 같은 state file을 빠르게 polling하는 구조라면, `state.json`을 직접 덮어쓰는 저장 방식은 부분 기록 JSON을 그대로 노출해 `Unexpected end of JSON input` 같은 race를 만든다. 이런 shell은 `src/runtime/file-store.js`에서 temp file write 후 `renameSync`로 state/artifact를 atomic하게 교체하는 편이 가장 작은 blast radius로 브라우저 polling과 runtime save를 함께 안정화한다.
- optional browser smoke가 green이어야 하는 핵심이 shell entry, button visibility, downstream state reflection이라면, 실제 mutation trigger까지 계속 browser click에 묶을 이유는 적다. `run-planner`, `run-builder-preflight`, `run-reviewer` 같은 단계는 기존 API/runtime assertion으로 실행을 닫고, 브라우저는 current shell copy, surface landing, selection visibility만 확인하는 편이 drift와 click flake를 줄이면서도 smoke value를 유지하기 쉽다.
- top-level status entrypoint가 다른 status script를 다시 감쌀 때는 exit code만 보면 실제 lane 상태를 잃기 쉽다. `ui_qa_status`처럼 JSON report를 내는 스크립트는 상위 `verification_status`가 `allChecksOk` 같은 semantic field를 직접 읽어야 informational lane이 진짜 빨간불과 단순 성공 종료를 구분할 수 있다.
- 긴 optional browser matrix를 blocking lane에 섞지 않더라도, top-level verification report 안에 `on-demand` bundle로 같이 드러내 두는 편이 운영자가 다음 검증 명령을 다시 찾기 쉽다. fast status와 long browser coverage를 분리하면서도 한 화면에서 이어 보이게 하는 것이 repo-native evidence refresh 흐름에 더 잘 맞는다.
- `on-demand` bundle을 top-level report에 노출만 해 두면 여전히 운영자가 별도 명령을 다시 입력해야 해서 evidence refresh 흐름이 한 번 끊긴다. `verification_status --include-on-demand`처럼 opt-in execution flag를 같은 entrypoint에 붙여 두면 기본 fast lane은 유지하면서도 필요할 때 전체 matrix를 한 명령으로 확장하기 쉽다.
- opt-in browser lane도 host-side localhost bind나 browser permission이 없는 sandbox에서는 실제 회귀와 무관하게 false red가 날 수 있다. 이런 경우에는 `listen EPERM 127.0.0.1` 같은 host restriction 패턴만 `skipped`로 승격하고, 실제 assertion failure는 그대로 `fail`로 남기는 편이 on-demand status 해석을 더 정확하게 만든다.
- host restriction branch를 “이번 sandbox에서 우연히 재현되길” 기다리면 smoke가 다시 환경 의존적으로 흔들린다. `verification_status` 같은 top-level script는 check-array override와 tiny fixture script를 허용해 두고, `skip`과 real `fail`을 deterministic smoke로 같이 고정하는 편이 status semantics를 오래 유지하기 쉽다.
- status entrypoint가 둘 이상으로 늘어나면 각 스크립트가 개별적으로 green이어도 운영자가 어떤 조합을 다시 돌려야 하는지 잊기 쉽다. `ui_qa_status`, `verification_status`, 그리고 deterministic status smoke까지 한 번에 묶는 repo-native aggregate를 두는 편이 evidence refresh와 handoff 품질을 더 안정적으로 만든다.
- aggregate status bundle도 기본 fast path와 opt-in long path를 같은 인터페이스로 제공하는 편이 운영자가 덜 헷갈린다. 상위 bundle이 `--include-on-demand`를 그대로 아래 `verification_status`로 전달해 주면, 기본 refresh와 browser-inclusive refresh를 명령 이름 하나로 확장할 수 있어 운영성이 좋아진다.
