...
async function submitBuilderReworkPreflight(actionButton) {
  ...
  try {
    ...
    state.builderReworkMutationApproval =
      payload.workOrderAttempt.status === 'waiting-gate'
        ? await fetchOptionalJson(
            `/api/rework-plans/${encodeURIComponent(record.id)}/builder-rework-mutation-approval`,
          )
        : null;
    elements.refreshStatus.textContent =
...
async function requestBuilderReworkMutationApproval(actionButton) {
  const record = state.reviewerReworkPlan;
  const envelope = state.builderReworkMutationApproval;
  const form = actionButton?.closest?.(
    '[data-form="request-builder-rework-mutation-approval"]',
  );
  if (!record || !envelope || !form) {
    throw new Error(
      'source-current Builder rework mutation approval evidence가 필요합니다.',
    );
  }
  const request = getBuilderReworkMutationApprovalRequest(envelope, {
    rationale: String(
      new FormData(form).get('builderReworkMutationRationale') || '',
    ),
    reviewedAt: new Date().toISOString(),
  });
  if (!request) {
    throw new Error('Builder rework mutation approval rationale이 필요합니다.');
  }

  state.mutating = true;
  elements.refreshStatus.textContent =
    `${record.id} mutation approval을 요청하는 중…`;
  render();
  try {
    const payload = await postJson(
      `/api/rework-plans/${encodeURIComponent(record.id)}/builder-rework-mutation-approval`,
    ...
    if (
      payload.approval?.allowedNextAction !==
        'builder-rework-live-mutation' ||
      payload.approval?.scope !== 'builder-rework' ||
      payload.decisionInboxItem?.sourceId !== payload.approval.id
    ) {
      throw new Error(
        'Builder rework mutation approval 응답이 exact evidence와 다릅니다.',
      );
    }
  ...
  try {
    ...
    state.reviewerReexecution =
      mutation.status === 'completed'
        ? (
            await fetchOptionalJson(
              `/api/rework-plans/${encodeURIComponent(reworkPlan.id)}/reviewer-reexecution`,
            )
          )?.reviewerReexecution || null
        : null;
...
async function runReviewerReexecution(actionButton) {
  const reworkPlan = state.reviewerReworkPlan;
  const envelope = state.reviewerReexecution;
  const form = actionButton?.closest?.(
    '[data-form="run-reviewer-reexecution"]',
  );
  if (!reworkPlan || !envelope || !form) {
    throw new Error('source-current Reviewer 재실행 evidence가 필요합니다.');
  }
  const formData = new FormData(form);
  const request = getReviewerReexecutionRequest(envelope, {
    acknowledgement: String(
      formData.get('reviewerReexecutionAcknowledgement') || '',
    ),
    rationale: String(formData.get('reviewerReexecutionRationale') || ''),
    reviewedAt: new Date().toISOString(),
  });
  if (!request) {
    throw new Error('exact acknowledgement와 Reviewer 재실행 rationale이 필요합니다.');
  }
  state.mutating = true;
  state.reviewerReexecution = null;
  elements.refreshStatus.textContent =
    `${reworkPlan.id} Reviewer attempt #2를 실행하는 중…`;
  render();
  try {
    const payload = await postJson(
      `/api/rework-plans/${encodeURIComponent(reworkPlan.id)}/reviewer-reexecution`,
    ...
    const result = payload.reviewerReexecution;
    if (
      result?.reworkPlanId !== reworkPlan.id ||
      !['running', 'completed', 'changes-requested', 'failed', 'interrupted'].includes(
        result.status,
      ) ||
      result.blockedActions?.includes('qa-execution') !== true
    ) {
...
function renderReviewerReexecution(reworkPlanId) {
  const result = state.reviewerReexecution;
  if (!result || result.reworkPlanId !== reworkPlanId) return '';
  const statusTone =
    result.status === 'completed'
      ? 'success'
      : ['changes-requested', 'failed'].includes(result.status)
        ? 'danger'
        : 'warning';
  const requestSource =
    result.readiness?.requestSource || result.requestSource || {};
  const findings = result.retainedFindings || result.findings || [];
  const findingRows = findings
    .map((finding) => {
      const label =
        typeof finding === 'string'
          ? finding
          : finding.text || finding.summary || finding.findingId || '';
      return `<li>${escapeHtml(label)}</li>`;
    })
    .join('');
  return `
    <section
      class="reviewer-reexecution"
      aria-label="Reviewer re-execution"
      data-reviewer-reexecution-status="${escapeHtml(result.status)}"
    >
      <div class="card-title-row card-title-row-tight">
        <strong>Reviewer attempt #2</strong>
      ...
      <dl class="reviewer-rework-evidence">
        <div>
          <dt>Mutation run</dt>
          <dd>${escapeHtml(requestSource.mutationRunId || result.mutationRun?.id || '')}</dd>
        </div>
        <div>
          <dt>Reviewer WorkOrder</dt>
          <dd>${escapeHtml(requestSource.reviewerWorkOrderId || result.reviewerWorkOrder?.id || '')}</dd>
        </div>
        <div>
          <dt>Mutation evidence</dt>
          <dd>${escapeHtml(requestSource.mutationEvidenceDigest || result.mutationEvidenceDigest || '')}</dd>
        </div>
        <div>
          <dt>Changed files</dt>
          <dd>${escapeHtml((result.changedFiles || []).join(', ') || 'none')}</dd>
        </div>
      </dl>
      ${
        findingRows
          ? `
            <div class="reviewer-reexecution-findings">
              <strong>Retained findings</strong>
              <ul>${findingRows}</ul>
            </div>
          `
          : ''
        ...
        result.status === 'ready'
          ? `
            ...
            >
              <label class="field">
                ...
                <input
                  name="reviewerReexecutionRationale"
                  type="text"
                  maxlength="500"
                  required
                  placeholder="Why the exact reworked source should be reviewed once"
                  ${state.loading || state.mutating ? 'disabled' : ''}
                />
              </label>
              <label class="builder-rework-acknowledgement">
                <input
                  name="reviewerReexecutionAcknowledgement"
                  type="checkbox"
                  value="review-exact-rework-result-once-and-stop-before-qa"
                  required
                  ${state.loading || state.mutating ? 'disabled' : ''}
                />
                <span>현재 rework 결과를 한 번 검토하고 QA 실행 전 중단</span>
              </label>
              <button
                class="primary-button"
                type="button"
                data-action="run-reviewer-reexecution"
          ...
          : `
            <dl class="reviewer-rework-evidence">
              <div>
                <dt>Attempt</dt>
                <dd>${escapeHtml(result.reviewerWorkOrderAttempt?.id || result.workOrderAttempt?.id || 'not recorded')}</dd>
              </div>
              <div>
                <dt>Run</dt>
                <dd>${escapeHtml(result.reviewerRun?.id || 'not recorded')}</dd>
              </div>
              <div>
                <dt>Review Artifact</dt>
                <dd>${escapeHtml(result.reviewArtifact?.id || 'none')}</dd>
              </div>
              <div>
                <dt>Next gate</dt>
                <dd>${escapeHtml(result.nextGate || 'blocked')}</dd>
              </div>
            </dl>
            <p class="detail-copy detail-copy-compact">
              QA는 실행되지 않았습니다. retry, recovery, commit, push, release는 계속 차단됩니다.
            </p>
          `
      }
    </section>
        ...
        mutation.status === 'ready'
          ...
          : `
            ...
            <p class="detail-copy detail-copy-compact">
              ${
                mutation.status === 'completed'
                  ? 'Reviewer는 별도 operator action으로 한 번만 실행할 수 있습니다. QA는 계속 차단됩니다.'
                  : 'Reviewer와 QA는 실행되지 않았습니다.'
              }
              retry, recovery, commit, push, release는 계속 차단됩니다.
            </p>
            ${
              mutation.status === 'completed'
...
function renderBuilderReworkMutationApproval(reworkPlanId) {
  ...
  const inboxItem = envelope.decisionInboxItem;
  const reviewerDecisionRefs =
    envelope.readiness?.reviewDecisionInboxItemRefs || [];
  const reviewerPriority = reviewerDecisionRefs.length
    ? `
      <div class="builder-rework-mutation-priority">
        <strong>Reviewer Decision 우선</strong>
        <span>${escapeHtml(reviewerDecisionRefs.join(', '))}</span>
        <p>이 decision은 계속 blocking 상태이며 mutation approval이 이를 해제하지 않습니다.</p>
      </div>
    `
    : `
      <p class="detail-copy detail-copy-compact">
        연결된 Reviewer Decision ref는 없습니다. 빈 ref 상태를 그대로 보존합니다.
      </p>
    `;
  const evidence = envelope.readiness?.requestSource || {};

  return `
    <div
      class="builder-rework-mutation-approval"
      aria-label="Builder rework mutation approval"
    >
      <div class="card-title-row card-title-row-tight">
        <strong>Builder rework mutation approval</strong>
        ${createToken(
          approval?.status || envelope.readiness?.status || 'request-ready',
      ...
      <dl class="reviewer-rework-evidence">
        <div>
          <dt>dispatch</dt>
          <dd>${escapeHtml(evidence.builderReworkDispatchId || '')}</dd>
        </div>
        <div>
          <dt>attempt</dt>
          <dd>${escapeHtml(evidence.workOrderAttemptId || '')}</dd>
        </div>
        <div>
          <dt>preflight run</dt>
          <dd>${escapeHtml(evidence.preflightRunId || '')}</dd>
        </div>
        <div>
          <dt>preflight artifact</dt>
          <dd>${escapeHtml(evidence.preflightArtifactId || '')}</dd>
        </div>
      </dl>
      ${
        !approval
          ? `
            ...
            >
              <label class="field">
                <span>Mutation approval rationale</span>
                <input
                  name="builderReworkMutationRationale"
                  type="text"
                  maxlength="500"
                  required
                  placeholder="Why this exact source-bound mutation gate should be reviewed"
                  ${state.loading || state.mutating ? 'disabled' : ''}
                />
              </label>
              <button
                class="primary-button"
                type="button"
                data-action="request-builder-rework-mutation-approval"
                data-id="${escapeHtml(reworkPlanId)}"
                ${state.loading || state.mutating ? 'disabled' : ''}
              >
                Mutation approval 요청
              </button>
            </form>
          `
          : `
            <div class="builder-rework-mutation-record">
              <strong>${escapeHtml(approval.id)}</strong>
              <span>${escapeHtml(approval.prompt)}</span>
              <code>${escapeHtml(approval.metadata.bindingDigest)}</code>
            </div>
            ${
              approval.status === 'pending' && inboxItem?.status === 'pending'
                ? `
                  <div class="form-actions form-actions-inline">
                    <button
                      class="primary-button"
                      type="button"
                      data-action="run-inbox-action"
                      data-id="${escapeHtml(inboxItem.id)}"
                      data-verb="approve"
                      ${state.loading || state.mutating ? 'disabled' : ''}
                    >
                      승인
  ...
  if (actionButton) {
    try {
      ...
      if (
        ...
        'request-builder-rework-mutation-approval'
      ) {
        await requestBuilderReworkMutationApproval(actionButton);
        return;
      }
