...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-363');
const port = 4664;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-363 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-363-'));
  const inputPath = path.join(tempDir, 'latest-result-token-density.txt');
  const outputPath = path.join(tempDir, 'latest-result-token-density.md');

  fs.writeFileSync(inputPath, 'Latest result token density smoke\n', 'utf8');
