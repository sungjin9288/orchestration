...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-382');
const port = 4683;
...
assert.match(harnessExecutionTokens, /export function getHarnessOutputChannelToken\(usesOutputFile\) \{/);
assert.match(
  harnessExecutionTokens,
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-382 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-382-'));
  const inputPath = path.join(tempDir, 'visible-output-file-token-wording.txt');
  const outputPath = path.join(tempDir, 'visible-output-file-token-wording.md');

  fs.writeFileSync(inputPath, 'Execution visible output file token wording smoke\n', 'utf8');
  ...
  try {
    ...
    assert.match(runPayload.harnessExecution?.outputPreview || '', /Execution visible output file token wording smoke/i);

    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionVisibleOutputFileTokenWording: {
            insertionPoint: 'executionResultRegister->visibleOutputFileTokenWording->tokenLabel',
            tokenLabel: '출력 파일',
