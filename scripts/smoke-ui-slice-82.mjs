...
assert.match(appJs, /officeLine: '최종 방향 결정과 인계 승인석'/);
assert.match(appJs, /officeLine: '목표와 우선순위를 다듬는 검토석'/);
assert.match(appJs, /officeLine: '구조 리스크와 경계를 봉쇄하는 자리'/);
assert.match(appJs, /officeLine: '첫 실행 셀과 체크포인트를 편성하는 자리'/);
assert.match(appJs, /안건을 올리면 귀여운 참모진이 각자 자리에서 바로 읽고/);
assert.match(appJs, /각 직급 AI가 자기 자리에서 같은 안건을 읽고/);
