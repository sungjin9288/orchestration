...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-status',
});
...
const REVIEW_CRITERIA = [
  {
    ...
    question:
      'Is the dry-run review acceptance finalization review acceptance finalization status green and ready for review?',
    requiredEvidenceRef: 'finalizationEnvelope.compatibility.finalizationState',
  },
  {
    id: 'finalization-findings-passed',
    question:
      'Did every dry-run review acceptance finalization review acceptance finalization finding pass without evidence gaps?',
...
function summarizeSources(sources) {
  ...
  return {
    ...
    lessonCaptured:
      /proposal record dry-run review acceptance finalization review acceptance finalization review.*proposal approval/i.test(
        lessons,
      ) ||
      /proposal approval.*proposal record dry-run review acceptance finalization review acceptance finalization review/i.test(
        lessons,
      ),
  ...
  return [
    ...
    },
    {
      ...
      ok:
        ...
        safetyBoundary.doesNotPromoteFinalizationReviewAcceptanceFinalizationToRecord === true,
      evidenceRef: 'readiness and safetyBoundary',
      observed: {
        ...
        doesNotPromoteFinalizationReviewAcceptanceFinalizationToRecord:
          safetyBoundary.doesNotPromoteFinalizationReviewAcceptanceFinalizationToRecord,
      },
    },
  ];
}
...
function buildReviewEnvelope({ finalizationPayload }) {
  ...
  return {
    ...
    reviewEnvelope: {
      reviewId:
        'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-candidate',
      ...
      reviewPurpose:
        'Review finalized dry-run finalization review acceptance evidence only for the next read-only acceptance check while keeping proposal approval, record creation, persistence, queue mutation, and implementation authority blocked.',
      reviewInputs: {
        finalizationState:
      ...
      blockedActions,
      acceptanceQuestion:
        'Can this reviewed dry-run finalization review acceptance finalization evidence move to a read-only acceptance check without approving, creating, persisting, or mutating proposal records?',
      nonApprovalStatement:
        'Proposal record dry-run review acceptance finalization review acceptance finalization review is review for a later read-only acceptance check only; it is not proposal approval, proposal record creation, queue persistence, implementation authorization, or source mutation authority.',
    },
    compatibility: {
...
const payload = {
  ...
  sourceSummary,
  inputStatuses: {
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalization: {
  ...
  nextRecommendedSlice: {
    ...
    reason:
      'The dry-run review acceptance finalization review acceptance finalization evidence is reviewed only for a read-only acceptance check; the next safe slice can accept review evidence without approving, creating, persisting, implementing, or mutating proposal records.',
  ...
  failures: {
    missingSources,
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalizationFailed:
