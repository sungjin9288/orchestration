...
const sentinelSecret = 'provider-slice-12-secret-sentinel';
...
const TARGET_FILE = 'docs/research-brief.md';
...
function createApiPayload(outputText, options = {}) {
  return {
    payload: {
      id: options.providerRunId || 'resp-provider-slice-12',
...
## Slice Goal
Draft one bounded research brief inside the knowledge-work pack.

## Intended Deliverable
- research brief
- target file: docs/research-brief.md
      ...
      artifact: {
        ...
        approvedAssumptions:
          options.artifact?.approvedAssumptions || [
            'The deliverable should stay as one bounded research brief draft.',
          ],
        noArchitectureChangeStatement:
          options.artifact?.noArchitectureChangeStatement ||
          'No architecture change is approved by this note. Downstream work stays inside the current research-brief drafting boundary.',
      ...
      artifact: {
        orderedSubTasks:
          options.artifact?.orderedSubTasks || [
            'Confirm the research-brief boundary and required sections.',
            'Prepare the bounded research brief draft.',
            'Review the brief for explicit next action and provenance trace.',
          ],
        checkpoints:
          options.artifact?.checkpoints || [
            'checkpoint 1: confirm research-brief scope',
            'checkpoint 2: draft findings and implications',
        ...
        verificationCheckpoints:
          options.artifact?.verificationCheckpoints || [
            'Verify findings, next action, and provenance trace.',
          ],
        reviewTriggerPoints:
          options.artifact?.reviewTriggerPoints || ['Trigger review after the bounded research brief draft exists.'],
        ...
        executionBoundarySummary:
          options.artifact?.executionBoundarySummary || [
            'Builder handoff stays bounded to docs/research-brief.md only.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
      ...
      artifact: {
        ...
        intendedChanges:
          options.artifact?.intendedChanges || [
            'Create one bounded research brief file with the required section structure.',
          ],
        risks:
          options.artifact?.risks || [
            'Fail closed if the research brief loses explicit findings or provenance trace.',
          ],
        verificationPlan:
          options.artifact?.verificationPlan || [
            'Run reviewer against the generated research brief and verify the rubric outcome.',
          ],
        reviewEvidenceExpectations:
          options.artifact?.reviewEvidenceExpectations || [
            'The review should confirm findings, next action, and trace marker.',
          ],
        escalationTriggers:
          options.artifact?.escalationTriggers || [
            'Escalate if the deliverable widens beyond docs/research-brief.md.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
      ...
      artifact: {
        changeSummary:
          options.artifact?.changeSummary || [
            'Prepare one bounded research brief draft inside the approved knowledge-work target file.',
          ],
        targetFiles: options.artifact?.targetFiles || [TARGET_FILE],
        ...
        risks:
          options.artifact?.risks || [
            'Fail closed if the research brief update leaves the approved target or loses provenance.',
          ],
        verificationNotes:
          options.artifact?.verificationNotes || [
            'Prepare the research brief draft for knowledge-work reviewer handoff only.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
        ...
        summary:
          options.normalizedResult?.summary ||
          'Builder live mutation prepared a bounded research brief draft for reviewer handoff.',
        decisionTitle: options.normalizedResult?.decisionTitle || '',
        decisionPrompt: options.normalizedResult?.decisionPrompt || '',
      },
    }),
...
function buildMutationMarker(approvalId, relativePath) {
  return `provider-slice-12 ${approvalId} ${relativePath}`;
}

function createResearchBriefContent(taskTitle, approvalId, { includeFindings = true } = {}) {
  const marker = buildMutationMarker(approvalId, TARGET_FILE);
  const findingsSection = includeFindings
    ? `## Findings
- The current context supports a bounded first conclusion
- The remaining uncertainty is explicit enough for operator review
...
## Research Question
What is the smallest evidence-backed next move inside the current project boundary?

## Current Context
- Scope is limited to the current repo state and approved mission intent
- The brief should stay reviewable as one bounded deliverable

${findingsSection}## Implications
- The operator can decide on the next move without opening implementation scope
- Additional research should only follow after the current brief is reviewed

## Unknowns
- Whether more evidence is required before downstream execution
- Which follow-up thread should own any deeper investigation

## Next Action
- Review the findings and confirm whether the brief is sufficient for the next decision
...
function createRuntimeForLabel(label) {
  const runtimeRoot = path.join(repoRoot, 'var', `runtime-provider-slice-12-${label}`);
  const projectRoot = path.join(repoRoot, 'var', `provider-slice-12-${label}-project`);
...
async function prepareKnowledgeWorkLiveContext(label, buildContent) {
  ...
  const project = runtime.createProject({
    name: `provider-slice-12 ${label}`,
    pack: 'knowledge-work',
    projectPath: projectRoot,
    provider: {
      adapter: 'openai-responses',
      mode: 'live',
      model: 'provider-slice-12-operator-model',
      env: {
  ...
  const task = runtime.createTask({
    ...
    title: `provider-slice-12 ${label}`,
    intent: 'Draft a bounded research brief and keep reviewer criteria explicit.',
  ...
  const plannerResult = await coordinator.runPlanner({
    ...
    routingOutcome: createRoutingOutcome(
      'Draft one bounded research brief and keep the review rubric explicit.',
    ),
  });
  ...
  const briefContent = buildContent(task.title, approvalRecord.id);
...
const happyContext = await prepareKnowledgeWorkLiveContext('happy', (taskTitle, approvalId) =>
  createResearchBriefContent(taskTitle, approvalId),
);
...
assert.match(String(happyBuilderRequestBody.input || ''), /docs\/research-brief\.md/);
...
happyContext.queuedFetch.push(
  createReviewerApiPayload(happyReviewerAnchor, {
    artifact: {
      ...
      verificationEvidence: ['changed files: docs/research-brief.md'],
      knowledgeWorkRubric: {
        deliverableType: 'research-brief',
    ...
    normalizedResult: {
      ...
      summary: 'Reviewer passed the bounded research brief and leaves commit-package as an explicit downstream step.',
      decisionTitle: '',
      decisionPrompt: '',
    },
    providerRunId: 'resp-reviewer-knowledge-work-research-brief-pass',
  }),
);
...
assert.match(String(happyReviewerRequestBody.input || ''), /^## Research Question$/m);
assert.match(happyReviewArtifact.content, /deliverable file reviewed: docs\/research-brief\.md/);
...
const changesContext = await prepareKnowledgeWorkLiveContext(
  'changes-requested',
  (taskTitle, approvalId) => createResearchBriefContent(taskTitle, approvalId, { includeFindings: false }),
);
...
changesContext.queuedFetch.push(
  createReviewerApiPayload(changesReviewerAnchor, {
    artifact: {
      ...
      findings: ['Missing required section: Findings.'],
      ...
      knowledgeWorkRubric: {
        ...
        deliverablePresent: true,
        missingRequiredSections: ['Findings'],
        emptyRequiredSections: [],
        explicitNextAction: true,
        traceMarkerVerified: true,
        checklistItemsPresent: true,
    ...
    normalizedResult: {
      ...
      needsDecision: false,
      nextStage: 'builder',
      summary: 'Return to builder to complete the missing findings section.',
      decisionTitle: '',
      decisionPrompt: '',
    },
    providerRunId: 'resp-reviewer-knowledge-work-research-brief-changes',
  }),
);
...
assert.match(changesArtifact.content, /Missing required section: Findings\./);
assert.match(changesArtifact.content, /deliverable file reviewed: docs\/research-brief\.md/);
  ...
  (reviewerAnchor) =>
    createReviewerApiPayload(reviewerAnchor, {
      ...
      normalizedResult: {
        ...
        needsDecision: false,
        nextStage: 'human gate',
        summary: 'Incorrectly attempt to pass the research brief despite a missing required section.',
        decisionTitle: '',
        decisionPrompt: '',
      },
      providerRunId: 'resp-reviewer-knowledge-work-research-brief-invalid-pass',
    }),
