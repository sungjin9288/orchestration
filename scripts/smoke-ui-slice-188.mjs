...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-188');
...
function buildUiData(runtime, coordinator, projectId) {
  ...
  return {
    activeProject: snapshot.projects[projectId] || null,
    approvals,
    artifacts,
...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Taskboard viewport strip aligned to the current evidence rail handoff truth.',
  };
...
function renderTaskboardViewportTokenRow({
  activeProject,
  createToken,
  evidenceRail,
  immediateTone,
  ...
  return `
    <div class="token-row token-row-compact">
      ...
      ${
        activeProject
          ? createToken(`프로젝트:${activeProject.name}`, 'success')
          : createToken('프로젝트:선택 필요', 'warning')
      }
      ${createToken(`실행셀:${taskCount}`, taskCount > 0 ? 'neutral' : 'warning')}
      ${createToken(
        `바로:${pendingApprovals > 0 || pendingInboxItems > 0 ? '결재함' : '작업판 상세'}`,
        immediateTone,
      )}
    </div>
  `;
...
const { createToken, getExecutionEvidenceRail, getGuardReasonDisplay } = loadHelpers();

assert.match(appJs, /const taskboardEvidenceState = getExecutionEvidenceRail\(focusedTask, data\);/);
assert.match(
  appJs,
  /const taskboardViewportStrip = renderViewportHandoffStrip\(\{[\s\S]*createToken\(\s*`현재:\$\{taskboardEvidenceState\.currentOwnerLabel\}`,\s*taskboardEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{taskboardEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*createToken\(`실행셀:\$\{data\.tasks\.length\}`[\s\S]*`바로:\$\{pendingApprovals\.length > 0 \|\| pendingInboxItems\.length > 0 \? '결재함' : '작업판 상세'\}`/s,
);
...
try {
  ...
  const waitingProject = runtime.createProject({
    name: 'taskboard-viewport-handoff-waiting',
    projectPath: repoRoot,
  });
  const waitingData = buildUiData(runtime, coordinator, waitingProject.id);
  const waitingRail = getExecutionEvidenceRail(null, waitingData);
  const waitingMarkup = renderTaskboardViewportTokenRow({
    activeProject: waitingData.activeProject,
    createToken,
    evidenceRail: waitingRail,
    immediateTone: 'warning',
    pendingApprovals: 0,
    pendingInboxItems: 0,
    taskCount: waitingData.tasks.length,
  });

  assert.equal(waitingRail.currentOwnerLabel, '인계 대기');
  assert.equal(waitingRail.nextHandoffLabel, '실행 셀 생성');
  assert.match(waitingMarkup, /현재:인계 대기/);
  assert.match(waitingMarkup, /다음:실행 셀 생성/);
  assert.match(waitingMarkup, /실행셀:0/);

  runtime.resetRuntime();

  const blockedProjectPath = fs.mkdtempSync(
    path.join(os.tmpdir(), 'orchestration-ui-slice-188-provider-'),
  );
  const blockedProject = runtime.createProject({
    name: 'taskboard-viewport-maker-blocked',
    projectPath: blockedProjectPath,
    provider: { mode: 'live' },
  });
  const blockedTask = runtime.createTask({
    projectId: blockedProject.id,
    title: 'Taskboard viewport maker blocked handoff',
    intent: 'Keep the taskboard viewport strip grounded when maker is blocked.',
  });
  ...
  const blockedMarkup = renderTaskboardViewportTokenRow({
    activeProject: blockedData.activeProject,
    createToken,
    evidenceRail: blockedRail,
    immediateTone: 'neutral',
    pendingApprovals: 0,
    pendingInboxItems: 0,
    taskCount: blockedData.tasks.length,
  });
  ...
  assert.match(blockedMarkup, /실행셀:1/);

  runtime.resetRuntime();

  const reviewedProject = runtime.createProject({
    name: 'taskboard-viewport-reviewed-handoff',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Taskboard viewport reviewed handoff',
    intent: 'Keep the taskboard viewport strip aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderTaskboardViewportTokenRow({
    activeProject: reviewedData.activeProject,
    createToken,
    evidenceRail: reviewedRail,
    immediateTone: 'neutral',
    pendingApprovals: 0,
    pendingInboxItems: 0,
    taskCount: reviewedData.tasks.length,
  });
  ...
  assert.match(reviewedMarkup, /실행셀:1/);

  console.log(
    JSON.stringify(
      {
        ok: true,
        taskboardViewportHandoffTokens: {
          ...
          waitingCurrentOwner: waitingRail.currentOwnerLabel,
          waitingNextHandoff: waitingRail.nextHandoffLabel,
          verifiedCases: [
            'taskboard viewport strip shows current and next handoff tokens from the shared rail truth',
            'waiting path keeps the viewport strip grounded before any execution cell exists',
            'maker-blocked path reuses the existing first blocked reason flow without adding new summary language',
            'reviewed path keeps the viewport strip aligned with the current critic-to-human-gate handoff',
          ],
        },
      },
      null,
