...
assert.match(runbook, /Dogfood Run 040 retained-evidence docs are being recorded locally before destructive cleanup/);
...
assert.match(handoff, /Dogfood Run 040 retained-evidence is being recorded locally before destructive cleanup/);
assert.match(handoff, /Dogfood Run 040 retained linked worktree path `\/Users\/sungjin\/dev\/personal\/orchestration--v1-dogfood-runner-037` and branch `worktree\/v1-dogfood-runner-037` remain retained until retained-evidence is committed and destructive cleanup removes them/);
