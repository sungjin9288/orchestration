...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-345');
const port = 4646;
...
assert.match(appJs, /data-harness-result-show="true"[\s\S]*?>\s*결과 다시 보기\s*<\/button>/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-345 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-345-'));
  const inputPath = path.join(tempDir, 'hidden-restore-wording.txt');
  const outputPath = path.join(tempDir, 'hidden-restore-wording.md');

  fs.writeFileSync(inputPath, 'Execution hidden restore wording smoke\n', 'utf8');
