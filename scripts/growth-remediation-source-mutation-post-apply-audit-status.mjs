...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-remediation-source-mutation-post-apply-audit-status',
});
...
const SOURCE_MUTATION_POST_APPLY_AUDIT_STATES = [
  'source-mutation-post-apply-audit-not-started',
  'source-mutation-post-apply-audit-ready',
  'source-mutation-post-apply-audit-blocked',
  'source-mutation-post-apply-audit-ready-for-review-contract',
  'source-mutation-post-apply-audit-rejected',
  'source-mutation-post-apply-audit-deferred',
  ...
  'stale-post-apply-audit-blocked',
  'closed-no-action',
];

const SOURCE_MUTATION_POST_APPLY_AUDIT_DECISION_TYPES = [
  ...
  'reject-source-mutation-post-apply-audit',
  'defer-source-mutation-post-apply-audit',
  'hold-baseline',
];

const SOURCE_MUTATION_POST_APPLY_AUDIT_EVIDENCE_TYPES = [
...
const SOURCE_MUTATION_POST_APPLY_AUDIT_BLOCKER_TYPES = [
...
function summarizeSources(sources) {
  ...
  return {
    ...
    harnessMentionsSourceMutationPostApplyAudit:
      /growth-remediation-source-mutation-post-apply-audit-status/.test(harnessBaseline),
    completionReadinessMentionsSourceMutationPostApplyAudit:
      /growth-remediation-source-mutation-post-apply-audit-status/.test(completionReadiness),
    ledgerMentionsSourceMutationPostApplyAudit:
      /growth-remediation-source-mutation-post-apply-audit-status-readonly-post-m7-841/.test(
        todo,
      ),
    verificationIncludesSourceMutationPostApplyAudit:
      /growth-remediation-source-mutation-post-apply-audit-status/.test(verificationStatus),
    ...
    postApplyAuditSeparateFromMutation:
      /post-apply audit status stays separate from actual patch application/.test(plan) ||
      /post-apply audit record status stays separate from actual patch application/.test(plan),
    postApplyAuditReviewStillBlocked:
      /before\s+source\s+mutation post-apply audit review can be\s+considered/.test(plan),
...
const sourceMutationPostApplyAuditSchema = {
  sourceMutationPostApplyAuditRecord: fields(
    [
      'postApplyAuditId',
      'applyFinalizationId',
      'applyClosureId',
  ...
  sourceMutationPostApplyAuditDecision: fields(
    [
      'decisionId',
      'postApplyAuditId',
      'applyFinalizationId',
  ...
  sourceMutationPostApplyAuditBlocker: fields(
    [
      'blockerId',
      'postApplyAuditId',
      'applyFinalizationId',
  ...
  sourceMutationPostApplyAuditIndex: fields(
    [
      'indexId',
      'postApplyAuditRefs',
...
const sourceMutationPostApplyAuditRules = [
  {
    id: 'post-apply-audit-requires-current-apply-finalization',
    rule: 'source mutation post-apply audit can only bind to one current apply finalization record and its accepted apply execution chain',
  },
  {
    id: 'post-apply-audit-is-not-source-mutation',
    rule: 'source mutation post-apply audit status may mark audit-review contract readiness, but it cannot apply patches, mutate source, mutate accepted records, execute an actual audit workflow, or execute remediation',
  },
  {
    id: 'post-apply-audit-requires-clean-baseline-and-operator-dispatch-intent',
    rule: 'clean baseline proof, exact scope lock, target lock, baseline digest, operator dispatch intent, finalization decision notes, post-apply audit notes, and post-apply audit decision notes must be present before source mutation post-apply audit review can be considered',
  },
  {
    id: 'post-apply-audit-requires-verification-dry-run-and-rollback-proof',
    rule: 'patch draft refs, diff preview refs, verification output refs, dry-run proof refs, rollback proof refs, source-of-truth refs, task ledger refs, and negative evidence clearance must be present before source mutation post-apply audit review can be considered',
  },
  {
    id: 'failed-stale-broad-dirty-or-mutating-post-apply-audit-blocks-review',
    rule: 'stale finalization, stale closure, stale result acceptance, stale result review, stale result, stale execution, dirty or untracked baseline, changed patch or diff proof, failed verification, missing rollback proof, missing post-apply audit decision note, unresolved negative evidence, unapproved file touches, or any attempted source mutation blocks post-apply audit review readiness',
  },
];
...
const requiredFieldsSatisfied = Object.values(sourceMutationPostApplyAuditSchema).every(
...
const ok =
  ...
  sourceSummary.harnessMentionsSourceMutationPostApplyAudit &&
  sourceSummary.completionReadinessMentionsSourceMutationPostApplyAudit &&
  sourceSummary.ledgerMentionsSourceMutationPostApplyAudit &&
  sourceSummary.verificationIncludesSourceMutationPostApplyAudit &&
  sourceSummary.sourceMutationPostApplyAuditReviewNextDocumented &&
  ...
  sourceSummary.postApplyAuditSeparateFromMutation &&
  sourceSummary.postApplyAuditReviewStillBlocked &&
...
const payload = {
  ok,
  mode: 'growth-remediation-source-mutation-post-apply-audit-status',
  posture: 'local-read-only-remediation-source-mutation-post-apply-audit-contract',
  currentHead: {
  ...
  schemaVersion: 'growth-remediation-source-mutation-post-apply-audit-status/v0',
  sourceSummary,
  vocabulary: {
    sourceMutationPostApplyAuditStates: SOURCE_MUTATION_POST_APPLY_AUDIT_STATES,
    sourceMutationPostApplyAuditDecisionTypes: SOURCE_MUTATION_POST_APPLY_AUDIT_DECISION_TYPES,
    sourceMutationPostApplyAuditEvidenceTypes: SOURCE_MUTATION_POST_APPLY_AUDIT_EVIDENCE_TYPES,
    sourceMutationPostApplyAuditBlockerTypes: SOURCE_MUTATION_POST_APPLY_AUDIT_BLOCKER_TYPES,
  },
  sourceMutationPostApplyAuditSchema,
  sourceMutationPostApplyAuditRules,
  sourceMutationPostApplyAuditState: {
    realSourceMutationPostApplyAuditFileAdopted: false,
    discoveredSourceMutationPostApplyAuditRecords: 0,
    postApplyAuditReviewAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    remediationExecutionAllowed: false,
    currentStatus: 'contract-only-no-active-source-mutation-post-apply-audit',
  },
  readiness: {
    requiredFieldsSatisfied,
    sourceMutationPostApplyAuditRecordTypes: Object.keys(sourceMutationPostApplyAuditSchema).length,
    ...
    postApplyAuditAndMutationSeparate: true,
    postApplyAuditReviewAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    ...
    readyForSourceMutationPostApplyAuditReviewStatus: true,
  },
  nextRecommendedSlice: {
    id: 'growth-remediation-source-mutation-post-apply-audit-review-status',
    commandToAdd: 'node scripts/growth-remediation-source-mutation-post-apply-audit-review-status.mjs',
    reason:
      'Source mutation post-apply audit is now modeled as read-only; the next safe slice should define post-apply audit review readiness without applying patches or mutating source.',
