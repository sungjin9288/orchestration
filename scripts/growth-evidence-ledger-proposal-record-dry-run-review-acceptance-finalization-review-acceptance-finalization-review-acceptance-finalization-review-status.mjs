...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-finalization-review-status',
});
...
const REVIEW_CRITERIA = [
  {
    ...
    question:
      'Is the dry-run review acceptance finalization review acceptance finalization review acceptance finalization status green and ready for review?',
    requiredEvidenceRef: 'finalizationEnvelope.compatibility.finalizationState',
  },
  {
    id: 'finalization-findings-passed',
    question:
      'Did every dry-run review acceptance finalization review acceptance finalization review acceptance finalization finding pass without evidence gaps?',
    requiredEvidenceRef: 'finalizationEnvelope.finalizationFindings',
  },
  {
    id: 'finalization-remains-non-approval',
    question:
      'Does review keep finalized finalization review acceptance evidence separate from proposal approval?',
...
function summarizeSources(sources) {
  ...
  return {
    ...
    lessonCaptured:
      /proposal record dry-run review acceptance finalization review acceptance finalization review acceptance finalization review.*proposal approval/i.test(
        lessons,
      ) ||
      /proposal approval.*proposal record dry-run review acceptance finalization review acceptance finalization review acceptance finalization review/i.test(
        lessons,
      ),
  ...
  return [
    ...
    },
    {
      ...
      ok:
        ...
        safetyBoundary.doesNotPromoteFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalizationToRecord ===
          true,
      evidenceRef: 'readiness and safetyBoundary',
      observed: {
...
function buildReviewEnvelope({ finalizationPayload }) {
  ...
  return {
    ...
    reviewEnvelope: {
      reviewId:
        'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-finalization-review-candidate',
      ...
      reviewPurpose:
        'Review finalized dry-run finalization review acceptance finalization review acceptance finalization evidence only for the next read-only acceptance check while keeping proposal approval, record creation, persistence, queue mutation, and implementation authority blocked.',
      reviewInputs: {
        finalizationState:
      ...
      blockedActions,
      acceptanceQuestion:
        'Can this reviewed dry-run finalization review acceptance finalization review acceptance finalization evidence move to a read-only acceptance check without approving, creating, persisting, or mutating proposal records?',
      nonApprovalStatement:
...
const payload = {
  ...
  sourceSummary,
  inputStatuses: {
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalization: {
  ...
  nextRecommendedSlice: {
    ...
    reason:
      'The dry-run review acceptance finalization review acceptance finalization review acceptance finalization evidence is reviewed only for a read-only acceptance check; the next safe slice can accept review evidence without approving, creating, persisting, implementing, or mutating proposal records.',
  ...
  failures: {
    missingSources,
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalizationFailed:
