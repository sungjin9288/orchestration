...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-356');
const port = 4657;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-356 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-356-'));
  const inputPath = path.join(tempDir, 'latest-result-output-copy-wording.txt');
  const outputPath = path.join(tempDir, 'latest-result-output-copy-wording.md');

  fs.writeFileSync(inputPath, 'Latest result output copy wording smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionResultOutputCopyWording: {
            insertionPoint: 'executionResultRegister->visibleOutputCopyWording->outputCopyActionLabel',
            actionLabel: '출력 경로',
