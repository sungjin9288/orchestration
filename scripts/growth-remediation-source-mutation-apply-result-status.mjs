...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-remediation-source-mutation-apply-result-status',
});
...
const SOURCE_MUTATION_APPLY_RESULT_STATES = [
  'source-mutation-apply-result-not-started',
  'source-mutation-apply-result-ready',
  'source-mutation-apply-result-blocked',
  'source-mutation-apply-result-ready-for-result-review-contract',
  'source-mutation-apply-result-rejected',
  'source-mutation-apply-result-deferred',
  ...
  'stale-apply-result-blocked',
  'closed-no-action',
];

const SOURCE_MUTATION_APPLY_RESULT_DECISION_TYPES = [
  ...
  'reject-source-mutation-apply-result',
  'defer-source-mutation-apply-result',
  'hold-baseline',
];

const SOURCE_MUTATION_APPLY_RESULT_EVIDENCE_TYPES = [
...
const SOURCE_MUTATION_APPLY_RESULT_BLOCKER_TYPES = [
...
function summarizeSources(sources) {
  ...
  return {
    ...
    harnessMentionsSourceMutationApplyResult:
      /growth-remediation-source-mutation-apply-result-status/.test(harnessBaseline),
    completionReadinessMentionsSourceMutationApplyResult:
      /growth-remediation-source-mutation-apply-result-status/.test(completionReadiness),
    ledgerMentionsSourceMutationApplyResult:
      /growth-remediation-source-mutation-apply-result-status-readonly-post-m7-836/.test(todo),
    verificationIncludesSourceMutationApplyResult:
      /growth-remediation-source-mutation-apply-result-status/.test(verificationStatus),
    ...
    applyResultSeparateFromMutation:
      /apply result status stays separate from actually applying patches/.test(plan),
    applyResultReviewStillBlocked:
      /before\s+source mutation apply result review can be considered/.test(plan),
...
const sourceMutationApplyResultSchema = {
  sourceMutationApplyResultRecord: fields(
    [
      'applyResultId',
      'applyExecutionId',
      'applyDispatchId',
  ...
  sourceMutationApplyResultDecision: fields(
    [
      'decisionId',
      'applyResultId',
      'decisionType',
  ...
  sourceMutationApplyResultBlocker: fields(
    [
      'blockerId',
      'applyResultId',
      'blockerType',
  ...
  sourceMutationApplyResultIndex: fields(
    [
      'indexId',
      'applyResultRefs',
      'stateCounts',
...
const sourceMutationApplyResultRules = [
  {
    id: 'apply-result-requires-current-apply-execution',
    rule: 'source mutation apply result can only bind to one current apply execution record and its apply dispatch chain',
  },
  {
    id: 'apply-result-is-not-source-mutation',
    rule: 'source mutation apply result status may mark apply-result-review-contract readiness, but it cannot apply patches, mutate source, apply proposals, approve authorizations, or execute remediation',
  },
  {
    id: 'apply-result-requires-clean-baseline-and-operator-dispatch-intent',
    rule: 'clean baseline proof, exact scope lock, target lock, baseline digest, and operator dispatch intent must be present before source mutation apply result review can be considered',
  },
  {
    id: 'apply-result-requires-verification-dry-run-and-rollback-proof',
    rule: 'patch draft refs, diff preview refs, verification output refs, dry-run proof refs, rollback proof refs, source-of-truth refs, task ledger refs, and negative evidence clearance must be present before source mutation apply result review can be considered',
  },
  {
    id: 'failed-stale-broad-dirty-or-mutating-result-blocks-apply-result-review',
    rule: 'stale execution, dirty or untracked baseline, changed patch/diff proof, failed verification, missing rollback proof, missing operator dispatch intent, unresolved negative evidence, unapproved file touches, or any attempted source mutation blocks apply result review readiness',
  },
];
...
const requiredFieldsSatisfied = Object.values(sourceMutationApplyResultSchema).every(
...
const ok =
  ...
  sourceSummary.harnessMentionsSourceMutationApplyResult &&
  sourceSummary.completionReadinessMentionsSourceMutationApplyResult &&
  sourceSummary.ledgerMentionsSourceMutationApplyResult &&
  sourceSummary.verificationIncludesSourceMutationApplyResult &&
  sourceSummary.sourceMutationApplyResultReviewNextDocumented &&
  ...
  sourceSummary.applyResultSeparateFromMutation &&
  sourceSummary.applyResultReviewStillBlocked &&
...
const payload = {
  ok,
  mode: 'growth-remediation-source-mutation-apply-result-status',
  posture: 'local-read-only-remediation-source-mutation-apply-result-contract',
  currentHead: {
  ...
  schemaVersion: 'growth-remediation-source-mutation-apply-result-status/v0',
  sourceSummary,
  vocabulary: {
    sourceMutationApplyResultStates: SOURCE_MUTATION_APPLY_RESULT_STATES,
    sourceMutationApplyResultDecisionTypes: SOURCE_MUTATION_APPLY_RESULT_DECISION_TYPES,
    sourceMutationApplyResultEvidenceTypes: SOURCE_MUTATION_APPLY_RESULT_EVIDENCE_TYPES,
    sourceMutationApplyResultBlockerTypes: SOURCE_MUTATION_APPLY_RESULT_BLOCKER_TYPES,
  },
  sourceMutationApplyResultSchema,
  sourceMutationApplyResultRules,
  sourceMutationApplyResultState: {
    realSourceMutationApplyResultFileAdopted: false,
    discoveredSourceMutationApplyResultRecords: 0,
    applyResultReviewAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    remediationExecutionAllowed: false,
    currentStatus: 'contract-only-no-active-source-mutation-apply-result',
  },
  readiness: {
    requiredFieldsSatisfied,
    sourceMutationApplyResultRecordTypes: Object.keys(sourceMutationApplyResultSchema).length,
    ...
    applyResultAndMutationSeparate: true,
    applyResultReviewAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    ...
    readyForSourceMutationApplyResultReviewStatus: true,
  },
  nextRecommendedSlice: {
    id: 'growth-remediation-source-mutation-apply-result-review-status',
    commandToAdd: 'node scripts/growth-remediation-source-mutation-apply-result-review-status.mjs',
    reason:
      'Source mutation apply result is now modeled as read-only; the next safe slice should define result-review records without applying patches or mutating source.',
