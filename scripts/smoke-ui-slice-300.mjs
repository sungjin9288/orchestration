...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-300');
const port = 4600;
const baseUrl = `http://127.0.0.1:${port}`;

const appJs = fs.readFileSync(appPath, 'utf8');

assert.match(appJs, /function renderDecisionInbox\(data\) \{/);
...
assert.match(
  appJs,
  /\$\{decisionDeck\}\s*\n\s*\$\{renderHarnessBriefRegister\(harnessBrief\)\}\s*\n\s*<div class="surface-grid surface-grid-inbox">/,
);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-300 server');
}
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessDecisionInboxRegister: {
            derivedKey: 'harnessConsumerBrief',
            insertionPoint: 'decisionDeck->harnessRegister->surfaceGrid',
