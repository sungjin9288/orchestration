...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-187');
...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Deliverables advanced-ops entry summary aligned to the current evidence rail handoff truth.',
  };
...
function renderOpsEntryTokenRow({ createToken, evidenceRail }) {
  return `
    <div class="token-row">
      ${createToken(
...
assert.match(appJs, /<strong>관제실 직행<\/strong>/);
assert.match(
  appJs,
...
assert.match(appJs, /\$\{opsEntrySignalRow\}/);
assert.match(appJs, /<p class="detail-copy">\$\{escapeHtml\(opsEntryHelperCopy\)\}<\/p>/);

try {
  runtime.resetRuntime();

  const blockedProjectPath = fs.mkdtempSync(
    path.join(os.tmpdir(), 'orchestration-ui-slice-187-provider-'),
  );
  const blockedProject = runtime.createProject({
    name: 'deliverables-ops-entry-maker-blocked',
    projectPath: blockedProjectPath,
    provider: { mode: 'live' },
  });
  const blockedTask = runtime.createTask({
    projectId: blockedProject.id,
    title: 'Deliverables advanced ops entry maker blocked',
    intent: 'Keep the advanced ops entry row aligned when maker is blocked.',
  });
  ...
  const blockedMarkup = renderOpsEntryTokenRow({
    createToken,
  ...
  const reviewedProject = runtime.createProject({
    name: 'deliverables-ops-entry-reviewed',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Deliverables advanced ops entry reviewed handoff',
    intent: 'Keep the advanced ops entry row aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderOpsEntryTokenRow({
    createToken,
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        deliverablesOpsEntryHandoffTokens: {
          ...
          verifiedCases: [
            'advanced ops entry row shows current and next handoff tokens from the shared rail truth',
            'maker-blocked path keeps the advanced ops entry row grounded in the existing blocked reason flow',
            'reviewed path keeps the advanced ops entry row aligned with the current critic-to-human-gate handoff',
          ],
        },
      },
      null,
