...
assert.match(councilConfigJs, /프로젝트 지정 후 실행/);
assert.match(councilConfigJs, /리뷰 후 완료/);
assert.match(councilConfigJs, /승인 후 커밋/);
assert.match(councilConfigJs, /한정된 실행 유지/);
...
assert.match(councilConfigJs, /summary: '각 역할이 같은 안건을 함께 읽고 방향을 정리합니다\.'/);
