...
assert.equal(payload.summary.primaryExecutable, primaryHarness?.executable ?? null);
assert.equal(payload.summary.primaryExecutable, true);
  ...
  JSON.stringify(
    {
      ok: true,
      runScript,
      primaryExecutable: payload.summary.primaryExecutable,
    },
    null,
    2,
  ),
