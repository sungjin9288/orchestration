...
const completionReviewScript = path.join(
  repoRoot,
  'scripts',
  'growth-remediation-source-mutation-completion-review-status.mjs',
);

function runCompletionReviewStatus(args = []) {
  const result = spawnSync(process.execPath, [completionReviewScript, ...args], {
    cwd: repoRoot,
    encoding: 'utf8',
...
const result = runCompletionReviewStatus();
assert.equal(
  result.status,
  0,
  `growth-remediation-source-mutation-completion-review-status failed: ${result.stderr}`,
);
const payload = result.payload;

assert.equal(payload.ok, true);
assert.equal(payload.mode, 'growth-remediation-source-mutation-completion-review-status');
assert.equal(
  payload.posture,
  'local-read-only-remediation-source-mutation-completion-review-contract',
);
assert.equal(
  payload.schemaVersion,
  'growth-remediation-source-mutation-completion-review-status/v0',
);
...
assert.equal(payload.sourceSummary.harnessMentionsSourceMutationCompletionReview, true);
assert.equal(
  payload.sourceSummary.completionReadinessMentionsSourceMutationCompletionReview,
  true,
);
assert.equal(payload.sourceSummary.ledgerMentionsSourceMutationCompletionReview, true);
assert.equal(payload.sourceSummary.verificationIncludesSourceMutationCompletionReview, true);
assert.equal(payload.sourceSummary.sourceMutationCompletionReviewAcceptanceNextDocumented, true);
...
assert.equal(payload.sourceSummary.sourceMutationCompletionReviewSeparateFromMutation, true);
assert.equal(payload.sourceSummary.sourceMutationCompletionReviewAcceptanceStillBlocked, true);
...
for (const state of [
  ...
  assert.ok(payload.vocabulary.sourceMutationCompletionReviewStates.includes(state));
}
assert.ok(
  payload.vocabulary.sourceMutationCompletionReviewDecisionTypes.includes(
    'approve-source-mutation-completion-review-acceptance-readiness',
  ),
);
assert.ok(
  payload.vocabulary.sourceMutationCompletionReviewEvidenceTypes.includes(
...
assert.ok(
  payload.vocabulary.sourceMutationCompletionReviewBlockerTypes.includes(
    'source-mutation-completion-review-status-attempts-source-mutation',
  ),
);

const recordRequired =
  payload.sourceMutationCompletionReviewSchema.sourceMutationCompletionReviewRecord.required;
for (const field of [
...
assert.ok(
  payload.sourceMutationCompletionReviewSchema.sourceMutationCompletionReviewDecision.required.includes(
    'allowedNextAction',
  ),
);
assert.ok(
  payload.sourceMutationCompletionReviewSchema.sourceMutationCompletionReviewBlocker.required.includes(
    'blocksSourceMutationCompletionReviewAcceptance',
  ),
);
assert.ok(
  payload.sourceMutationCompletionReviewSchema.sourceMutationCompletionReviewIndex.required.includes(
    'sourceMutationCompletionReviewRefs',
  ),
);

for (const ruleId of [
  'source-mutation-completion-review-requires-current-completion',
  'source-mutation-completion-review-is-not-source-mutation',
  'source-mutation-completion-review-requires-reviewer-and-decision-notes',
  'source-mutation-completion-review-requires-verification-dry-run-and-rollback-proof',
  'failed-stale-broad-dirty-or-mutating-completion-review-blocks-acceptance',
]) {
  assert.ok(payload.sourceMutationCompletionReviewRules.some((rule) => rule.id === ruleId));
}

assert.equal(
  payload.sourceMutationCompletionReviewState.realSourceMutationCompletionReviewFileAdopted,
  false,
);
assert.equal(payload.sourceMutationCompletionReviewState.discoveredSourceMutationCompletionReviewRecords, 0);
assert.equal(
  payload.sourceMutationCompletionReviewState.sourceMutationCompletionReviewAcceptanceAllowed,
  false,
);
assert.equal(payload.sourceMutationCompletionReviewState.sourceMutationAllowed, false);
assert.equal(payload.sourceMutationCompletionReviewState.acceptedRecordMutationAllowed, false);
assert.equal(payload.sourceMutationCompletionReviewState.rollbackExecutionAllowed, false);
assert.equal(payload.sourceMutationCompletionReviewState.remediationExecutionAllowed, false);
assert.equal(
  payload.sourceMutationCompletionReviewState.currentStatus,
  'contract-only-no-active-source-mutation-completion-review',
);
assert.equal(payload.readiness.requiredFieldsSatisfied, true);
assert.equal(payload.readiness.sourceMutationCompletionReviewRecordTypes, 4);
...
assert.equal(payload.readiness.sourceMutationCompletionReviewerNoteRequired, true);
assert.equal(payload.readiness.sourceMutationCompletionReviewDecisionNoteRequired, true);
...
assert.equal(payload.readiness.sourceMutationCompletionReviewAndMutationSeparate, true);
assert.equal(payload.readiness.sourceMutationCompletionReviewAcceptanceAllowed, false);
...
assert.equal(payload.readiness.readyForSourceMutationCompletionReviewAcceptanceStatus, true);
assert.equal(
...
const typoResult = runCompletionReviewStatus(['--typo']);
assert.equal(typoResult.status, 2);
assert.equal(typoResult.payload?.ok, false);
assert.equal(typoResult.payload?.mode, 'growth-remediation-source-mutation-completion-review-status');
...
assert.match(plan, /node scripts\/growth-remediation-source-mutation-completion-review-status\.mjs/);
...
console.log(
  JSON.stringify(
    {
      ok: true,
      growthRemediationSourceMutationCompletionReviewStatus: {
        command: 'node scripts/growth-remediation-source-mutation-completion-review-status.mjs',
