...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Decision Inbox list rows aligned to the current evidence rail handoff truth.',
  };
...
function renderInboxListTokens({
  approval,
  createToken,
  evidenceRail,
  formatDate,
  ...
  const inboxSnapshot = getInboxListSnapshot(item, task, approval);
...
assert.match(appJs, /const inboxEvidenceState = getExecutionEvidenceRail\(inboxTask, data\);/);
assert.match(
  appJs,
  /<div class="token-row token-row-compact">[\s\S]*createToken\(\s*`현재:\$\{inboxEvidenceState\.currentOwnerLabel\}`,\s*inboxEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{inboxEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*\$\{inboxSnapshot\.tokens\.join\(''\)\}[\s\S]*\$\{createToken\(formatDate\(item\.updatedAt \|\| item\.createdAt\), 'neutral'\)\}/,
);
...
try {
  ...
  const pendingProject = runtime.createProject({
    name: 'decision-list-pending-approval',
    projectPath: repoRoot,
  });
  const pendingTask = runtime.createTask({
    projectId: pendingProject.id,
    title: 'Decision list pending approval',
    intent: 'Keep the Decision Inbox list rows aligned while builder approval is pending.',
  });
  ...
  const pendingMarkup = renderInboxListTokens({
  ...
  const reviewedProject = runtime.createProject({
    name: 'decision-list-reviewed-follow-up',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Decision list reviewed follow-up',
    intent: 'Keep the Decision Inbox list rows aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderInboxListTokens({
    approval: null,
    createToken,
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        decisionInboxListHandoffTokens: {
          ...
          verifiedCases: [
            'decision inbox list rows show current and next handoff tokens from the shared rail truth',
            'pending builder approval rows keep the list summary aligned with the existing approval kind and status tokens',
            'reviewed follow-up decision rows keep the list summary aligned after the task reaches the critic-to-human-gate handoff',
          ],
        },
      },
      null,
