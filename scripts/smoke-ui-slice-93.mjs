...
assert.match(appJs, /copy: '현재 작업 셀과 다음 배정을 조정합니다\.'/);
assert.match(appJs, /copy: '현재 실행 로그와 다음 확인 대상을 빠르게 훑습니다\.'/);
assert.match(appJs, /copy: '현재 증적과 연결 근거를 검토합니다\.'/);
assert.match(appJs, /copy: '현재 승인 안건과 다음 처리를 판단합니다\.'/);
assert.match(appJs, /kicker: '실행 셀'/);
assert.match(appJs, /kicker: '실행 로그'/);
assert.match(appJs, /kicker: '증적'/);
assert.match(appJs, /kicker: '승인선'/);
