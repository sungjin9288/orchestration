...
assert.match(appJs, /copy: '왼쪽 버튼을 누르면 해당 desk만 열립니다\. 작업 결과는 산출물, 근거는 아티팩트, 실행 흐름은 로그에서 확인합니다\.'/);
assert.match(appJs, /copy: '승인이 필요하면 결정함, 실제 작업 근거는 아티팩트, 실행 중 무슨 일이 있었는지는 로그에서 봅니다\.'/);
assert.match(appJs, /copy: '작업판은 세부 실행 셀의 관제 위치입니다\. 여기서 담당 desk와 역할을 정리하고 실제 결과 확인은 산출물·아티팩트로 돌아갑니다\.'/);
