...
assert.equal(payload.summary.primaryInstallReview, primaryHarness?.installReview ?? null);
assert.equal(payload.summary.primaryInstallReview, 'pipx install markitdown');
  ...
  JSON.stringify(
    {
      ok: true,
      runScript,
      primaryInstallReview: payload.summary.primaryInstallReview,
    },
    null,
    2,
  ),
