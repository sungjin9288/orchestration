...
assert.match(appJs, /const SURFACE_NAV_GUIDANCE = \{/);
assert.match(appJs, /mission:\s*'목표와 제약을 입력하고 첫 안건을 확인'/);
assert.match(appJs, /execution:\s*'진행 중 작업, 막힘, 다음 실행을 확인'/);
assert.match(appJs, /deliverables:\s*'완료 결과와 인계 패킷을 확인'/);
assert.match(appJs, /artifacts:\s*'작업 증적, 파일, 패킷 근거를 확인'/);
assert.match(appJs, /logs:\s*'실행 run 기록과 오류 흐름을 추적'/);
assert.match(appJs, /'decision-inbox':\s*'승인·보류가 필요한 사람 판단을 처리'/);
assert.match(appJs, /작업 결과는 산출물, 근거는 아티팩트, 실행 흐름은 로그에서 확인합니다\./);
assert.match(appJs, /승인이 필요하면 결정함, 실제 작업 근거는 아티팩트, 실행 중 무슨 일이 있었는지는 로그에서 봅니다\./);
assert.match(appJs, /작업판은 세부 실행 셀의 관제 위치입니다\./);
