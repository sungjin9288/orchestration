...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-status',
});
...
const ACCEPTANCE_CRITERIA = [
  {
    ...
    question: 'Is the dry-run review acceptance finalization review status green and ready for acceptance?',
    requiredEvidenceRef: 'reviewEnvelope.compatibility.reviewState',
  },
  {
    id: 'review-findings-passed',
    question: 'Did every dry-run review acceptance finalization review finding pass without evidence gaps?',
    requiredEvidenceRef: 'reviewEnvelope.reviewFindings',
  },
  {
    id: 'review-remains-non-approval',
    question: 'Does acceptance keep reviewed finalization evidence separate from proposal approval?',
...
function summarizeSources(sources) {
  ...
  return {
    ...
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusDocumented:
    ...
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusAggregateRegistered:
      /growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-status/.test(
        verificationStatus,
      ),
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusLedgered:
      /growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-status-readonly-post-m7/.test(
        todo,
      ),
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceLessonCaptured:
      /proposal record dry-run review acceptance finalization review acceptance.*proposal approval/i.test(
        lessons,
      ) ||
      /proposal approval.*proposal record dry-run review acceptance finalization review acceptance/i.test(
        lessons,
      ),
    engineRoutesPastProposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptance:
      /nextRecommendedSlice[\s\S]*growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization/.test(
        engine,
      ),
    reflectionRoutesPastProposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptance:
      /nextRecommendedSlice[\s\S]*growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization/.test(
        reflection,
      ),
  ...
  return [
    {
      ...
      ok:
        ...
        reviewCompatibility.reviewState ===
    ...
    },
    {
      ...
      ok:
        ...
        safetyBoundary.doesNotPromoteFinalizationReviewToRecord === true,
      evidenceRef: 'readiness and safetyBoundary',
      observed: {
        ...
        doesNotPromoteFinalizationReviewToRecord:
          safetyBoundary.doesNotPromoteFinalizationReviewToRecord,
      },
    },
  ];
}
...
function buildAcceptanceEnvelope({ reviewPayload }) {
  ...
  return {
    ...
    acceptanceEnvelope: {
      acceptanceId:
        'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-candidate',
      sourceReviewId: reviewEnvelope.reviewId || null,
      targetQueueContract: 'growth-proposal-queue-status',
      targetSchema: 'proposalRecord',
      acceptancePurpose:
        'Accept reviewed dry-run finalization evidence only for the next read-only finalization check while keeping proposal approval, record creation, persistence, queue mutation, and implementation authority blocked.',
      acceptanceInputs: {
        reviewState:
          reviewPayload?.reviewEnvelope?.compatibility?.reviewState || null,
        reviewFindingIds: (reviewEnvelope.reviewFindings || []).map(
          (finding) => finding.id,
        ),
        acceptedReviewStates:
          reviewEnvelope.acceptedReviewStates || [],
        rejectedReviewStates:
          reviewEnvelope.rejectedReviewStates || [],
      ...
      blockedActions,
      finalizationQuestion:
        'Can this accepted dry-run finalization review evidence move to a read-only finalization check without approving, creating, persisting, or mutating proposal records?',
      nonApprovalStatement:
        'Proposal record dry-run review acceptance finalization review acceptance is acceptance for a later read-only finalization check only; it is not proposal approval, proposal record creation, queue persistence, implementation authorization, or source mutation authority.',
    },
    compatibility: {
      reviewStatusReady:
        reviewPayload?.ok === true &&
      ...
      acceptanceDoesNotApprove:
        ...
        blockedActions.includes('accept-dry-run-finalization-review-acceptance-as-approval') &&
      ...
      durableRecordPromotionBlocked:
        ...
        blockedActions.includes('promote-dry-run-finalization-review-acceptance-to-record'),
    },
  };
}
...
function buildReadiness({ sourceSummary, reviewResult, acceptanceEnvelope }) {
  ...
  return {
    ...
    docsAndAggregateReady:
      sourceSummary.proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusDocumented &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusAggregateRegistered &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceStatusLedgered &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceLessonCaptured,
    engineReflectionAdvanced:
      sourceSummary.engineRoutesPastProposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptance &&
      sourceSummary.reflectionRoutesPastProposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptance,
...
const reviewResult = runStatusScript(
  'scripts/growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-status.mjs',
);
const acceptanceEnvelope = buildAcceptanceEnvelope({
  reviewPayload: reviewResult.payload,
});
...
const payload = {
  ...
  nextRecommendedSlice: {
    ...
    reason:
      'The dry-run review acceptance finalization review evidence is accepted only for a read-only finalization check; the next safe slice can finalize acceptance evidence without approving, creating, persisting, implementing, or mutating proposal records.',
