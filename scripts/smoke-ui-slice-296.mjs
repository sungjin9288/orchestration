...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-296');
const port = 4596;
...
assert.match(appJs, /data-harness-register="true"/);
assert.match(appJs, /Harness brief/);
assert.match(appJs, /하네스 실행 안내/);
assert.match(appJs, /brief\.actionMessage/);
assert.match(appJs, /brief\.actionCommand/);
assert.match(appJs, /\$\{renderHarnessBriefRegister\(harnessBrief\)\}/);
assert.doesNotMatch(appJs, /harness-run\.mjs doctor/);
assert.doesNotMatch(appJs, /harness-consumer-status\.mjs/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-296 server');
}
      ...
      JSON.stringify(
        {
          ...
          harnessOpsRegister: {
            ...
            headline: harnessBrief.headline,
