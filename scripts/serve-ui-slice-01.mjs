...
const server = createServer(async (request, response) => {
  ...
  const learningCandidateMemoryItemMatch = url.pathname.match(
    /^\/api\/learning-candidates\/([^/]+)\/memory-item$/,
  );
  if (method === 'GET' && learningCandidateMemoryItemMatch) {
    try {
      const learningCandidateId = decodeURIComponent(
        learningCandidateMemoryItemMatch[1],
      );
      const result = runtime.getLearningCandidateMemoryItem(learningCandidateId);
      json(response, 200, {
        ...result,
        generatedAt: new Date().toISOString(),
      });
      ...
      const statusCode =
        error.statusCode || (/not found/i.test(error.message) ? 404 : 400);
      json(response, statusCode, {
        error: error.message || 'MemoryItem 조회에 실패했습니다.',
      });
      return;
    }
  }

  const learningCandidatePersistMemoryItemMatch = url.pathname.match(
    /^\/api\/learning-candidates\/([^/]+)\/persist-memory-item$/,
  );
  if (method === 'POST' && learningCandidatePersistMemoryItemMatch) {
    try {
      const learningCandidateId = decodeURIComponent(
        learningCandidatePersistMemoryItemMatch[1],
      );
      const input = await readBoundedJsonBody(request, 64 * 1024);
      const expectedFields = [
        'learningCandidateReviewId',
      ...
      if (
        ...
        const error = new Error(
          'MemoryItem persistence body has unexpected or missing fields',
        );
        error.statusCode = 400;
        throw error;
      }
      const result = runtime.persistLearningCandidateMemoryItem({
        learningCandidateId,
        ...input,
      });
      json(response, result.idempotent ? 200 : 201, {
        learningCandidate: result.learningCandidate,
        learningCandidateReview: result.learningCandidateReview,
        memoryCandidatePreview: result.memoryCandidatePreview,
        memoryItem: result.memoryItem,
        mutation: {
          idempotent: result.idempotent,
          kind: 'persist-memory-item',
          learningCandidateId,
          learningCandidateReviewId: result.learningCandidateReview.id,
          memoryItemId: result.memoryItem.id,
          stoppedAt: 'memory-item-stored-application-blocked',
        },
      });
      return;
    } catch (error) {
      const statusCode =
        error.statusCode || (/not found/i.test(error.message) ? 404 : 400);
      json(response, statusCode, {
        error: error.message || 'MemoryItem 저장에 실패했습니다.',
      });
      return;
    }
  }

  const memoryItemRecallPreviewMatch = url.pathname.match(
    /^\/api\/memory-items\/([^/]+)\/recall-preview$/,
  );
  if (method === 'POST' && memoryItemRecallPreviewMatch) {
    try {
      const memoryItemId = decodeURIComponent(memoryItemRecallPreviewMatch[1]);
      const input = await readBoundedJsonBody(request, 64 * 1024);
      const expectedFields = [
      ...
      if (
        ...
        const error = new Error(
          'MemoryRecall preview body has unexpected or missing fields',
        );
        error.statusCode = 400;
        throw error;
      }
      const memoryRecallPreview = runtime.previewMemoryItemRecall({
        memoryItemId,
        ...input,
      });
      json(response, 200, {
      ...
      const statusCode =
        error.statusCode || (/not found/i.test(error.message) ? 404 : 400);
      json(response, statusCode, {
        error: error.message || 'MemoryRecall preview 생성에 실패했습니다.',
      });
      return;
    }
  }
