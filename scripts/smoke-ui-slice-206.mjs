...
assert.match(appJs, /현재 preflight 아티팩트 기준으로 빌더 라이브 변경 승인 게이트가 생성된 상태입니다\./);
...
assert.match(appJs, /빌더 preflight는 \$\{escapeHtml\(builderPreflightState\.latestPlanArtifact\?\.id \|\| '최신 계획 아티팩트'\)\}, \$\{escapeHtml\(builderPreflightState\.latestArchitectureArtifact\?\.id \|\| '최신 설계 아티팩트'\)\}, \$\{escapeHtml\(builderPreflightState\.latestBreakdownArtifact\?\.id \|\| '최신 분해 아티팩트'\)\}를 읽고 쓰기 없는 preflight 아티팩트를 남긴 뒤 리뷰어를 명시적 후속 단계로 남깁니다\./);
assert.match(appJs, /아직 breakdown 아티팩트가 없습니다\. 계획과 설계 아티팩트가 준비된 뒤 태스크 분해를 실행합니다\./);
assert.match(appJs, /아직 빌더 preflight 아티팩트가 없습니다\. 계획, 설계, 분해 아티팩트가 준비된 뒤 빌더 preflight를 실행합니다\./);
  ...
  JSON.stringify(
    {
      ...
      builderPreflightHelperCopy: {
        markers: ['계획 아티팩트', '설계 아티팩트', '분해 아티팩트', '쓰기 없는 preflight'],
      },
    },
    null,
    2,
