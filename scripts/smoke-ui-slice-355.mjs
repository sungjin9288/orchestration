...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-355');
const port = 4656;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-355 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-355-'));
  const inputPath = path.join(tempDir, 'latest-result-input-copy-wording.txt');
  const outputPath = path.join(tempDir, 'latest-result-input-copy-wording.md');

  fs.writeFileSync(inputPath, 'Latest result input copy wording smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionResultInputCopyWording: {
            insertionPoint: 'executionResultRegister->visibleInputCopyWording->inputCopyActionLabel',
            actionLabel: '입력 경로',
