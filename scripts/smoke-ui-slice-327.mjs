...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-327');
const port = 4628;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-327 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-327-'));
  const inputPath = path.join(tempDir, 'hidden-path-summary.txt');
  const outputPath = path.join(tempDir, 'hidden-path-summary.md');

  fs.writeFileSync(inputPath, 'Execution hidden path summary smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHiddenPathSummary: {
