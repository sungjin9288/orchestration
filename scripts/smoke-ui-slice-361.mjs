...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-361');
const port = 4662;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-361 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-361-'));
  const inputPath = path.join(tempDir, 'latest-result-detail-density.txt');
  const outputPath = path.join(tempDir, 'latest-result-detail-density.md');

  fs.writeFileSync(inputPath, 'Latest result detail density smoke\n', 'utf8');
