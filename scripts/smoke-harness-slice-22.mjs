...
assert.equal(payload.summary.primaryPosture, primaryHarness?.posture ?? null);
assert.equal(payload.summary.primaryPosture, 'approved-now');
  ...
  JSON.stringify(
    {
      ok: true,
      runScript,
      primaryPosture: payload.summary.primaryPosture,
    },
    null,
    2,
  ),
