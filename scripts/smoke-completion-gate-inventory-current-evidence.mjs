...
const inventoryGateRows = [
  ...
  /\| Required aggregate synthetic gate \| pass \| `node scripts\/verification_status\.mjs` \| Current worktree returns `ok=true`: required `1\/1` and informational `312\/312` pass; total passed is `313\/313`\. \| Keep the required docs\/runtime aggregate gate authoritative and do not report optional live-provider checks as completed\. \|/,
  ...
  /\| Completion gate inventory current evidence \| pass \| `node scripts\/smoke-completion-gate-inventory-current-evidence\.mjs`, `node scripts\/verification_status\.mjs` \| Current worktree inventory is pinned to aggregate required `1\/1`, informational `312\/312`, total `313\/313`, UI QA `94\/94`, zero unchecked task lines, and AI Company Stage 7A implementation through DEC-227\. \| Keep this smoke in aggregate so required versus informational results do not drift behind README, UI QA, or growth routing evidence\. \|/,
