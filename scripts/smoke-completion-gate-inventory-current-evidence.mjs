...
const inventoryGateRows = [
  ...
  /\| Required aggregate synthetic gate \| pass \| `node scripts\/verification_status\.mjs` \| Current worktree returns `ok=true`: required `1\/1` passes; informational `250\/309` pass and `59` non-blocking readiness\/status checks fail; total passed is `251\/310`\. \| Keep the required docs\/runtime aggregate gate authoritative; track informational readiness gaps separately and do not report them as passed\. \|/,
  ...
  /\| Completion gate inventory current evidence \| pass \| `node scripts\/smoke-completion-gate-inventory-current-evidence\.mjs`, `node scripts\/verification_status\.mjs` \| Current worktree inventory is pinned to aggregate required `1\/1`, informational `250\/309` pass with `59` non-blocking failures, total `251\/310` pass, UI QA `93\/93`, zero unchecked task lines, and AI Company implementation through DEC-224\. \| Keep this smoke in aggregate so required versus informational results do not drift behind README, UI QA, or growth routing evidence\. \|/,
