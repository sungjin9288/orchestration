...
const inventoryGateRows = [
  ...
  /\| UI QA synthetic gate \| pass \| `node scripts\/ui_qa_status\.mjs` \| `ok=true`; required `33\/33`; snapshot reachability informational skipped when the local UI server is not running \| Treat snapshot reachability as optional unless a UI server is intentionally started\. \|/,
  /\| Completion gate inventory current evidence \| pass \| `node scripts\/smoke-completion-gate-inventory-current-evidence\.mjs`, `node scripts\/verification_status\.mjs` \| Current-head inventory evidence is pinned to aggregate registration, UI QA `33\/33`, zero-open backlog, post-completion router, proposal-record lifecycle review alias boundaries, proposal generation evidence, AI Company Phase 5 evidence, Phase 6 planning plus implementation evidence, and Phase 7 planning-only evidence \| Keep this smoke in aggregate so gate inventory counts do not drift behind README, UI QA, or growth routing evidence\. \|/,
  ...
  /\| AI Company checkpoint resume and recovery planning \| pass \| `docs\/64_ai-company-checkpoint-resume-recovery-plan\.md`, `docs\/65_ai-company-checkpoint-resume-recovery-implementation-decision-handoff\.md`, `node scripts\/smoke-ai-company-checkpoint-resume-recovery-planning\.mjs` \|/,
];
