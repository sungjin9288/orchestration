...
const inventoryGateRows = [
  ...
  /\| Required aggregate synthetic gate \| pass \| `node scripts\/verification_status\.mjs` \| Current worktree returns `ok=true`: required `1\/1` passes; informational `251\/310` pass and `59` non-blocking readiness\/status checks fail; total passed is `252\/311`\. \| Keep the required docs\/runtime aggregate gate authoritative; track informational readiness gaps separately and do not report them as passed\. \|/,
  /\| UI QA synthetic gate \| pass \| `node scripts\/ui_qa_status\.mjs` \| Current-worktree required `93\/93` pass; the optional port-4315 snapshot check is informationally skipped\. \| Keep snapshot reachability optional unless that UI server is intentionally started\. \|/,
  /\| Completion gate inventory current evidence \| pass \| `node scripts\/smoke-completion-gate-inventory-current-evidence\.mjs`, `node scripts\/verification_status\.mjs` \| Current worktree inventory is pinned to aggregate required `1\/1`, informational `251\/310` pass with `59` non-blocking failures, total `252\/311` pass, UI QA `93\/93`, zero unchecked task lines, AI Company implementation through DEC-224, and Stage 7A planning through DEC-226\. \| Keep this smoke in aggregate so required versus informational results do not drift behind README, UI QA, or growth routing evidence\. \|/,
  ...
  `${JSON.stringify(
    {
      ...
      aggregate: {
        required: '1/1',
        informational: '251/310',
        informationalFailed: 59,
        total: '252/311',
      },
    },
    null,
