...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-339');
const port = 4640;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-339 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-339-'));
  const inputPath = path.join(tempDir, 'hidden-run-context.txt');
  const outputPath = path.join(tempDir, 'hidden-run-context.md');

  fs.writeFileSync(inputPath, 'Execution hidden run context smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHiddenRunContext: {
