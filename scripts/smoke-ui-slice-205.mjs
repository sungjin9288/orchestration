...
assert.match(appJs, /런타임 상태 요약 연결부터 복구해야 아래 화면이 다시 열립니다\./);
...
assert.doesNotMatch(appJs, /runtime snapshot 연결부터 복구해야 아래 화면이 다시 열립니다\./);
assert.doesNotMatch(appJs, /서버 snapshot 상태를 그대로 따릅니다\./);
  ...
  JSON.stringify(
    {
      ...
      surfaceProvenanceHelperCopy: {
        markers: ['아래 화면', '근거', '현재 화면', '서버 상태 요약'],
      },
    },
    null,
    2,
