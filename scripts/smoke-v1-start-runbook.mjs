...
assert.match(runbook, /Dogfood Run 025 retained linked worktree cleanup is pending explicit operator approval/);
...
assert.match(runbook, /inspect the Dogfood Run 025 evidence/);
...
assert.match(runbook, /Dogfood Run 025 linked worktree remains at `\/Users\/sungjin\/dev\/personal\/orchestration--v1-dogfood-runner-022`/);
assert.match(runbook, /Dogfood Run 025 runtime evidence remains available under `var\/runtime-v1-dogfood-runner-v1-dogfood-runner-022`/);
...
assert.doesNotMatch(runbook, /No retained dogfood linked worktree remains/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 025 evidence is recorded/);
...
assert.match(handoff, /approve a local evidence commit for Dogfood Run 025 retained evidence/);
assert.match(handoff, /approve destructive cleanup for the Dogfood Run 025 retained linked worktree and branch/);
  ...
  JSON.stringify(
    {
      ...
      v1StartRunbook: {
        ...
        nextPriorityPinned: 'approval-gated operator choices while Dogfood Run 025 retained evidence is pending cleanup',
      },
    },
    null,
    2,
