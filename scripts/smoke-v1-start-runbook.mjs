...
assert.match(runbook, /inspect the clean\/published kickoff evidence after the final cleanup evidence is published/);
...
assert.match(runbook, /No dogfood linked worktree remains retained after Dogfood Run 012 cleanup/);
...
assert.match(handoff, /node scripts\/v1-local-completion-status\.mjs` reports `localDevelopmentComplete=false` while Run 012 evidence and cleanup-completed docs remain uncommitted/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 012 evidence is recorded/);
assert.match(handoff, /approve a local evidence commit for Dogfood Run 012 execute plus cleanup evidence/);
