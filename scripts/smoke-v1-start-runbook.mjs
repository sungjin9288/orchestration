...
assert.match(runbook, /Recorded at `2026-05-01 18:14:39 \+0900` on published `main`/);
assert.match(runbook, /dogfood triage status: `Dogfood Run 001` through `Dogfood Run 014` recorded/);
assert.match(runbook, /node scripts\/v1-dogfood-linked-worktree-runner\.mjs --execute --slug v1-dogfood-runner-011/);
assert.match(runbook, /178f4de0c46ce43a6f15fd82aea1b884c71f8233/);
assert.match(runbook, /\/Users\/sungjin\/dev\/personal\/orchestration--v1-dogfood-runner-011/);
assert.match(runbook, /retained linked worktree status: cleaned up after explicit destructive approval/);
assert.match(runbook, /Dogfood Run 014 retained linked worktree cleanup has completed/);
...
assert.match(runbook, /inspect the clean\/published kickoff and dogfood evidence after the final cleanup evidence is published/);
...
assert.match(runbook, /No dogfood linked worktree remains retained after Dogfood Run 014 cleanup/);
...
assert.match(handoff, /current local v1 development baseline is complete on `main` after the explicitly approved Dogfood Run 014 execute and cleanup loop/);
...
assert.match(handoff, /push had completed before the Dogfood Run 014 execute pass; Run 014 cleanup-completed evidence docs are ready for publish after commit approval/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 014 evidence is recorded/);
assert.match(handoff, /approve a local evidence commit for Dogfood Run 014 cleanup-completed evidence/);
