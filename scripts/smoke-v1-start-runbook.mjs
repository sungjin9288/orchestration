...
assert.match(runbook, /inspect the Dogfood Run 024 evidence/);
...
assert.match(runbook, /publish only if a future local commit makes `main` ahead of `origin\/main`/);
assert.match(runbook, /no publish action is currently pending on the clean\/published baseline/);
...
assert.match(handoff, /current local v1 development baseline is clean\/published on `main` after the explicitly approved Dogfood Run 024 execute and cleanup loop plus the V1 kickoff browser polling hardening push/);
assert.match(handoff, /Dogfood Run 024 has now been explicitly approved, executed as linked-worktree evidence, cleaned up after explicit destructive approval, and published as part of the current baseline/);
...
assert.match(handoff, /push had completed before the Dogfood Run 024 execute pass; Run 024 retained-evidence docs, cleanup-completed evidence, and the V1 kickoff browser polling hardening fix are now published on current `main`/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 024 evidence is recorded/);
...
assert.match(handoff, /approve publishing only if a future local commit makes `main` ahead of `origin\/main`/);
assert.match(handoff, /no publish action is currently pending on the clean\/published baseline/);
