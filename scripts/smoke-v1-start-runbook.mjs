...
assert.match(runbook, /Recorded at `2026-05-03 15:48:13 \+0900` on published `main`/);
assert.match(runbook, /dogfood triage status: `Dogfood Run 001` through `Dogfood Run 020` recorded/);
assert.match(runbook, /node scripts\/v1-dogfood-linked-worktree-runner\.mjs --execute --slug v1-dogfood-runner-017/);
assert.match(runbook, /fafd653385c7f1d99884326530d8759a360a1cb4/);
assert.match(runbook, /\/Users\/sungjin\/dev\/personal\/orchestration--v1-dogfood-runner-017/);
assert.match(runbook, /retained linked worktree status: cleaned up after explicit destructive approval/);
assert.match(runbook, /Dogfood Run 020 retained linked worktree cleanup has completed/);
...
assert.match(runbook, /inspect the cleanup-completed Dogfood Run 020 evidence/);
...
assert.match(runbook, /previous baseline push was complete before Dogfood Run 020 execute/);
...
assert.match(runbook, /No dogfood linked worktree remains retained after Dogfood Run 020 cleanup/);
assert.match(runbook, /commit Dogfood Run 020 cleanup-completed evidence locally/);
...
assert.match(handoff, /current local v1 development baseline is complete on `main` after the explicitly approved Dogfood Run 020 execute and cleanup loop/);
...
assert.match(handoff, /push had completed before the Dogfood Run 020 execute pass; Run 020 cleanup-completed docs are ready for local evidence commit and later publish approval/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 020 evidence is recorded/);
assert.match(handoff, /approve a local evidence commit for Dogfood Run 020 cleanup-completed evidence/);
