...
assert.match(runbook, /Dogfood Run 013 retained linked worktree cleanup is pending explicit operator approval/);
...
assert.match(runbook, /inspect the latest retained Dogfood Run 013 evidence/);
...
assert.match(runbook, /Dogfood Run 002, Run 004, Run 005, Run 006, Run 007, Run 008, Run 009, Run 010, Run 011, and Run 012 retained dogfood linked worktree cleanup is complete/);
assert.match(runbook, /remove the retained Dogfood Run 013 linked worktree and branch/);
assert.match(runbook, /Dogfood Run 013 linked worktree remains retained dirty by design/);
...
assert.match(handoff, /current local v1 development baseline was clean and published before the explicitly approved Dogfood Run 013 execute pass/);
assert.match(handoff, /node scripts\/v1-local-completion-status\.mjs` reports `localDevelopmentComplete=false` while Dogfood Run 013 retained linked worktree cleanup remains approval-blocked and evidence docs remain local/);
assert.match(handoff, /The next action is no longer an implementation backlog item by default/);
assert.match(handoff, /push had completed before the Dogfood Run 013 execute pass; Run 013 retained-evidence docs are local until commit approval/);
...
assert.match(handoff, /Dogfood Run 001 through Dogfood Run 013 evidence is recorded locally/);
assert.match(handoff, /Dogfood Run 013 retained linked worktree cleanup is pending explicit operator approval/);
assert.match(handoff, /approve a local evidence commit for Dogfood Run 013 retained evidence/);
assert.match(handoff, /approve destructive cleanup for the retained Dogfood Run 013 linked worktree and branch/);
