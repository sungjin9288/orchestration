...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-326');
const port = 4627;
...
assert.match(appJs, /data-harness-result-hidden-rerun="true"/);
...
assert.match(appJs, /canRenderHiddenHarnessInputPathActions\s+\?\s+`\s+<button[\s\S]*?data-action="rerun-harness-execution-paths"/);
assert.match(
  appJs,
...
assert.doesNotMatch(appJs, /\$\{\s*canRenderHiddenHarnessInputPathActions\s+\?\s+`\s+<button[\s\S]*?data-action="rerun-harness-execution-paths"/);
assert.doesNotMatch(appJs, /\$\{\s*hiddenHarnessInputPath\s+\?\s+`\s+<button[\s\S]*?data-action="rerun-harness-execution-paths"/);
assert.doesNotMatch(
  appJs,
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-326 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-326-'));
  const inputPath = path.join(tempDir, 'hidden-rerun.txt');
  const outputPath = path.join(tempDir, 'hidden-rerun.md');

  fs.writeFileSync(inputPath, 'Execution hidden rerun smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHiddenRerun: {
            insertionPoint: 'hiddenExecutionResultRegister->rerunExecutionPathsAction->runHarnessOperatorActionRoute',
            sourceMarker: 'data-harness-result-hidden-rerun',
            namedValues: [
