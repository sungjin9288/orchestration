...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-320 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-320-'));
  const inputPath = path.join(tempDir, 'result-show.txt');
  const outputPath = path.join(tempDir, 'result-show.md');

  fs.writeFileSync(inputPath, 'Execution result show-again smoke\n', 'utf8');
