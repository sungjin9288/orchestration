...
const ACCEPTANCE_CRITERIA = [
  {
    ...
    question:
      'Is the dry-run finalization review acceptance finalization review acceptance finalization review status green and ready for acceptance?',
    requiredEvidenceRef: 'reviewEnvelope.compatibility.reviewState',
  },
  {
    id: 'review-findings-passed',
    question:
      'Did every dry-run finalization review acceptance finalization review acceptance finalization review finding pass?',
    requiredEvidenceRef: 'reviewEnvelope.reviewFindings',
  },
  {
    id: 'review-remains-non-approval',
    question:
      'Does acceptance keep reviewed finalization review acceptance finalization review acceptance finalization review evidence separate from proposal approval?',
...
function summarizeSources(sources) {
  ...
  return {
    ...
    lessonCaptured:
      /proposal record dry-run review acceptance finalization review acceptance finalization review acceptance finalization review acceptance.*proposal approval/i.test(
        lessons,
      ) ||
      /proposal approval.*proposal record dry-run review acceptance finalization review acceptance finalization review acceptance finalization review acceptance/i.test(
        lessons,
      ),
    engineRoutesPastAcceptance:
      /nextRecommendedSlice[\s\S]*growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-finalization/.test(
        engine,
      ),
  ...
  return [
    ...
    },
    {
      ...
      ok:
        ...
        safetyBoundary.doesNotPromoteFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewToRecord ===
          true,
      evidenceRef: 'readiness and safetyBoundary',
      observed: {
...
function buildAcceptanceEnvelope({ reviewPayload }) {
  ...
  return {
    ...
    acceptanceEnvelope: {
      acceptanceId:
        'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-finalization-review-acceptance-candidate',
      sourceReviewId: reviewEnvelope.reviewId || null,
      targetQueueContract: 'growth-proposal-queue-status',
      targetSchema: 'proposalRecord',
      acceptancePurpose:
        'Accept reviewed dry-run finalization review acceptance finalization review acceptance finalization review evidence only for the next read-only finalization check while keeping proposal approval, record creation, persistence, queue mutation, and implementation authority blocked.',
      acceptanceInputs: {
      ...
      blockedActions,
      finalizationQuestion:
        'Can this accepted dry-run finalization review acceptance finalization review acceptance finalization review evidence move to a read-only finalization check without approving, creating, persisting, or mutating proposal records?',
      nonApprovalStatement:
...
const payload = {
  ...
  sourceSummary,
  inputStatuses: {
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalizationReview: {
  ...
  failures: {
    missingSources,
    proposalRecordDryRunReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewAcceptanceFinalizationReviewFailed:
      !reviewResult.ok,
