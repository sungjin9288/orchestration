...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-306');
const port = 4606;
...
assert.match(serveUi, /url\.pathname === '\/api\/harness\/operator-action\/run'/);
assert.match(serveUi, /kind: 'run-harness-operator-action'/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-306 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-306-'));
  ...
  fs.writeFileSync(
    inputPath,
    [
      'Execution launch smoke',
      '',
      'This fixture proves the explicit execution route runs the repo-native wrapper.',
      '',
      '- frozen operator action',
      '- local-only UI mutation path',
    ].join('\n'),
    'utf8',
  );
  ...
  try {
    ...
    assert.equal(runPayload.mutation?.kind, 'run-harness-operator-action');
    assert.equal(runPayload.mutation?.harnessId, 'markitdown');
    ...
    assert.equal(runPayload.harnessExecution?.outputExists, true);
    assert.equal(fs.existsSync(outputPath), true);

    const output = fs.readFileSync(outputPath, 'utf8');
    assert.match(output, /Execution launch smoke/i);
