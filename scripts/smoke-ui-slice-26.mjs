...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-26');
const port = 4336;
...
function createGitFixtureRepo(label) {
  const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), `orchestration-ui-slice-26-${label}-`));

  runGit(projectPath, ['init', '-q']);
  runGit(projectPath, ['config', 'user.name', 'ui-slice-26']);
  runGit(projectPath, ['config', 'user.email', 'ui-slice-26@example.com']);
...
## Input Summary
- synthetic preflight fixture for ui-slice-26 smoke
`;
}
...
## Verification Notes
- synthetic live mutation bundle for ui-slice-26 smoke
`;
}
...
## Slice Goal
Prepare a local commit bundle that can be resumed from the primary execution surface.
`,
  });
...
`,
  ...
  const approval = runtime.createApprovalPlaceholder({
    ...
    prompt: `Approve the synthetic bundle ${label} for ui-slice-26 smoke.`,
  });
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-26 server');
}

async function main() {
  const runtime = createRuntimeService({ runtimeRoot });
  runtime.resetRuntime();
  const fixtureProjectPath = createGitFixtureRepo('local-commit');
  ...
  try {
    ...
    const missionPayload = await postJson('/api/missions', {
      ...
      constraints:
        'Keep local commit execution bounded to the latest approved commit package only.',
      goal:
        'Verify that the primary execution surface can resume a bounded local commit after the current commit gate is approved.',
      title: 'Primary local commit CTA smoke',
    });
    ...
    const label = 'primary-local-commit';

    fs.writeFileSync(path.join(fixtureProjectPath, 'scoped.txt'), `scoped change ${label}\n`, 'utf8');
    ...
    await postJson(`/api/decision-inbox/${encodeURIComponent(commitInboxItemId)}/actions`, {
      verb: 'approve',
    });

    const readinessBefore = coordinator.getCommitExecutionReadiness({ taskId });
    assert.equal(readinessBefore.allowed, true);
    assert.equal(readinessBefore.latestApprovalStatus, 'approved');
    ...
    const commitResultArtifact = localCommitPayload.artifactDetail;
    const readinessAfter = localCommitPayload.derived.commitExecutionReadinessSummaries[taskId];

    assert.equal(localCommitPayload.mutation.kind, 'run-local-commit');
    assert.equal(commitResultArtifact.type, 'commit-result');
    assert.equal(readinessAfter.allowed, false);
    assert.equal(readinessAfter.conflict, true);
    assert.equal(readinessAfter.existingCommitResultArtifactId, commitResultArtifact.id);
      ...
      JSON.stringify(
        {
          ...
          localCommit: {
            ...
            commitSha: localCommitPayload.mutation.commitSha,
          },
        },
        null,
        2,
