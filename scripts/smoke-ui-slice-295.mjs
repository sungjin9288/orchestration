...
assert.match(appJs, /function getHarnessBriefSignalValue\(brief\)/);
assert.match(appJs, /label: '하네스'/);
assert.match(appJs, /return `\$\{brief\.primaryHarnessId\} · \$\{brief\.actionLabel \|\| 'No action'\}`;/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-295 server');
}
...
async function main() {
  ...
  try {
    ...
    const harnessBrief = snapshotPayload.derived?.harnessConsumerBrief;

    assert.equal(harnessBrief?.ok, true);
    assert.equal(harnessBrief?.mode, 'harness-consumer-brief');
    assert.equal(harnessBrief?.brief?.currentHostState, 'runnable');
    assert.equal(harnessBrief?.brief?.primaryHarnessId, 'markitdown');
      ...
      JSON.stringify(
        {
          ...
          harnessUiBridge: {
            ...
            actionCommand: harnessBrief.brief.actionCommand,
            actionLabel: harnessBrief.brief.actionLabel,
            primaryHarnessId: harnessBrief.brief.primaryHarnessId,
          },
        },
        null,
        2,
