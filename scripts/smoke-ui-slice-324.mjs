...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-324');
const port = 4625;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-324 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-324-'));
  const inputPath = path.join(tempDir, 'hidden-input-copy.txt');
  const outputPath = path.join(tempDir, 'hidden-input-copy.md');

  fs.writeFileSync(inputPath, 'Execution hidden input copy smoke\n', 'utf8');
