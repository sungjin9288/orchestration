...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-remediation-source-mutation-apply-result-review-status',
});
...
const SOURCE_MUTATION_APPLY_RESULT_REVIEW_STATES = [
  'source-mutation-apply-result-review-not-started',
  'source-mutation-apply-result-review-ready',
  'source-mutation-apply-result-review-blocked',
  'source-mutation-apply-result-review-ready-for-result-acceptance-contract',
  'source-mutation-apply-result-review-rejected',
  'source-mutation-apply-result-review-deferred',
  ...
  'stale-apply-result-review-blocked',
  'closed-no-action',
];

const SOURCE_MUTATION_APPLY_RESULT_REVIEW_DECISION_TYPES = [
  ...
  'reject-source-mutation-apply-result-review',
  'defer-source-mutation-apply-result-review',
  'hold-baseline',
];

const SOURCE_MUTATION_APPLY_RESULT_REVIEW_EVIDENCE_TYPES = [
...
const SOURCE_MUTATION_APPLY_RESULT_REVIEW_BLOCKER_TYPES = [
...
function summarizeSources(sources) {
  ...
  const sourceMutationApplyResult = sourceText(
    sources,
    'scripts/growth-remediation-source-mutation-apply-result-status.mjs',
  );
  ...
  return {
    ...
    sourceMutationApplyResultDocumented:
    ...
    sourceMutationApplyResultImplemented:
      /mode: 'growth-remediation-source-mutation-apply-result-status'/.test(
        sourceMutationApplyResult,
      ),
    ...
    harnessMentionsSourceMutationApplyResultReview:
      /growth-remediation-source-mutation-apply-result-review-status/.test(harnessBaseline),
    completionReadinessMentionsSourceMutationApplyResultReview:
      /growth-remediation-source-mutation-apply-result-review-status/.test(completionReadiness),
    ledgerMentionsSourceMutationApplyResultReview:
      /growth-remediation-source-mutation-apply-result-review-status-readonly-post-m7-837/.test(
        todo,
      ),
    verificationIncludesSourceMutationApplyResultReview:
      /growth-remediation-source-mutation-apply-result-review-status/.test(verificationStatus),
    ...
    applyResultReviewSeparateFromMutation:
      /apply result review status stays separate from actually applying patches/.test(plan),
    resultAcceptanceStillBlocked:
      /before\s+source mutation apply result acceptance can be considered/.test(plan),
...
const sourceMutationApplyResultReviewSchema = {
  sourceMutationApplyResultReviewRecord: fields(
    [
      'applyResultReviewId',
      'applyResultId',
      'applyExecutionId',
  ...
  sourceMutationApplyResultReviewDecision: fields(
    [
      'decisionId',
      'applyResultReviewId',
      'applyResultId',
  ...
  sourceMutationApplyResultReviewBlocker: fields(
    [
      'blockerId',
      'applyResultReviewId',
      'applyResultId',
  ...
  sourceMutationApplyResultReviewIndex: fields(
    [
      'indexId',
...
const sourceMutationApplyResultReviewRules = [
  {
    id: 'apply-result-review-requires-current-apply-result',
    rule: 'source mutation apply result review can only bind to one current apply result record and its apply execution chain',
  },
  {
    id: 'apply-result-review-is-not-source-mutation',
    rule: 'source mutation apply result review status may mark result-acceptance-contract readiness, but it cannot accept results, apply patches, mutate source, apply proposals, approve authorizations, or execute remediation',
  },
  {
    id: 'apply-result-review-requires-clean-baseline-and-operator-dispatch-intent',
    rule: 'clean baseline proof, exact scope lock, target lock, baseline digest, operator dispatch intent, reviewer notes, and acceptance criteria must be present before source mutation apply result acceptance can be considered',
  },
  {
    id: 'apply-result-review-requires-verification-dry-run-and-rollback-proof',
    rule: 'patch draft refs, diff preview refs, verification output refs, dry-run proof refs, rollback proof refs, source-of-truth refs, task ledger refs, and negative evidence clearance must be present before source mutation apply result acceptance can be considered',
  },
  {
    id: 'failed-stale-broad-dirty-or-mutating-result-review-blocks-result-acceptance',
    rule: 'stale result, stale execution, dirty or untracked baseline, changed patch/diff proof, failed verification, missing rollback proof, missing operator dispatch intent, missing reviewer note, missing acceptance criteria, unresolved negative evidence, unapproved file touches, or any attempted source mutation blocks result acceptance readiness',
  },
];
...
const requiredFieldsSatisfied = Object.values(sourceMutationApplyResultReviewSchema).every(
  (schema) => schema.required.length > 0,
);
const ok =
  missingSources.length === 0 &&
  sourceSummary.growthGatewayPlanPresent &&
  sourceSummary.sourceMutationApplyResultDocumented &&
  sourceSummary.sourceMutationApplyResultReviewDocumented &&
  sourceSummary.sourceMutationApplyResultImplemented &&
  ...
  sourceSummary.harnessMentionsSourceMutationApplyResultReview &&
  sourceSummary.completionReadinessMentionsSourceMutationApplyResultReview &&
  sourceSummary.ledgerMentionsSourceMutationApplyResultReview &&
  sourceSummary.verificationIncludesSourceMutationApplyResultReview &&
  sourceSummary.sourceMutationApplyResultAcceptanceNextDocumented &&
  ...
  sourceSummary.applyResultReviewSeparateFromMutation &&
  sourceSummary.resultAcceptanceStillBlocked &&
...
const payload = {
  ok,
  mode: 'growth-remediation-source-mutation-apply-result-review-status',
  posture: 'local-read-only-remediation-source-mutation-apply-result-review-contract',
  currentHead: {
  ...
  schemaVersion: 'growth-remediation-source-mutation-apply-result-review-status/v0',
  sourceSummary,
  vocabulary: {
    sourceMutationApplyResultReviewStates: SOURCE_MUTATION_APPLY_RESULT_REVIEW_STATES,
    sourceMutationApplyResultReviewDecisionTypes:
      SOURCE_MUTATION_APPLY_RESULT_REVIEW_DECISION_TYPES,
    sourceMutationApplyResultReviewEvidenceTypes:
      SOURCE_MUTATION_APPLY_RESULT_REVIEW_EVIDENCE_TYPES,
    sourceMutationApplyResultReviewBlockerTypes:
      SOURCE_MUTATION_APPLY_RESULT_REVIEW_BLOCKER_TYPES,
  },
  sourceMutationApplyResultReviewSchema,
  sourceMutationApplyResultReviewRules,
  sourceMutationApplyResultReviewState: {
    realSourceMutationApplyResultReviewFileAdopted: false,
    discoveredSourceMutationApplyResultReviewRecords: 0,
    resultAcceptanceAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    remediationExecutionAllowed: false,
    currentStatus: 'contract-only-no-active-source-mutation-apply-result-review',
  },
  readiness: {
    requiredFieldsSatisfied,
    sourceMutationApplyResultReviewRecordTypes: Object.keys(
      sourceMutationApplyResultReviewSchema,
    ).length,
    ...
    applyResultReviewAndMutationSeparate: true,
    resultAcceptanceAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    ...
    readyForSourceMutationApplyResultAcceptanceStatus: true,
  },
  nextRecommendedSlice: {
    id: 'growth-remediation-source-mutation-apply-result-acceptance-status',
    commandToAdd: 'node scripts/growth-remediation-source-mutation-apply-result-acceptance-status.mjs',
    reason:
      'Source mutation apply result review is now modeled as read-only; the next safe slice should define result-acceptance records without applying patches or mutating source.',
