...
## Slice Goal
Verify qa-slice-04 planner and architect live verification through browser plus API assertions.
...
- planner and architect execute live
- architect artifacts stay inspectable
...
function assertRoleReadiness(roleReadiness) {
  ...
  assert.equal(roleReadiness.taskBreaker.readiness, 'degraded');
  assert.equal(roleReadiness.taskBreaker.allowed, false);
  assert.equal(roleReadiness.builderPreflight.readiness, 'degraded');
  assert.equal(roleReadiness.builderPreflight.allowed, false);
...
async function verifyBrowserProjectSummary({
  ...
  assert.match(providerReadySnapshot, /planner and architect/i);
...
async function verifyBrowserHumanGateFlow({
  ...
  assert.match(artifactsText, /Blocking Architecture Issues/i);
...
export async function runQaSlice04SyntheticSmoke(options = {}) {
  ...
  try {
    ...
    const fitTaskPayload = await postJson(harness.baseUrl, '/api/tasks', {
      intent: 'Verify synthetic fit architect live flow for qa-slice-04.',
      title: 'QA slice 04 architect fit',
    });
    const gateTaskPayload = await postJson(harness.baseUrl, '/api/tasks', {
      intent: 'Verify synthetic human-gate architect live flow for qa-slice-04.',
      title: 'QA slice 04 architect human gate',
    });
    ...
    const fitPlannerResult = await syntheticCoordinator.runPlanner({
      ...
      routingOutcome: createRoutingOutcome('Verify synthetic fit architect live flow for qa-slice-04.'),
    });
    const fitAnchor = {
      projectId,
      taskId: fitTaskPayload.task.id,
    ...
    const gatePlannerResult = await syntheticCoordinator.runPlanner({
      ...
      routingOutcome: createRoutingOutcome(
        'Verify synthetic human-gate architect live flow for qa-slice-04.',
      ),
    });
    const gateAnchor = {
      projectId,
    ...
    queuedFetch.push(
      createArchitectApiPayload(gateAnchor, {
        ...
        artifact: {
          ...
          policyImpact: ['Decision Inbox remains a surface-only browser verification target.'],
          decisionLogImpact: ['None. The slice closes QA coverage only.'],
          approvedAssumptions: ['Role readiness is asserted through the coordinator.'],
          noArchitectureChangeStatement:
            'No architecture change is approved by this note. Human resolution is required first.',
          blockingArchitectureIssues: ['The plan needs human gate resolution before task-breaker may proceed.'],
        },
        normalizedResult: {
          blockers: ['The plan needs human gate resolution before task-breaker may proceed.'],
          needsDecision: true,
          nextStage: 'human gate',
          summary: 'Architect found a blocking issue that routes through the human gate.',
          decisionTitle: 'Architecture decision required',
          decisionPrompt: 'Resolve the blocked architecture issue before downstream work may continue.',
        },
      }),
    );
    ...
    assert.equal(gateArchitectResult.run.summary.nextStage, 'human gate');
    assert.ok(gateArchitectResult.decisionInboxItem);

    const snapshotPayload = await waitForSnapshotPayload(
      harness.baseUrl,
      'synthetic qa-slice-04 snapshot',
      (payload) =>
        countArtifacts(payload.snapshot, fitTaskPayload.task.id, 'architecture') === 1 &&
        countArtifacts(payload.snapshot, gateTaskPayload.task.id, 'architecture') === 1 &&
    ...
    const gateDecisionItem =
      Object.values(snapshotPayload.snapshot.decisionInboxItems).find(
        (item) =>
          ...
          item.status === 'pending' &&
          item.title === gateArchitectResult.decisionInboxItem.title,
      ) || null;
    ...
    const fitArtifactPayload = await fetchArtifactPayload(harness.baseUrl, fitArchitectResult.artifact.id);
    const gateArtifactPayload = await fetchArtifactPayload(harness.baseUrl, gateArchitectResult.artifact.id);
    ...
    assert.match(fitArtifactPayload.artifact.content, /## Affected Components or Contracts/);
    assert.match(gateArtifactPayload.artifact.content, /## Blocking Architecture Issues/);
    ...
    const browserGateTexts = await verifyBrowserHumanGateFlow({
      architectRunId: gateArchitectResult.run.id,
      architectureArtifactId: gateArchitectResult.artifact.id,
      decisionTitle: gateArchitectResult.decisionInboxItem.title,
      outputRoot,
      overrideEnvVar,
    ...
    return {
      ...
      scenarios: {
        fit: {
          ...
          nextStage: fitArchitectResult.run.summary.nextStage,
          planArtifactId: fitPlannerResult.artifact.id,
          plannerRunId: fitPlannerResult.run.id,
        },
        humanGate: {
          architectureArtifactId: gateArchitectResult.artifact.id,
          decisionInboxItemId: gateArchitectResult.decisionInboxItem.id,
          nextStage: gateArchitectResult.run.summary.nextStage,
          planArtifactId: gatePlannerResult.artifact.id,
...
export async function runQaSlice04RealSmoke(options = {}) {
  ...
  try {
    ...
    const taskPayload = await postJson(harness.baseUrl, '/api/tasks', {
      intent: 'Run planner then architect through the optional real qa-slice-04 live smoke.',
    ...
    const plannerResult = await coordinator.runPlanner({
      ...
      routingOutcome: createRoutingOutcome(
        'Validate the optional real planner then architect live path for qa-slice-04.',
      ),
    });
    ...
    const artifactPayload = await fetchArtifactPayload(harness.baseUrl, architectResult.artifact.id);
    ...
    assert.match(artifactPayload.artifact.content, /## Affected Components or Contracts/);
