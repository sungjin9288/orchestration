...
const artifactStructuredRenderSource = fs.readFileSync(
  path.join(repoRoot, 'ui', 'artifact-structured-render.js'),
  'utf8',
);
...
assert.match(appJsSource, /종료 가드/);
assert.match(
  appJsSource,
  /탐색 전용으로 남습니다\. 승인된 종료 정리 이어가기는 태스크 상세에서 실행합니다\./,
);
