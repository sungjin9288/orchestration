...
const worktreeLabelsPath = path.join(repoRoot, 'ui', 'worktree-labels.js');
const appJs = fs.readFileSync(appPath, 'utf8');
const worktreeLabels = fs.readFileSync(worktreeLabelsPath, 'utf8');
...
assert.match(worktreeLabels, /parts\.push\('현재 프로젝트 경로'\)/);
assert.match(worktreeLabels, /return option\?\.branch \|\| pathParts\[pathParts\.length - 1\] \|\| '연결-워크트리';/);
...
assert.doesNotMatch(worktreeLabels, /parts\.push\('현재 project_path'\)/);
