...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-301');
const port = 4601;
const baseUrl = `http://127.0.0.1:${port}`;

const appJs = fs.readFileSync(appPath, 'utf8');

assert.match(appJs, /function renderExecution\(data\) \{/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-301 server');
}
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionRegister: {
            derivedKey: 'harnessConsumerBrief',
            insertionPoint: 'executionCommandDeck->harnessRegister->surfaceGrid',
