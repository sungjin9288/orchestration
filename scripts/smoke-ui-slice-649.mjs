...
assert.match(growthPanels, /function renderGrowthCandidateDrilldown\(growth\)/);
assert.match(growthPanels, /data-growth-candidate-drilldown="true"/);
assert.match(growthPanels, /function renderGrowthDashboardEvidenceDepth\(growth\)/);
assert.match(growthPanels, /data-growth-dashboard-evidence-depth="read-only"/);
assert.match(growthPanels, /data-failure-pattern-groups="true"/);
assert.match(growthPanels, /data-regression-comparison="read-only"/);
assert.match(growthPanels, /data-rollback-evidence-links="true"/);
assert.match(growthPanels, /data-growth-dashboard-action-allowed="false"/);
assert.match(growthPanels, /실패 묶음, 회귀 비교, 되돌림 근거를 함께 봅니다/);
assert.match(growthPanels, /묶인 실패 패턴/);
assert.match(growthPanels, /회귀 비교/);
assert.match(growthPanels, /되돌림 근거 링크/);
assert.match(growthPanels, /function renderGrowthProposalReviewPreview\(growth\)/);
assert.match(growthPanels, /data-growth-proposal-review="blocked"/);
assert.match(growthPanels, /data-proposal-generation-allowed="\$\{GROWTH_AUTHORITY_BOUNDARY\.proposalGenerationAllowed\}"/);
assert.match(growthPanels, /data-proposal-application-allowed="\$\{GROWTH_AUTHORITY_BOUNDARY\.proposalApplicationAllowed\}"/);
...
assert.match(growthPanels, /data-long-term-memory-store-allowed="\$\{GROWTH_AUTHORITY_BOUNDARY\.longTermMemoryStoreAllowed\}"/);
...
assert.match(growthPanels, /리뷰 질문/);
assert.match(growthPanels, /제안 검토 게이트/);
assert.match(growthPanels, /승인 전 적용 차단/);
