...
async function main() {
  ...
  try {
    ...
    assert.equal(populatedMigrated.schemaVersion, 11);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v12', { ...createEmptyState(), schemaVersion: 12 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
    ...
    assert.equal(persistedState.schemaVersion, 11);
