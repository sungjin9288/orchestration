...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 30,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 30);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v31', { ...createEmptyState(), schemaVersion: 31 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
