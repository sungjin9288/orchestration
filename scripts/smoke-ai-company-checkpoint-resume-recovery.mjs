...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 26,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 26);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v27', { ...createEmptyState(), schemaVersion: 27 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
