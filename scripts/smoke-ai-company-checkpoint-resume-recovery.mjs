...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 25,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 25);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v26', { ...createEmptyState(), schemaVersion: 26 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
