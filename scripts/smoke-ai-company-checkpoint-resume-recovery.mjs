...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 22,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 22);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v23', { ...createEmptyState(), schemaVersion: 23 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
