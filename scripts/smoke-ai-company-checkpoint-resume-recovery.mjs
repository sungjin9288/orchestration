...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 29,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 29);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v30', { ...createEmptyState(), schemaVersion: 30 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
