...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 27,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 27);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v28', { ...createEmptyState(), schemaVersion: 28 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
