...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 28,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 28);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v29', { ...createEmptyState(), schemaVersion: 29 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
