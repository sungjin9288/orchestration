...
async function main() {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(fs.readFileSync(path.join(migrationRoot, 'state.json'), 'utf8')).schemaVersion, 21,
    );
    ...
    assert.equal(populatedMigrated.schemaVersion, 21);
    ...
    for (const [name, state, pattern] of [
      ...
      ['future-v22', { ...createEmptyState(), schemaVersion: 22 }, /Unsupported runtime state/],
    ]) {
      const root = path.join(tempRoot, name);
