...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-362');
const port = 4663;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-362 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-362-'));
  const inputPath = path.join(tempDir, 'latest-result-preview-empty-density.txt');
  const outputPath = path.join(tempDir, 'latest-result-preview-empty-density.md');

  fs.writeFileSync(inputPath, 'Latest result preview empty density smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionResultPreviewEmptyDensity: {
            insertionPoint: 'executionResultRegister->visiblePreviewEmptyDensity->compactEmptyCopy',
