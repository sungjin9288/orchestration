...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-193');
...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Decision Inbox action block aligned to the current evidence rail handoff truth.',
  };
...
function renderDecisionActionTokens({
  actionLabel,
  approvalActionLabel,
  createToken,
  evidenceRail,
  ...
  return `
    <div class="token-row token-row-compact">
      ...
      ${createToken(actionLabel, actionLabel === '결정 처리' ? 'warning' : 'accent')}
      ${approvalActionLabel ? createToken(approvalActionLabel, 'neutral') : ''}
    </div>
  `;
}
...
const { createToken, getApprovalActionLabel, getExecutionEvidenceRail } = loadHelpers();

assert.match(
  appJs,
  /<p class="decision-action-copy">이 안건은 여기서 승인 또는 반려만 결정합니다\.<\/p>[\s\S]*createToken\(\s*`현재:\$\{decisionDetailEvidenceState\.currentOwnerLabel\}`,\s*decisionDetailEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{decisionDetailEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*createToken\('승인 요청', 'accent'\)/,
);
assert.match(
  appJs,
  /<p class="decision-action-copy">이 안건은 여기서 해결만 기록하고 다음 실행 판단으로 넘깁니다\.<\/p>[\s\S]*createToken\(\s*`현재:\$\{decisionDetailEvidenceState\.currentOwnerLabel\}`,\s*decisionDetailEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{decisionDetailEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*createToken\('결정 처리', 'warning'\)/,
);
...
try {
  ...
  const pendingProject = runtime.createProject({
    name: 'decision-action-handoff-pending-approval',
    projectPath: repoRoot,
  });
  const pendingTask = runtime.createTask({
    projectId: pendingProject.id,
    title: 'Decision action handoff pending approval',
    intent: 'Keep the Decision Inbox action block aligned while builder approval is pending.',
  });
  ...
  const pendingMarkup = renderDecisionActionTokens({
    actionLabel: '승인 요청',
    approvalActionLabel: getApprovalActionLabel(pendingApprovalRecord.allowedNextAction),
    createToken,
  ...
  assert.match(
    pendingMarkup,
    new RegExp(getApprovalActionLabel(pendingApprovalRecord.allowedNextAction)),
  );

  runtime.resetRuntime();

  const reviewedProject = runtime.createProject({
    name: 'decision-action-handoff-reviewed-follow-up',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Decision action handoff reviewed follow-up',
    intent: 'Keep the Decision Inbox action block aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderDecisionActionTokens({
    actionLabel: reviewedItem.kind === 'decision' ? '결정 처리' : '읽기 전용',
    approvalActionLabel: null,
    createToken,
  ...
  assert.match(reviewedMarkup, /결정 처리/);

  console.log(
    JSON.stringify(
      {
        ok: true,
        decisionInboxActionHandoffTokens: {
          ...
          verifiedCases: [
            'decision action block shows current and next handoff tokens from the shared rail truth',
            'pending builder approval path keeps the action block aligned with the existing approval action semantics',
            'reviewed follow-up decision path keeps the action block aligned with the critic-to-human-gate handoff',
          ],
        },
      },
      null,
