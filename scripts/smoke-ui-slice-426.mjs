...
assert.match(
  stylesCss,
  /\.surface\[data-surface="execution"\] \.relation-strip-hidden-compact > \.form-actions-hidden-compact \[data-harness-result-hidden-preview-copy="true"\]\s*\{[\s\S]*border-color:\s*rgba\(60,\s*95,\s*137,\s*0\.08\);[\s\S]*background:\s*linear-gradient\(180deg,\s*rgba\(245,\s*248,\s*251,\s*0\.94\),\s*rgba\(236,\s*241,\s*246,\s*0\.97\)\);[\s\S]*color:\s*rgba\(67,\s*79,\s*90,\s*0\.82\);/s,
);
