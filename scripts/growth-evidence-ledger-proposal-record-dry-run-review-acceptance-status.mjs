...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-status',
});
...
const ACCEPTANCE_CRITERIA = [
  {
    ...
    question: 'Is the dry-run review status green and ready for acceptance checking?',
    requiredEvidenceRef: 'reviewEnvelope.compatibility.reviewState',
  },
  {
    id: 'review-findings-passed',
    question: 'Did every dry-run review finding pass without evidence gaps?',
    requiredEvidenceRef: 'reviewEnvelope.reviewFindings',
  },
  {
    id: 'review-remains-non-approval',
    question: 'Does acceptance keep review evidence separate from proposal approval?',
...
function summarizeSources(sources) {
  ...
  return {
    ...
    proposalRecordDryRunReviewAcceptanceStatusDocumented:
      /Post-Completion Implemented Slice: `growth-evidence-ledger-proposal-record-dry-run-review-acceptance-status`/.test(
        plan,
      ) && /Growth Evidence Ledger proposal record dry-run review acceptance status/.test(inventory),
    proposalRecordDryRunReviewAcceptanceStatusAggregateRegistered:
      /growth-evidence-ledger-proposal-record-dry-run-review-acceptance-status/.test(
        verificationStatus,
      ),
    proposalRecordDryRunReviewAcceptanceStatusLedgered:
      /growth-evidence-ledger-proposal-record-dry-run-review-acceptance-status-readonly-post-m7/.test(
        todo,
      ),
    proposalRecordDryRunReviewAcceptanceLessonCaptured:
      /proposal record dry-run review acceptance.*proposal approval/i.test(lessons) ||
      /proposal approval.*proposal record dry-run review acceptance/i.test(lessons),
    engineRoutesPastProposalRecordDryRunReviewAcceptance:
      /nextRecommendedSlice[\s\S]*growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization/.test(
        engine,
      ),
    reflectionRoutesPastProposalRecordDryRunReviewAcceptance:
      /nextRecommendedSlice[\s\S]*growth-evidence-ledger-proposal-record-dry-run-review-acceptance-finalization/.test(
        reflection,
      ),
  ...
  return [
    {
      ...
      ok:
        ...
        reviewCompatibility.reviewState === 'review-ready-for-acceptance-check' &&
    ...
    },
    {
      ...
      ok:
        ...
        safetyBoundary.doesNotPromoteReviewToRecord === true,
      evidenceRef: 'readiness and safetyBoundary',
      observed: {
        ...
        doesNotPromoteReviewToRecord: safetyBoundary.doesNotPromoteReviewToRecord,
      },
    },
  ];
}
...
function buildAcceptanceEnvelope({ reviewPayload }) {
  ...
  return {
    ...
    acceptanceEnvelope: {
      acceptanceId:
        'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-candidate',
      sourceReviewId: reviewEnvelope.reviewId || null,
      targetQueueContract: 'growth-proposal-queue-status',
      targetSchema: 'proposalRecord',
      acceptancePurpose:
        'Accept dry-run review evidence for the next read-only finalization check while keeping proposal approval, record creation, persistence, queue mutation, and implementation authority blocked.',
      acceptanceInputs: {
    ...
    compatibility: {
      ...
      durableRecordPromotionBlocked:
        blockedActions.includes('promote-dry-run-review-to-record') &&
...
function buildReadiness({ sourceSummary, reviewResult, acceptanceEnvelope }) {
  ...
  return {
    ...
    docsAndAggregateReady:
      sourceSummary.proposalRecordDryRunReviewAcceptanceStatusDocumented &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceStatusAggregateRegistered &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceStatusLedgered &&
      sourceSummary.proposalRecordDryRunReviewAcceptanceLessonCaptured,
    engineReflectionAdvanced:
      sourceSummary.engineRoutesPastProposalRecordDryRunReviewAcceptance &&
      sourceSummary.reflectionRoutesPastProposalRecordDryRunReviewAcceptance,
...
const payload = {
  ...
  schemaVersion: 'growth-evidence-ledger-proposal-record-dry-run-review-acceptance-status/v0',
  currentHead: {
  ...
  nextRecommendedSlice: {
    ...
    reason:
      'The dry-run review evidence is accepted only for a read-only finalization check; the next safe slice can define finalization without approving, creating, persisting, implementing, or mutating proposal records.',
  ...
  failures: {
    missingSources,
    proposalRecordDryRunReviewFailed: !reviewResult.ok,
