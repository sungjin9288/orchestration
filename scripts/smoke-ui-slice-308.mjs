...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-308 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-308-'));
  ...
  fs.writeFileSync(
    inputPath,
    [
      'Execution result restore smoke',
      '',
      'This fixture proves the latest harness execution survives a snapshot refresh.',
      '',
      '- local-only derived persistence',
      '- execution result restore',
    ].join('\n'),
    'utf8',
  );
  ...
  try {
    ...
    assert.equal(initialSnapshot.derived?.latestHarnessExecution || null, null);
    ...
    assert.match(runPayload.harnessExecution?.outputPreview || '', /Execution result restore smoke/i);
    ...
    assert.match(latestHarnessExecution.outputPreview || '', /Execution result restore smoke/i);

    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionRestore: {
            insertionPoint: 'snapshotDerivedLatestHarnessExecution->executionResultRegister',
            route: '/api/harness/operator-action/run',
            derivedKey: 'latestHarnessExecution',
            outputPath: latestHarnessExecution.resolvedOutputPath,
          },
        },
        null,
        2,
