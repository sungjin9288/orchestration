...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-299');
const port = 4599;
const baseUrl = `http://127.0.0.1:${port}`;

const appJs = fs.readFileSync(appPath, 'utf8');

assert.match(appJs, /function renderArtifacts\(data\) \{/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-299 server');
}
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessArtifactsRegister: {
            derivedKey: 'harnessConsumerBrief',
            insertionPoint: 'artifactsDeck->harnessRegister->surfaceGrid',
