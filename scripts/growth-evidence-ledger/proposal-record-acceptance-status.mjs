...
const MODE = `${BASE}-review-acceptance-status`;
const SCRIPT_PATH = 'scripts/growth-evidence-ledger/proposal-record-acceptance-status.mjs';
const PREVIOUS_STATUS = 'scripts/growth-evidence-ledger/proposal-record-review-status.mjs';
const NEXT_SLICE = `${BASE}-review-acceptance-finalization`;
const NEXT_COMMAND = 'node scripts/growth-evidence-ledger/proposal-record-acceptance-finalization-status.mjs';
const STATUS_LABEL =
...
const requiredReviewBlockedActions = [
...
function summarizeSources(sources, missingSources) {
  return {
    ...
    statusLedgered: sourceMentions(
      sources.todo,
      'growth-evidence-ledger-proposal-record-acceptance-alias-status-readonly-post-m7-1905',
    ),
    filenameLimitLessonCaptured: /filesystem filename limits/.test(sources.lessons),
    engineRoutesPastAcceptance: sourceMentions(sources.engine, NEXT_SLICE),
    reflectionRoutesPastAcceptance: sourceMentions(sources.reflection, NEXT_SLICE),
  ...
  return [
    {
      ...
      ok:
        ...
        previousReadiness.reviewReadyForAcceptanceCheck === true,
    ...
    },
    {
      ...
      ok:
        ...
        requiredReviewBlockedActions.every((action) => blockedActions.includes(action)),
...
function buildAcceptanceEnvelope({ reviewEnvelope, acceptanceFindings, acceptanceBlockedActions }) {
  ...
  const acceptanceRecord = {
    acceptanceId: `${BASE}-review-acceptance-candidate`,
    sourceReviewId: reviewEnvelope.reviewId || null,
    targetQueueContract: 'growth-proposal-queue-status',
    targetSchema: 'proposalRecord',
    acceptancePurpose:
      'Accept reviewed dry-run evidence only for a later read-only finalization check.',
    acceptanceFindings,
  ...
  return {
    ...
    compatibility: {
      ...
      acceptanceDoesNotApprove:
        acceptanceBlockedActions.includes(
          'accept-dry-run-finalization-review-acceptance-finalization-review-acceptance-as-approval',
...
function buildPayload() {
  ...
  const reviewEnvelope = reviewResult.payload?.reviewEnvelope?.reviewEnvelope || {};
  const reviewFindings = reviewEnvelope.reviewFindings || [];
  const previousReadiness = reviewResult.payload?.readiness || {};
  const previousSafety = reviewResult.payload?.safetyBoundary || {};
  ...
  return {
    ok,
    mode: MODE,
    posture: 'local-read-only-ledger-proposal-record-acceptance-alias',
    ...
    nextRecommendedSlice: {
      ...
      reason:
        'The dry-run review evidence is accepted only for a read-only finalization check; the next safe slice can finalize accepted evidence without approving, creating, persisting, implementing, or mutating proposal records.',
    ...
    liveInputObservation: {
      reviewStatusOk: reviewResult.ok,
      reviewStatusExit: reviewResult.status,
    },
    failures: {
      missingSources,
