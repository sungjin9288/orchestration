...
const SCRIPT_PATH = 'scripts/growth-evidence-ledger/proposal-record-acceptance-finalization-review-status.mjs';
const PREVIOUS_STATUS = 'scripts/growth-evidence-ledger/proposal-record-acceptance-finalization-status.mjs';
const NEXT_SLICE = `${BASE}-acceptance`;
const NEXT_COMMAND = 'node scripts/growth-evidence-ledger/proposal-record-acceptance-finalization-review-acceptance-status.mjs';
const STATUS_LABEL =
...
function summarizeSources(sources, missingSources) {
  return {
    ...
    statusLedgered: sourceMentions(
      sources.todo,
      'growth-evidence-ledger-proposal-record-acceptance-finalization-review-alias-status-readonly-post-m7-1907',
    ),
    filenameLimitLessonCaptured: /filesystem filename limits/.test(sources.lessons),
    engineRoutesPastAcceptanceFinalizationReview: sourceMentions(sources.engine, NEXT_SLICE),
    reflectionRoutesPastAcceptanceFinalizationReview: sourceMentions(sources.reflection, NEXT_SLICE),
...
function buildReadiness({ finalizationEnvelope, reviewEnvelope, sourceSummary }) {
  return {
    ...
    engineReflectionAdvanced:
      sourceSummary.engineRoutesPastAcceptanceFinalizationReview && sourceSummary.reflectionRoutesPastAcceptanceFinalizationReview,
...
function buildPayload() {
  ...
  return {
    ok,
    mode: MODE,
    posture: 'local-read-only-ledger-proposal-record-acceptance-finalization-review-alias',
    ...
    sourceSummary,
    inputStatuses: {
      proposalRecordAcceptanceFinalizationAlias: {
