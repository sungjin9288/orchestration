...
const MODE = `${BASE}-review-status`;
const SCRIPT_PATH = 'scripts/growth-evidence-ledger/proposal-record-review-status.mjs';
const PREVIOUS_STATUS = 'scripts/growth-evidence-ledger/proposal-record-finalization-status.mjs';
const NEXT_SLICE = `${BASE}-review-acceptance`;
const NEXT_COMMAND = 'node scripts/growth-evidence-ledger/proposal-record-acceptance-status.mjs';
const STATUS_LABEL =
...
function summarizeSources(sources, missingSources) {
  return {
    ...
    statusLedgered: sourceMentions(
      sources.todo,
      'growth-evidence-ledger-proposal-record-review-alias-status-readonly-post-m7-1904',
    ),
    filenameLimitLessonCaptured: /filesystem filename limits/.test(sources.lessons),
    engineRoutesPastReview: sourceMentions(sources.engine, NEXT_SLICE),
    reflectionRoutesPastReview: sourceMentions(sources.reflection, NEXT_SLICE),
...
function buildReviewEnvelope({ finalizationEnvelope, reviewFindings, reviewBlockedActions }) {
  ...
  const reviewRecord = {
    reviewId: `${BASE}-review-candidate`,
    ...
    reviewPurpose: 'Review finalized dry-run evidence only for a later read-only acceptance check.',
    reviewFindings,
...
function buildPayload() {
  ...
  const finalizationEnvelope =
    finalizationResult.payload?.finalizationEnvelope?.finalizationEnvelope || {};
  const finalizationFindings = finalizationEnvelope.finalizationFindings || [];
  const previousReadiness = finalizationResult.payload?.readiness || {};
  const previousSafety = finalizationResult.payload?.safetyBoundary || {};
  ...
  return {
    ok,
    mode: MODE,
    posture: 'local-read-only-ledger-proposal-record-review-alias',
    ...
    sourceSummary,
    inputStatuses: {
      proposalRecordFinalizationAlias: {
    ...
    nextRecommendedSlice: {
      ...
      reason:
        'The dry-run finalization evidence is reviewed only for a read-only acceptance check; the next safe slice can accept reviewed evidence without approving, creating, persisting, implementing, or mutating proposal records.',
    ...
    liveInputObservation: {
      finalizationStatusOk: finalizationResult.ok,
      finalizationStatusExit: finalizationResult.status,
    },
    failures: {
      missingSources,
