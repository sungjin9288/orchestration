...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-330 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-330-'));
  const inputPath = path.join(tempDir, 'hidden-command-summary.txt');
  const outputPath = path.join(tempDir, 'hidden-command-summary.md');

  fs.writeFileSync(inputPath, 'Execution hidden command summary smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHiddenCommandSummary: {
