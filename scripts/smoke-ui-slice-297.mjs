...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-297');
const port = 4597;
const baseUrl = `http://127.0.0.1:${port}`;

const appJs = fs.readFileSync(appPath, 'utf8');

assert.match(appJs, /function renderTaskboard\(data\) \{/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-297 server');
}
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessTaskboardRegister: {
            derivedKey: 'harnessConsumerBrief',
            insertionPoint: 'taskboardDeck->harnessRegister->bootstrapPanel',
