...
assert.match(deliverablesLeftDeck, /heading:\s*'결과 보고실'/);
assert.match(
  deliverablesLeftDeck,
  /copy:\s*'결과 보고실 왼쪽 패널도 현재 보고 판단, 다음 행동, 연결 근거부터 먼저 보여 줍니다\.'/,
);
...
assert.match(
  ...
  /copy:\s*'결과 보고실 오른쪽 패널은 현재 보고 묶음, 결재선, 다음 후속을 먼저 보여 주고 깊은 점검은 아래로 미룹니다\.'/,
);
