...
assert.match(councilConfig, /rank: '회의 리드'/);
assert.match(councilConfig, /rank: '전략 역할'/);
assert.match(councilConfig, /rank: '설계 역할'/);
assert.match(councilConfig, /rank: '실행 역할'/);
assert.match(councilConfig, /orderLabel: '역할 순서 1'/);
assert.match(councilConfig, /orderLabel: '역할 순서 2'/);
assert.match(councilConfig, /orderLabel: '역할 순서 3'/);
assert.match(councilConfig, /orderLabel: '역할 순서 4'/);
assert.match(councilConfig, /현재 결론과 다음 인계 판단을 한 지점에서 정리합니다\./);
assert.match(councilConfig, /목표 해석과 범위 조정을 맡는 전략 역할입니다\./);
assert.match(councilConfig, /설계 파급을 줄이고 시스템 경계를 지키는 역할입니다\./);
assert.match(councilConfig, /첫 실행 단위를 정리하고 바로 인계 가능한 수준으로 나눕니다\./);
