...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-323');
const port = 4624;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-323 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-323-'));
  const inputPath = path.join(tempDir, 'hidden-output-copy.txt');
  const outputPath = path.join(tempDir, 'hidden-output-copy.md');

  fs.writeFileSync(inputPath, 'Execution hidden output copy smoke\n', 'utf8');
