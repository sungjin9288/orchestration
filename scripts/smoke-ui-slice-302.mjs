...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-302');
const port = 4602;
const baseUrl = `http://127.0.0.1:${port}`;

const appJs = fs.readFileSync(appPath, 'utf8');

assert.match(appJs, /function renderDeliverables\(data\) \{/);
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-302 server');
}
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessDeliverablesRegister: {
            derivedKey: 'harnessConsumerBrief',
            insertionPoint: 'deliverablesDeck->harnessRegister->surfaceGrid',
