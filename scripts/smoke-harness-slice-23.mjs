...
assert.equal(payload.summary.primaryCommand, primaryHarness?.command ?? null);
assert.equal(payload.summary.primaryCommand, 'markitdown');
  ...
  JSON.stringify(
    {
      ok: true,
      runScript,
      primaryCommand: payload.summary.primaryCommand,
    },
    null,
    2,
  ),
