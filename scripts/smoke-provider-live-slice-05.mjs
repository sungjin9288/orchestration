...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-provider-live-slice-05');
...
function createFixtureProject() {
  const projectPath = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-provider-live-slice-05-'));
  fs.writeFileSync(path.join(projectPath, 'README.md'), '# provider-live-slice-05\n', 'utf8');
  return projectPath;
}
...
assert.equal(taskBreakerResult.run.summary.inputArtifactIds[0], plannerResult.artifact.id);
assert.equal(taskBreakerResult.run.summary.inputArtifactIds[1], architectResult.artifact.id);
assert.equal(taskBreakerResult.run.summary.inputRunIds[0], plannerResult.run.id);
assert.equal(taskBreakerResult.run.summary.inputRunIds[1], architectResult.run.id);
...
assert.equal(builderPreflightResult.run.summary.planArtifactId, plannerResult.artifact.id);
assert.equal(builderPreflightResult.run.summary.planRunId, plannerResult.run.id);
assert.equal(builderPreflightResult.run.summary.architectureArtifactId, architectResult.artifact.id);
...
assert.match(preflightArtifact.content, /^## Input Summary$/m);
assert.ok(
  ['request-builder-live-mutation-approval', 'architect', 'task-breaker', 'human gate'].includes(
    builderPreflightResult.run.summary.nextStage,
  ),
);

let approval = null;

if (builderPreflightResult.run.summary.nextStage === 'request-builder-live-mutation-approval') {
  approval = runtime.requestBuilderLiveMutationApproval({
    taskId: task.id,
  });
  assert.equal(approval.allowedNextAction, 'builder-live-mutation');
  assert.equal(approval.targetArtifactId, builderPreflightResult.artifact.id);
  assert.equal(approval.targetRunId, builderPreflightResult.run.id);
} else if (builderPreflightResult.run.summary.nextStage === 'human gate') {
  assert.ok(builderPreflightResult.decisionInboxItem);
} else {
  assert.equal(builderPreflightResult.decisionInboxItem, null);
}
  ...
  JSON.stringify(
    {
      ...
      breakdownArtifactId: taskBreakerResult.artifact.id,
      builderPreflightRunId: builderPreflightResult.run.id,
      preflightArtifactId: builderPreflightResult.artifact.id,
      builderPreflightNextStage: builderPreflightResult.run.summary.nextStage,
