...
assert.match(appJs, /council:\s*\{\s*copy: '참모진이 추천안과 방향을 정렬합니다\.',\s*kicker: '협의'/);
assert.match(appJs, /'decision-inbox':\s*\{\s*copy: '현재 안건과 다음 처리를 판단합니다\.',\s*kicker: '결재'/);
assert.match(appJs, /deliverables:\s*\{\s*copy: '현재 보고 판단과 다음 행동을 확인합니다\.',\s*kicker: '보고'/);
assert.match(appJs, /execution:\s*\{\s*copy: '현재 작전 판단과 다음 행동을 조정합니다\.',\s*kicker: '작전'/);
assert.match(appJs, /logs:\s*\{\s*copy: '현재 run과 다음 확인을 빠르게 훑습니다\.',\s*kicker: '실행기록'/);
assert.match(appJs, /mission:\s*\{\s*copy: '현재 안건 판단과 바로 이동을 시작합니다\.',\s*kicker: '접수'/);
assert.match(appJs, /taskboard:\s*\{\s*copy: '현재 실행 셀과 다음 실행을 조정합니다\.',\s*kicker: '실행셀'/);
  ...
  JSON.stringify(
    {
      ...
      surfaceKickerBaseline: {
        dockMetadata: ['증적', '협의', '결재', '보고', '작전', '실행기록', '접수', '실행셀'],
        fallback: '표면',
      },
    },
    null,
