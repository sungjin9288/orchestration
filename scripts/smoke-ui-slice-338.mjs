...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-338');
const port = 4639;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-338 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-338-'));
  const inputPath = path.join(tempDir, 'hidden-metadata-groups.txt');
  const outputPath = path.join(tempDir, 'hidden-metadata-groups.md');

  fs.writeFileSync(inputPath, 'Execution hidden metadata groups smoke\n', 'utf8');
  ...
  try {
    ...
    assert.ok(harnessConsumerStatus?.statusCard?.primaryHarnessId);
    assert.ok(harnessConsumerStatus?.statusCard?.primaryRunner);
    assert.ok(harnessConsumerStatus?.operatorAction?.repoNativeCommand);

    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHiddenMetadataGroups: {
