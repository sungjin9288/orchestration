...
const sentinelSecret = 'provider-slice-10-secret-sentinel';
...
const TARGET_FILE = 'docs/decision-memo.md';
...
function createApiPayload(outputText, options = {}) {
  return {
    payload: {
      id: options.providerRunId || 'resp-provider-slice-10',
...
## Slice Goal
Draft one bounded decision memo inside the knowledge-work pack.

## Intended Deliverable
- decision memo
- target file: docs/decision-memo.md
      ...
      artifact: {
        ...
        approvedAssumptions:
          options.artifact?.approvedAssumptions || [
            'The deliverable should stay as one bounded decision memo draft.',
          ],
        noArchitectureChangeStatement:
          options.artifact?.noArchitectureChangeStatement ||
          'No architecture change is approved by this note. Downstream work stays inside the current decision-memo drafting boundary.',
      ...
      artifact: {
        orderedSubTasks:
          options.artifact?.orderedSubTasks || [
            'Confirm the decision memo boundary and required sections.',
            'Prepare the bounded decision memo draft.',
            'Review the memo for explicit next action and provenance trace.',
          ],
        checkpoints:
          options.artifact?.checkpoints || [
            'checkpoint 1: confirm memo scope',
        ...
        reviewTriggerPoints:
          options.artifact?.reviewTriggerPoints || ['Trigger review after the bounded memo draft exists.'],
        ...
        executionBoundarySummary:
          options.artifact?.executionBoundarySummary || [
            'Builder handoff stays bounded to docs/decision-memo.md only.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
      ...
      artifact: {
        ...
        intendedChanges:
          options.artifact?.intendedChanges || [
            'Create one bounded decision memo file with the required section structure.',
          ],
        risks:
          options.artifact?.risks || [
            'Fail closed if the decision memo loses explicit next action or provenance trace.',
          ],
        verificationPlan:
          options.artifact?.verificationPlan || [
            'Run reviewer against the generated memo and verify the rubric outcome.',
          ],
        ...
        escalationTriggers:
          options.artifact?.escalationTriggers || [
            'Escalate if the deliverable widens beyond docs/decision-memo.md.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
      ...
      artifact: {
        changeSummary:
          options.artifact?.changeSummary || [
            'Prepare one bounded decision memo draft inside the approved knowledge-work target file.',
          ],
        targetFiles: options.artifact?.targetFiles || [TARGET_FILE],
        ...
        risks:
          options.artifact?.risks || [
            'Fail closed if the decision memo update leaves the approved target or loses provenance.',
          ],
        verificationNotes:
          options.artifact?.verificationNotes || [
            'Prepare the decision memo draft for knowledge-work reviewer handoff only.',
          ],
      },
      normalizedResult: {
        blockers: options.normalizedResult?.blockers || [],
        ...
        summary:
          options.normalizedResult?.summary ||
          'Builder live mutation prepared a bounded decision memo draft for reviewer handoff.',
        decisionTitle: options.normalizedResult?.decisionTitle || '',
        decisionPrompt: options.normalizedResult?.decisionPrompt || '',
      },
    }),
...
function buildMutationMarker(approvalId, relativePath) {
  return `provider-slice-10 ${approvalId} ${relativePath}`;
}

function createDecisionMemoContent(taskTitle, approvalId, { includeRecommendation = true } = {}) {
  const marker = buildMutationMarker(approvalId, TARGET_FILE);
  const recommendationSection = includeRecommendation
    ? `## Recommendation
...
## Decision
Prepare one bounded decision memo for the next operator move.

## Context
- Scope for this memo: one reviewable local-first recommendation
- The memo should stay bounded and reviewable

${recommendationSection}## Options Considered
...
function createRuntimeForLabel(label) {
  const runtimeRoot = path.join(repoRoot, 'var', `runtime-provider-slice-10-${label}`);
  const projectRoot = path.join(repoRoot, 'var', `provider-slice-10-${label}-project`);
...
async function prepareKnowledgeWorkLiveContext(label, buildContent) {
  ...
  const project = runtime.createProject({
    name: `provider-slice-10 ${label}`,
    pack: 'knowledge-work',
    projectPath: projectRoot,
    provider: {
      adapter: 'openai-responses',
      mode: 'live',
      model: 'provider-slice-10-operator-model',
      env: {
  ...
  const task = runtime.createTask({
    ...
    title: `provider-slice-10 ${label}`,
    intent: 'Draft a bounded decision memo and keep reviewer criteria explicit.',
  ...
  const plannerResult = await coordinator.runPlanner({
    ...
    routingOutcome: createRoutingOutcome(
      'Draft one bounded decision memo and keep the review rubric explicit.',
    ),
  });
  ...
  const memoContent = buildContent(task.title, approvalRecord.id);
...
const happyContext = await prepareKnowledgeWorkLiveContext('happy', (taskTitle, approvalId) =>
  createDecisionMemoContent(taskTitle, approvalId),
);
...
assert.match(String(happyBuilderRequestBody.input || ''), /docs\/decision-memo\.md/);
...
happyContext.queuedFetch.push(
  createReviewerApiPayload(happyReviewerAnchor, {
    artifact: {
      ...
      verificationEvidence: ['changed files: docs/decision-memo.md'],
      knowledgeWorkRubric: {
        deliverableType: 'decision-memo',
    ...
    normalizedResult: {
      ...
      summary: 'Reviewer passed the bounded decision memo and leaves commit-package as an explicit downstream step.',
      decisionTitle: '',
      decisionPrompt: '',
    },
    providerRunId: 'resp-reviewer-knowledge-work-decision-memo-pass',
  }),
);
...
assert.match(String(happyReviewerRequestBody.input || ''), /^## Decision$/m);
assert.match(happyReviewArtifact.content, /deliverable file reviewed: docs\/decision-memo\.md/);
...
const changesContext = await prepareKnowledgeWorkLiveContext(
  'changes-requested',
  (taskTitle, approvalId) => createDecisionMemoContent(taskTitle, approvalId, { includeRecommendation: false }),
);
...
changesContext.queuedFetch.push(
  createReviewerApiPayload(changesReviewerAnchor, {
    artifact: {
      ...
      findings: ['Missing required section: Recommendation.'],
    ...
    normalizedResult: {
      ...
      needsDecision: false,
      nextStage: 'builder',
      summary: 'Return to builder to complete the missing recommendation section.',
      decisionTitle: '',
      decisionPrompt: '',
    },
    providerRunId: 'resp-reviewer-knowledge-work-decision-memo-changes',
  }),
);
...
assert.match(changesArtifact.content, /Missing required section: Recommendation\./);
assert.match(changesArtifact.content, /deliverable file reviewed: docs\/decision-memo\.md/);
  ...
  (reviewerAnchor) =>
    createReviewerApiPayload(reviewerAnchor, {
      ...
      normalizedResult: {
        ...
        needsDecision: false,
        nextStage: 'human gate',
        summary: 'Incorrectly attempt to pass the decision memo despite a missing required section.',
        decisionTitle: '',
        decisionPrompt: '',
      },
      providerRunId: 'resp-reviewer-knowledge-work-decision-memo-invalid-pass',
    }),
