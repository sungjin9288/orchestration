...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-192');
...
const helperSourceByName = new Map([
  ...
  ['getInboxDetailSnapshot', taskDetailSnapshots],
]);
...
function loadHelpers() {
  ...
  return {
    ...
    getInboxDetailSnapshot: context.getInboxDetailSnapshot,
...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Decision Inbox detail summary aligned to the current evidence rail handoff truth.',
  };
...
function renderDecisionDetailTokens({ createToken, evidenceRail, inboxDetailSnapshot }) {
  return `
    <div class="token-row token-row-compact">
      ...
      ${inboxDetailSnapshot.tokens.join('')}
    </div>
  `;
}
...
assert.match(appJs, /const decisionDetailEvidenceState = getExecutionEvidenceRail\(selectedTask, data\);/);
assert.match(
  appJs,
  /renderNarrativeDeck\(\{[\s\S]*heading: '현재 상태와 다음 처리를 먼저 보는 결재 상세'[\s\S]*createToken\(\s*`현재:\$\{decisionDetailEvidenceState\.currentOwnerLabel\}`,\s*decisionDetailEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{decisionDetailEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*\.\.\.inboxDetailSnapshot\.tokens/s,
);
...
try {
  ...
  const pendingProject = runtime.createProject({
    name: 'decision-detail-handoff-pending-approval',
    projectPath: repoRoot,
  });
  const pendingTask = runtime.createTask({
    projectId: pendingProject.id,
    title: 'Decision detail handoff pending approval',
    intent: 'Keep the Decision Inbox detail summary aligned while builder approval is pending.',
  });
  ...
  const pendingSnapshot = getInboxDetailSnapshot(
    pendingItem,
    pendingTaskRecord,
    pendingApprovalRecord,
  );
  const pendingMarkup = renderDecisionDetailTokens({
    createToken,
    evidenceRail: pendingRail,
    inboxDetailSnapshot: pendingSnapshot,
  });
  ...
  assert.match(
    pendingMarkup,
    new RegExp(`결재:${getApprovalStatusDisplay(pendingApprovalRecord.status)}`),
  );

  runtime.resetRuntime();

  const reviewedProject = runtime.createProject({
    name: 'decision-detail-handoff-reviewed-follow-up',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Decision detail handoff reviewed follow-up',
    intent: 'Keep the Decision Inbox detail summary aligned after review evidence exists.',
  });
  ...
  const reviewedSnapshot = getInboxDetailSnapshot(reviewedItem, reviewedTaskRecord, null);
  const reviewedMarkup = renderDecisionDetailTokens({
    createToken,
    evidenceRail: reviewedRail,
    inboxDetailSnapshot: reviewedSnapshot,
  });
  ...
  assert.match(reviewedMarkup, /결정/);

  console.log(
    JSON.stringify(
      {
        ok: true,
        decisionInboxDetailHandoffTokens: {
          ...
          verifiedCases: [
            'decision detail summary shows current and next handoff tokens from the shared rail truth',
            'pending builder approval path keeps the detail summary grounded in the existing approval status flow',
            'reviewed follow-up decision path keeps the detail summary aligned with the critic-to-human-gate handoff',
          ],
        },
      },
      null,
