...
requireNoCliArgs(process.argv.slice(2), {
  mode: 'growth-remediation-source-mutation-apply-closure-status',
});
...
const SOURCE_MUTATION_APPLY_CLOSURE_STATES = [
  'source-mutation-apply-closure-not-started',
  'source-mutation-apply-closure-ready',
  'source-mutation-apply-closure-blocked',
  'source-mutation-apply-closure-ready-for-finalization-contract',
  'source-mutation-apply-closure-rejected',
  'source-mutation-apply-closure-deferred',
  ...
  'stale-apply-closure-blocked',
  'closed-no-action',
];

const SOURCE_MUTATION_APPLY_CLOSURE_DECISION_TYPES = [
  ...
  'reject-source-mutation-apply-closure',
  'defer-source-mutation-apply-closure',
  'hold-baseline',
];

const SOURCE_MUTATION_APPLY_CLOSURE_EVIDENCE_TYPES = [
...
const SOURCE_MUTATION_APPLY_CLOSURE_BLOCKER_TYPES = [
...
function summarizeSources(sources) {
  ...
  return {
    ...
    harnessMentionsSourceMutationApplyClosure:
      /growth-remediation-source-mutation-apply-closure-status/.test(harnessBaseline),
    completionReadinessMentionsSourceMutationApplyClosure:
      /growth-remediation-source-mutation-apply-closure-status/.test(completionReadiness),
    ledgerMentionsSourceMutationApplyClosure:
      /growth-remediation-source-mutation-apply-closure-status-readonly-post-m7-839/.test(
        todo,
      ),
    verificationIncludesSourceMutationApplyClosure:
      /growth-remediation-source-mutation-apply-closure-status/.test(verificationStatus),
    ...
    currentApplyExecutionDocumented: /current apply execution record|current apply execution chain/.test(
      plan,
    ),
    currentApplyDispatchDocumented: /current apply dispatch record|current apply execution chain/.test(
      plan,
    ),
    currentApplyExecutionReadinessDocumented:
      /current apply execution readiness record|current apply execution chain/.test(plan),
    currentApplyPreflightDocumented: /current apply preflight record|current apply execution chain/.test(
      plan,
    ),
    ...
    applyClosureSeparateFromMutation:
      /apply closure status stays separate from actual patch application/.test(plan) ||
      /apply lifecycle closure status stays separate from actual patch application/.test(plan),
    applyFinalizationStillBlocked: /before\s+source\s+mutation apply finalization can be\s+considered/.test(
      plan,
    ),
...
const sourceMutationApplyClosureSchema = {
  sourceMutationApplyClosureRecord: fields(
    [
      'applyClosureId',
  ...
  sourceMutationApplyClosureDecision: fields(
    [
      'decisionId',
      'applyClosureId',
      'applyResultAcceptanceId',
  ...
  sourceMutationApplyClosureBlocker: fields(
    [
      'blockerId',
      'applyClosureId',
      'applyResultAcceptanceId',
  ...
  sourceMutationApplyClosureIndex: fields(
    [
      'indexId',
      'applyClosureRefs',
...
const sourceMutationApplyClosureRules = [
  {
    id: 'apply-closure-requires-current-apply-result-acceptance',
    rule: 'source mutation apply closure can only bind to one current apply result acceptance record and its apply execution chain',
  },
  {
    id: 'apply-closure-is-not-source-mutation',
    rule: 'source mutation apply closure status may mark finalization-contract readiness, but it cannot apply patches, mutate source, mutate accepted records, open actual apply closure, finalize, or execute remediation',
  },
  {
    id: 'apply-closure-requires-clean-baseline-and-operator-dispatch-intent',
    rule: 'clean baseline proof, exact scope lock, target lock, baseline digest, operator dispatch intent, reviewer notes, acceptance criteria, acceptance decision notes, and closure decision notes must be present before source mutation apply finalization can be considered',
  },
  {
    id: 'apply-closure-requires-verification-dry-run-and-rollback-proof',
    rule: 'patch draft refs, diff preview refs, verification output refs, dry-run proof refs, rollback proof refs, source-of-truth refs, task ledger refs, and negative evidence clearance must be present before source mutation apply finalization can be considered',
  },
  {
    id: 'failed-stale-broad-dirty-or-mutating-apply-closure-blocks-finalization',
    rule: 'stale result acceptance, stale result review, stale result, stale execution, dirty or untracked baseline, changed patch or diff proof, failed verification, missing rollback proof, missing closure decision note, unresolved negative evidence, unapproved file touches, or any attempted source mutation blocks apply finalization readiness',
  },
];
...
const requiredFieldsSatisfied = Object.values(sourceMutationApplyClosureSchema).every(
...
const ok =
  ...
  sourceSummary.harnessMentionsSourceMutationApplyClosure &&
  sourceSummary.completionReadinessMentionsSourceMutationApplyClosure &&
  sourceSummary.ledgerMentionsSourceMutationApplyClosure &&
  sourceSummary.verificationIncludesSourceMutationApplyClosure &&
  sourceSummary.sourceMutationApplyFinalizationNextDocumented &&
  ...
  sourceSummary.applyClosureSeparateFromMutation &&
  sourceSummary.applyFinalizationStillBlocked &&
...
const payload = {
  ok,
  mode: 'growth-remediation-source-mutation-apply-closure-status',
  posture: 'local-read-only-remediation-source-mutation-apply-closure-contract',
  currentHead: {
  ...
  schemaVersion: 'growth-remediation-source-mutation-apply-closure-status/v0',
  sourceSummary,
  vocabulary: {
    sourceMutationApplyClosureStates: SOURCE_MUTATION_APPLY_CLOSURE_STATES,
    sourceMutationApplyClosureDecisionTypes: SOURCE_MUTATION_APPLY_CLOSURE_DECISION_TYPES,
    sourceMutationApplyClosureEvidenceTypes: SOURCE_MUTATION_APPLY_CLOSURE_EVIDENCE_TYPES,
    sourceMutationApplyClosureBlockerTypes: SOURCE_MUTATION_APPLY_CLOSURE_BLOCKER_TYPES,
  },
  sourceMutationApplyClosureSchema,
  sourceMutationApplyClosureRules,
  sourceMutationApplyClosureState: {
    realSourceMutationApplyClosureFileAdopted: false,
    discoveredSourceMutationApplyClosureRecords: 0,
    applyFinalizationAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    remediationExecutionAllowed: false,
    currentStatus: 'contract-only-no-active-source-mutation-apply-closure',
  },
  readiness: {
    requiredFieldsSatisfied,
    sourceMutationApplyClosureRecordTypes: Object.keys(sourceMutationApplyClosureSchema).length,
    ...
    applyClosureAndMutationSeparate: true,
    applyFinalizationAllowed: false,
    sourceMutationAllowed: false,
    acceptedRecordMutationAllowed: false,
    rollbackExecutionAllowed: false,
    ...
    readyForSourceMutationApplyFinalizationStatus: true,
  },
  nextRecommendedSlice: {
    ...
    reason:
      'Source mutation apply closure is now modeled as read-only; the next safe slice should define finalization readiness without applying patches or mutating source.',
