...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-195');
...
function loadHelpers() {
  ...
  return {
    ...
    getInboxKindDisplay: context.getInboxKindDisplay,
    getInboxStatusDisplay: context.getInboxStatusDisplay,
...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Decision Inbox meta strip aligned to the current evidence rail handoff truth.',
  };
...
function renderDecisionMetaTokens({
  createToken,
  evidenceRail,
  getInboxKindDisplay,
  getInboxStatusDisplay,
...
assert.match(
  appJs,
  /<p class="detail-key">결재 기본 정보<\/p>[\s\S]*createToken\(\s*`현재:\$\{decisionDetailEvidenceState\.currentOwnerLabel\}`,\s*decisionDetailEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{decisionDetailEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*createToken\(getInboxKindDisplay\(selectedItem\.kind\), getInboxTone\(selectedItem\)\)[\s\S]*createToken\(getInboxStatusDisplay\(selectedItem\.status\), selectedItem\.status === 'pending' \? 'warning' : 'success'\)/,
);
...
try {
  ...
  const pendingProject = runtime.createProject({
    name: 'decision-meta-pending-approval',
    projectPath: repoRoot,
  });
  const pendingTask = runtime.createTask({
    projectId: pendingProject.id,
    title: 'Decision meta pending approval',
    intent: 'Keep the Decision Inbox meta strip aligned while builder approval is pending.',
  });
  ...
  const pendingMarkup = renderDecisionMetaTokens({
    createToken,
  ...
  assert.match(pendingMarkup, new RegExp(getInboxKindDisplay(pendingItem.kind)));
  assert.match(pendingMarkup, new RegExp(getInboxStatusDisplay(pendingItem.status)));

  runtime.resetRuntime();

  const reviewedProject = runtime.createProject({
    name: 'decision-meta-reviewed-follow-up',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Decision meta reviewed follow-up',
    intent: 'Keep the Decision Inbox meta strip aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderDecisionMetaTokens({
    createToken,
  ...
  assert.match(reviewedMarkup, new RegExp(getInboxKindDisplay(reviewedItem.kind)));
  assert.match(reviewedMarkup, new RegExp(getInboxStatusDisplay(reviewedItem.status)));

  console.log(
    JSON.stringify(
      {
        ok: true,
        decisionInboxMetaHandoffTokens: {
          ...
          verifiedCases: [
            'decision meta strip shows current and next handoff tokens from the shared rail truth',
            'pending builder approval path keeps the meta strip aligned with the existing approval kind and status tokens',
            'reviewed follow-up decision path keeps the meta strip aligned after the task reaches the critic-to-human-gate handoff',
          ],
        },
      },
      null,
