...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-375');
const port = 4676;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-375 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-375-'));
  const inputPath = path.join(tempDir, 'history-title-wording.txt');
  const outputPath = path.join(tempDir, 'history-title-wording.md');

  fs.writeFileSync(inputPath, 'Execution history title wording smoke\n', 'utf8');
