...
async function createPlanningChain(coordinator, taskId) {
  const routingOutcome = {
    ...
    missingContext: [],
    scopeStatement:
      'Keep the Taskboard detail summary aligned to the current evidence rail handoff truth.',
  };
...
function renderTaskDetailSummaryTokens({ createToken, evidenceRail, lifecycleDisplay }) {
  return `
    <div class="token-row token-row-compact">
      ...
      ${createToken(lifecycleDisplay, 'neutral')}
    </div>
  `;
}
...
const { createToken, getExecutionEvidenceRail, getGuardReasonDisplay, getTaskLifecycleDisplay } =
  loadHelpers();

assert.match(appJs, /const taskDetailEvidenceState = getExecutionEvidenceRail\(task, data\);/);
assert.match(
  appJs,
  /renderNarrativeDeck\(\{[\s\S]*eyebrow: '작업판 판단 요약'[\s\S]*createToken\(\s*`현재:\$\{taskDetailEvidenceState\.currentOwnerLabel\}`,\s*taskDetailEvidenceState\.blockedReason \? 'danger' : 'accent',\s*\)[\s\S]*createToken\(`다음:\$\{taskDetailEvidenceState\.nextHandoffLabel\}`,\s*'neutral'\)[\s\S]*createToken\(getTaskLifecycleDisplay\(task\.lifecycleState\), 'neutral'\)/s,
);
...
try {
  ...
  const blockedProjectPath = fs.mkdtempSync(
    path.join(os.tmpdir(), 'orchestration-ui-slice-189-provider-'),
  );
  const blockedProject = runtime.createProject({
    name: 'taskboard-detail-maker-blocked',
    projectPath: blockedProjectPath,
    provider: { mode: 'live' },
  });
  const blockedTask = runtime.createTask({
    projectId: blockedProject.id,
    title: 'Taskboard detail maker blocked handoff',
    intent: 'Keep the taskboard detail summary grounded when maker is blocked.',
  });
  ...
  const blockedMarkup = renderTaskDetailSummaryTokens({
    createToken,
    evidenceRail: blockedRail,
    lifecycleDisplay: getTaskLifecycleDisplay(blockedTaskRecord.lifecycleState),
  });
  ...
  assert.match(
    blockedMarkup,
    new RegExp(getTaskLifecycleDisplay(blockedTaskRecord.lifecycleState)),
  );

  runtime.resetRuntime();

  const reviewedProject = runtime.createProject({
    name: 'taskboard-detail-reviewed-handoff',
    projectPath: repoRoot,
  });
  const reviewedTask = runtime.createTask({
    projectId: reviewedProject.id,
    title: 'Taskboard detail reviewed handoff',
    intent: 'Keep the taskboard detail summary aligned after review evidence exists.',
  });
  ...
  const reviewedMarkup = renderTaskDetailSummaryTokens({
    createToken,
    evidenceRail: reviewedRail,
    lifecycleDisplay: getTaskLifecycleDisplay(reviewedTaskRecord.lifecycleState),
  });
  ...
  assert.match(
    reviewedMarkup,
    new RegExp(getTaskLifecycleDisplay(reviewedTaskRecord.lifecycleState)),
  );
    ...
    JSON.stringify(
      {
        ...
        taskboardDetailHandoffTokens: {
          ...
          verifiedCases: [
            'taskboard detail summary shows current and next handoff tokens from the shared rail truth',
            'maker-blocked path keeps the detail summary grounded in the existing first blocked reason flow',
            'reviewed path keeps the detail summary aligned with the current critic-to-human-gate handoff',
          ],
        },
      },
      null,
