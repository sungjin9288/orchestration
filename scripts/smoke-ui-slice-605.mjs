...
assert.match(harnessExecutionTokens, /return execution\?\.requestId \|\| execution\?\.executionId \|\| '';/);
...
assert.match(harnessExecutionTokens, /export function getHarnessPrimaryTokenLabel\(execution\) \{/);
assert.match(harnessExecutionTokens, /return `대표:\$\{execution\.harnessId\}`;/);
assert.match(harnessExecutionTokens, /export function getHarnessRequestTokenLabel\(requestId\) \{/);
assert.match(harnessExecutionTokens, /return `요청:\$\{requestId\}`;/);
assert.match(harnessExecutionTokens, /export function getHarnessPolicyReportTokenLabel\(isPolicyReport\) \{/);
assert.match(harnessExecutionTokens, /return isPolicyReport \? '정책 리포트' : '';/);
...
assert.doesNotMatch(appJs, /const visibleHarnessRequestId =\s+visibleHarnessExecutionResult\?\.requestId \|\| visibleHarnessExecutionResult\?\.executionId \|\| '';/);
assert.doesNotMatch(appJs, /const hiddenHarnessRequestId =\s+hiddenHarnessExecutionResult\?\.requestId \|\| hiddenHarnessExecutionResult\?\.executionId \|\| '';/);
...
assert.doesNotMatch(appJs, /\$\{\s*canRenderVisibleHarnessPrimaryToken\s+\?\s+createToken\(visibleHarnessPrimaryTokenLabel, 'neutral'\)/);
assert.doesNotMatch(appJs, /\$\{\s*canRenderVisibleHarnessRequestToken\s+\?\s+createToken\(visibleHarnessRequestTokenLabel, 'neutral'\)/);
assert.doesNotMatch(appJs, /\$\{\s*canRenderVisibleHarnessPolicyReportToken\s+\?\s+createToken\(visibleHarnessPolicyReportTokenLabel, 'neutral'\)/);
