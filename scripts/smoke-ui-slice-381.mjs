...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-381');
const port = 4682;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-381 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-381-'));
  const inputPath = path.join(tempDir, 'visible-header-token-wording.txt');
  const outputPath = path.join(tempDir, 'visible-header-token-wording.md');

  fs.writeFileSync(inputPath, 'Execution visible header token wording smoke\n', 'utf8');
  ...
  try {
    ...
    assert.match(runPayload.harnessExecution?.outputPreview || '', /Execution visible header token wording smoke/i);
