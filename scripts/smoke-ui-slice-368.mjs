...
const runtimeRoot = path.join(repoRoot, 'var', 'runtime-ui-slice-368');
const port = 4669;
...
async function waitForServer() {
  ...
  throw new Error('Timed out waiting for ui-slice-368 server');
}
...
async function main() {
  ...
  let stderr = '';
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orchestration-ui-slice-368-'));
  const inputPath = path.join(tempDir, 'history-action-density.txt');
  const outputPath = path.join(tempDir, 'history-action-density.md');

  fs.writeFileSync(inputPath, 'Execution history action density smoke\n', 'utf8');
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        {
          ok: true,
          harnessExecutionHistoryActionDensity: {
            insertionPoint: 'executionHistoryRegister->historyActionDensity->compactActionRow',
