...
const completionVerificationEvidence = [
  ...
  /The proposal generation packet, proposal generation handoff, and proposal application\s+decision packet status scripts now run upstream status checks through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper/,
  /the helper uses the current Node binary and\s+the shared large JSON buffer, while before\/after JSON diffs keep the emitted status payloads\s+unchanged/,
