...
const completionVerificationEvidence = [
  ...
  /The durable proposal record planning, implementation, and proposal application status\s+scripts now run upstream status checks and focused smokes through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper/,
  /the helper uses the current Node binary and\s+the shared large JSON buffer, while before\/after JSON\s+diffs keep the emitted status payloads\s+unchanged/,
