...
const completionFocusEvidence = [
  ...
  /one deterministic local proposal draft\s+from exactly one existing Growth Evidence Ledger candidate/,
  /packet is decision input only/,
  /does not approve planning, implementation, provider-assisted generation, durable proposal record\s+creation, proposal application, memory persistence, source mutation, commit, or push/,
  /docs\/41_proposal-generation-operator-decision-handoff\.md/,
  /fielded response\s+shape/,
  /rejects shortcut approvals such as `continue`, `do everything`, and\s+`approve all`/,
  /keeps planning blocked until the full\s+`approve-proposal-generation-planning-only` field set is present/,
  /Close-out position: the handoff evidence is complete as a read-only product\/planning surface/,
  /waiting for a separate fielded operator decision before it can\s+write a proposal generation plan/,
  /must remain limited to one deterministic local\s+draft from one existing Growth Evidence Ledger candidate/,
  /rollback plan,\s+focused smoke plan, and aggregate verification evidence/,
  ...
  /required `1\/1`, informational `170\/170`, total `171\/171`/,
...
const growthSourceEvidence = [
  ...
  /aggregate `171\/171`, UI QA `28\/28`, zero-open backlog/,
...
const completionVerificationEvidence = [
  ...
  /the helper uses the current Node binary and the shared large JSON buffer, while before\/after\s+JSON diffs keep the emitted status payloads unchanged/,
  /node scripts\/smoke-completion-gate-inventory-current-evidence\.mjs/,
  /node scripts\/vnext-development-audit-status\.mjs[\s\S]*The vNext audit status runs its upstream growth, reflection, proposal\s+readiness, lifecycle review, and source mutation implementation checks through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted status\s+payload/,
  /node scripts\/vnext-growth-dashboard-evidence-depth-status\.mjs[\s\S]*It runs the vNext audit status through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted status\s+payload/,
  /node scripts\/vnext-authority-expansion-review-status\.mjs[\s\S]*It runs upstream vNext audit, growth dashboard, proposal review, and\s+memory readiness statuses through the shared `scripts\/vnext-status-assertions\.mjs` `runStatus`\s+helper without changing the emitted status payload/,
  /node scripts\/vnext-authority-implementation-decision-packet-status\.mjs[\s\S]*It runs upstream vNext\s+audit and authority expansion review statuses through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted status\s+payload/,
  /node scripts\/vnext-durable-proposal-record-planning-preview-status\.mjs[\s\S]*It runs upstream status checks\s+through the shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the\s+emitted status payload/,
  /node scripts\/vnext-durable-proposal-record-implementation-plan-status\.mjs[\s\S]*It runs upstream status checks through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted status\s+payload/,
  /node scripts\/vnext-durable-proposal-record-implementation-status\.mjs[\s\S]*It runs the focused smoke through the\s+shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted\s+status payload/,
  /node scripts\/vnext-proposal-application-decision-packet-status\.mjs[\s\S]*It runs upstream status checks through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper, using the current Node binary and shared\s+large JSON buffer without changing the emitted status payload/,
  /node scripts\/vnext-proposal-application-operator-decision-handoff-status\.mjs[\s\S]*It runs upstream status checks through the shared\s+`scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted status\s+payload/,
  /node scripts\/vnext-proposal-application-implementation-plan-status\.mjs[\s\S]*It runs upstream status checks through the\s+shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted\s+status payload/,
  /node scripts\/vnext-proposal-application-implementation-decision-handoff-status\.mjs[\s\S]*It runs upstream status checks through\s+the shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing the emitted\s+status payload/,
  /node scripts\/vnext-proposal-application-implementation-status\.mjs[\s\S]*It runs\s+the focused smoke through the shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper\s+without changing the emitted status payload/,
  /node scripts\/vnext-proposal-application-source-mutation-implementation-status\.mjs[\s\S]*It runs the focused\s+smoke through the shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper without changing\s+the emitted status payload/,
  ...
  /completion inventory counts,\s+aggregate `171\/171`, UI QA `28\/28`, zero-open backlog/,
