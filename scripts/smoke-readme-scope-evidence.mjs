...
const completionVerificationEvidence = [
  ...
  /The proposal generation packet and handoff status scripts, plus the proposal application\s+decision packet through implementation decision handoff status scripts, now run upstream status\s+checks through the shared `scripts\/vnext-status-assertions\.mjs` `runStatus` helper/,
  /the helper uses\s+the current Node binary and the shared large JSON buffer, while before\/after JSON diffs keep the\s+emitted status payloads unchanged/,
