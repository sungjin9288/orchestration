...
assert.match(readme, /The latest implementation lane closed a behavior-preserving module extraction pass/);
assert.match(readme, /Coordinator git\/diff\/path helpers now live in `src\/execution\/coordinator\/git\.js`/);
assert.match(readme, /src\/runtime\/proposal-records\.js/);
assert.match(readme, /ui\/markdown-artifact-parsing\.js/);
