...
const completionFocusEvidence = [
  /The current development focus is post-completion lifecycle-close-finalization-acceptance status\s+evidence close-out/,
...
const lifecycleCloseFocusEvidence = [
  /The active close-out slice is\s+`growth-lifecycle-close-finalization-acceptance-status-smoke-readability-post-m7-2314`/,
  /keeps\s+the lifecycle-close-finalization-acceptance status smoke behavior-preserving and read-only/,
  /source summary, vocabulary, schema, readiness, next-slice,\s+safety\s+boundary, invalid-argument, plan, and cross-document evidence groups/,
  /source\s+close-finalization-acceptance-status script, runtime behavior, UI behavior, provider configuration,\s+memory persistence, proposal generation\/application, source mutation authority, commit, push,\s+connector reach, automation, and lifecycle semantics remain unchanged/,
];
