...
const completionFocusEvidence = [
  ...
  /current runtime is schema v8 with an\s+additive WorkflowCheckpoint map, read-only recovery classification, and exact resume\/cancel routes/,
  ...
  /runtime remains schema v8, the preview remains `persisted=false` and\s+`missionDone=false`/,
  ...
  /required `1\/1`, informational `200\/200`, total `201\/201`/,
  /UI QA is required `34\/34`/,
...
const growthSourceEvidence = [
  ...
  /aggregate `201\/201`, UI QA `34\/34`, zero-open backlog/,
...
const completionVerificationEvidence = [
  ...
  /completion inventory counts,\s+aggregate `201\/201`, UI QA `34\/34`, zero-open backlog/,
