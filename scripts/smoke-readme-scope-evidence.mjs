...
const completionFocusEvidence = [
  ...
  /DEC-166 keeps StaffingPlan immutable, DEC-167 and DEC-168\s+record the Council-first binding plan and handoff/,
  /DEC-169 implements one exact StaffingEntry\s+binding plus alignment-only stop boundary/,
  /New direct local-stub starts require this durable entry/,
  /DEC-170 and DEC-171 define one explicit local\s+dependency-ready role step with durable attempt evidence, and DEC-172 implements that bounded\s+schema-v19 path/,
  /Provider-backed WorkOrders, retry\/rework\/recovery, parallel or autonomous\s+scheduling, memory application, source\/profile mutation, approval bypass, and runtime-agent\s+commit\/push\/release remain blocked/,
  ...
  /Planning-only `DEC-201` defines the Stage 5F Builder rework source mutation contract/,
  /`DEC-203` implements one dedicated local-stub path/,
  /Planning-only `DEC-204` defines the Stage 5G Reviewer re-execution boundary/,
  /`DEC-205` records its complete fielded\s+implementation handoff/,
  /`DEC-206`\s+implements that exact local-stub path/,
  /Planning-only `DEC-207` defines the Stage 5H rework QA boundary/,
  /`DEC-208` records its complete fielded\s+implementation handoff/,
  /Exact `DEC-209`\s+implements that bounded path/,
  /Planning-only `DEC-210` defines the Stage 5I response-only rework DeliveryPackage boundary/,
  /`DEC-211` records its complete\s+fielded implementation handoff/,
  /Exact `DEC-212`\s+implements the dedicated deeply frozen runtime projection/,
  /Planning-only `DEC-213` defines the Stage 5J durable rework DeliveryPackage boundary/,
  /`DEC-214` records its complete\s+fielded implementation handoff/,
  /active-before-worker persistence, source-derived targets, realpath-open and\s+single-link file identity, FD-level baseline verification, credential-sensitive source refusal,\s+bounded decode, post-mutation digests, non-clobbering rollback, source-current exact replay/,
  ...
  /`DEC-168`\s+records the complete fielded implementation handoff/,
  /`DEC-169` consumes\s+that decision and implements one immutable schema-v18 StaffingEntry/,
  /`DEC-162` reconciles the pre-implementation schema-v16 AI Company evidence/,
  /`DEC-165` clarifies\s+the exact active-project, fresh blueprint and role-source digest/,
  ...
  /DEC-161 browser baseline plus DEC-162 through\s+DEC-169 StaffingPlan and StaffingEntry implementation evidence, DEC-170 through DEC-172 Stage 3\s+scheduler evidence, and DEC-173 through DEC-176 Stage 4A planning and response-only implementation\s+evidence plus DEC-177 through DEC-179 Stage 4B planning, handoff, and fixed first-attempt\s+implementation evidence plus DEC-180 through DEC-182 Stage 4C retry planning, handoff, and exact\s+implementation evidence plus DEC-183 through DEC-185 Stage 6A inspect-only planning, handoff, and\s+implementation evidence plus DEC-186 through DEC-188 Stage 5 Reviewer rework preview planning,\s+handoff, and response-only implementation evidence plus DEC-189 through DEC-191 Stage 5B durable\s+ReworkPlan planning, fielded handoff, and record-only implementation evidence plus DEC-192 through\s+DEC-194 Stage 5C ReworkPlanAcceptance planning, handoff, and evidence-only implementation evidence\s+plus DEC-195 through DEC-197 Stage 5D Builder rework preflight planning, fielded handoff, and\s+bounded implementation evidence plus DEC-198 through DEC-200 Stage 5E mutation Approval,\s+DEC-201 through DEC-203 Stage 5F source mutation, DEC-204 through DEC-206 Stage 5G Reviewer\s+re-execution planning, handoff, and bounded implementation evidence, and DEC-207 through DEC-209\s+Stage 5H rework QA planning, fielded handoff, and bounded implementation evidence plus DEC-210\s+through DEC-212 Stage 5I response-only rework DeliveryPackage planning, fielded handoff, and\s+bounded implementation evidence plus DEC-213 through DEC-215 Stage 5J durable rework\s+DeliveryPackage planning, fielded handoff, and record-and-inspect implementation evidence plus\s+DEC-216 through DEC-218 Stage 5K acceptance planning, fielded handoff, and implementation evidence,\s+plus DEC-219 through DEC-221 Stage 6B quarantine planning, fielded handoff, and implementation, and\s+DEC-222 through DEC-224 Stage 6C QA-only safe-resume planning, fielded handoff, and implementation,\s+plus DEC-225 through DEC-227 Stage 7A reviewed Mission context attachment planning, fielded\s+handoff, and bounded implementation, plus DEC-228 through DEC-229 Stage 7B Strategist context\s+consumption planning and fielded handoff,\s+pass the focused,\s+aggregate, and UI QA\s+gates/,
  ...
  /LLM-native visual-system convergence is accepted by `DEC-159`/,
  /Every existing route, action, count, evidence disclosure, runtime record, review gate, approval gate,\s+provider contract, and state boundary remains unchanged/,
  /docs\/111_llm-native-first-viewport-corrective-redesign-plan\.md/,
  /first-viewport corrective redesign is accepted by `DEC-160`/,
  /`project_path` is the dominant\s+input/,
  /docs\/112_agent-operations-desk-visual-redesign-plan\.md/,
  /reference-led Agent Operations Desk redesign is accepted by `DEC-161`/,
  /graphite coded project rail/,
